...
const Friends = {
    ...
    try {
      ...
      for (const sol of solicitudes) {
        ...
        const avatarHTML = photoURL
          ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover;" onerror="Utils.avatarFallback(this)"><div class="nivel-badge" style="background: ${colorNivel}; color: white; text-shadow: 0 0 1px black;">${nivel}</div></div>`
  ...
  _renderListaAmigos({ amigos, niveles }) {
    ...
    for (const amigo of amigos) {
      ...
      const avatarHTML = photoURL
        ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover;" onerror="Utils.avatarFallback(this)"><div class="nivel-badge" style="${badgeStyle}">${nivel}</div></div>`
  ...
  async abrirModalAmigo(uid) {
    ...
    try {
      ...
      // Si es amigo, intentamos obtener los datos reales
      if (esAmigo) {
        try {
      ...
      const avatarHTML = photoURL
        ? `<img src="${Utils.escapeHTML(photoURL)}" style="width:60px; height:60px; border-radius:50%; object-fit:cover; border:2px solid var(--accent-blue);" onerror="Utils.avatarFallback(this)">`
