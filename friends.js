...
const Friends = {
  ...
  async _refreshCurrentUserData() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        // 🔥 Mismo motivo que en actualizarBadgeSolicitudes: se escribe en
        // el contador de amistad, no en el total, para no pisar el de
        // entrenador (ver AppState._recalcularSolicitudesPendientes).
  ...
  async cargarPanelAlumnos() {
    const container = document.getElementById('entrenadorAlumnosList');
    if (!container || !AppState.currentUserId) return;
    container.innerHTML = '<p style="text-align:center; padding:20px; color:var(--text-secondary);">Cargando...</p>';
    try {
      const userDoc = await firebaseServices.db.collection('users').doc(AppState.currentUserId).get();
      ...
      const amigosData = [];
      for (let i = 0; i < friendIds.length; i += 10) {
        const lote = friendIds.slice(i, i + 10);
      ...
      for (const amigo of amigosData) {
        ...
        if (alumnosAceptados.includes(amigo.uid)) {
          accionHTML = `<span style="color:var(--zone-2); font-weight:bold; font-size:11px; white-space:nowrap;">✅ ALUMNO</span>
  ...
  async quitarAlumno(uid) {
    const confirmado = await Utils.confirm('Quitar alumno', '¿Dejar de poder enviarle sesiones a este amigo? Tendría que aceptarte de nuevo si quieres volver a entrenarlo.');
    if (!confirmado) return;
    const ok = await Storage.removeAlumno(AppState.currentUserId, uid);
    if (!ok) { Utils.showToast('Error al quitar alumno', 'error'); return; }
    Utils.showToast('Alumno eliminado', 'info');
    this.cargarPanelAlumnos();
  },
...
console.log('✅ friends.js v3.54 - Nuevo panel de entrenador/alumnos: un entrenador ya solo puede enviar sesiones a los amigos que le acepten expresamente como tal (ver Storage.sendTrainerRequest y Friends.cargarPanelAlumnos)');
