...
const Friends = {
  ...
  // 🔥 FIX (ronda de optimización Firestore): antes precargaba TODOS los
  // amigos (con 100 amigos, 100 lecturas de gamification/{uid} en cada
  // login, la inmensa mayoría de las cuales el usuario nunca llega a abrir
  // en esa sesión). Se opta por la opción (b) del audit -- limitar la
  // precarga a los primeros _MAX_PERFILES_PRECARGA amigos -- en vez de (a)
  // eliminarla del todo (perdería la carga instantánea para el caso normal
  // de <20 amigos, con diferencia el más común) o (c) precarga por scroll
  // (más invasivo de implementar sin tocar el resto de friends.js). El
  // resto de amigos sigue cargando perezosamente y en caché al abrir su
  // modal (ver abrirModalAmigo más abajo, que ya contempla un _perfilCache
  // sin entrada para ese uid) -- no se toca ese camino.
  _MAX_PERFILES_PRECARGA: 20,

  async precargarPerfilesAmigos() {
    try {
      const friendIds = AppState.currentUserData?.friendIds || [];
      if (!friendIds.length) return;
      const idsAPrecargar = friendIds.slice(0, this._MAX_PERFILES_PRECARGA);
      await Promise.all(idsAPrecargar.map(async (uid) => {
        try {
  ...
  async obtenerSolicitudesPendientes() {
    ...
    // 🔥 FIX (ronda de optimización Firestore): si los listeners en tiempo
    // real de app.js (unsubscribeFriendRequests / unsubscribeSolicitudesEnviadas)
    // ya han disparado su primer snapshot, AppState.solicitudesRecibidasCache /
    // solicitudesEnviadasCache están al día -- se reutilizan sin volver a
    // consultar Firestore. Solo se cae al .get() de siempre si algún día
    // no hay listener corriendo (null = "aún sin datos en vivo").
    if (AppState.solicitudesRecibidasCache !== null && AppState.solicitudesEnviadasCache !== null) {
      return {
        enviadas: AppState.solicitudesEnviadasCache,
        recibidas: AppState.solicitudesRecibidasCache
      };
    }
    try {
  ...
  async cargarPanelAlumnos(forceRefresh = false) {
    ...
    try {
      ...
      let datos;
      if (!friendIds.length) {
        datos = { amigosData: [], alumnosAceptados: [], pendientePorUid: {}, niveles: {} };
      } else {
  ...
  _renderPanelAlumnos({ amigosData, alumnosAceptados, pendientePorUid, niveles }) {
    const container = document.getElementById('entrenadorAlumnosList');
    if (!container) return;
    if (!amigosData.length) {
      container.innerHTML = '<p style="text-align:center; padding:20px; color:var(--text-secondary); line-height:1.5;">Todavía no tienes amigos a quien invitar. Añade amigos desde Comunidad primero.</p>';
      return;
    }
    let html = '';
...
console.log('✅ friends.js v3.58 - "Entrenado por" colorea cada tarjeta según el nivel del entrenador');
