...
const Friends = {
  ...
  async cargarSolicitudes(tipo) {
    ...
    try {
      ...
      // Los datos de cada usuario de la solicitud (perfil + nivel) se
      // piden todos EN PARALELO antes de montar el HTML, en vez de uno a
      // uno con await secuencial dentro del bucle: antes, con 10-15
      // solicitudes pendientes, esta pantalla se quedaba varios segundos
      // en blanco esperando a que cada usuario se resolviera antes de
      // pasar al siguiente (mismo patrón que ya se corrigió en
      // Wall.showLikesModal).
      const otroUids = solicitudes.map(sol => tipo === 'recibidas' ? sol.from : sol.to);
      const [usuarios, niveles] = await Promise.all([
        Promise.all(otroUids.map(uid => Storage.getUser(uid))),
        this.getNivelesDirectos(otroUids)
      ]);
      let html = '';
      solicitudes.forEach((sol, idx) => {
        const otroUid = otroUids[idx];
        const otroUser = usuarios[idx];
  ...
  async abrirModalAmigo(uid) {
    ...
    try {
      ...
      setTimeout(() => {
        // Igual que en Wall: el contenido del modal se acaba de reemplazar
        // por completo (innerHTML = contenido, arriba), así que estos son
        // botones nuevos que nunca tuvieron el listener puesto -- el
        // removeEventListener previo no hacía nada, solo trabajo de más.
...
console.log('✅ friends.js v3.46 - Mejora de privacidad en perfiles de no amigos');