...
// Versión: 3.61 - DIAGNÓSTICO "no se ha podido mandar la invitación":
//                invitarAlumno() ahora captura el error real que puede
//                lanzar Storage.sendTrainerRequest() (ver storage.js v3.34)
//                y muestra error.code en el propio toast, en vez del
//                mensaje genérico de siempre -- para identificar la causa
//                exacta la próxima vez que falle.
// Versión: 3.60 - OPTIMIZACIÓN LECTURAS FIRESTORE: _limpiarAmigosHuérfanos()
//                ahora deduplica la promesa en vuelo por uid (mismo patrón
//                que SessionInvites._cargarHistorialSiHaceFalta). Antes,
//                si dos sitios la llamaban casi a la vez con el mismo
//                friendIds recién leído -- típicamente Profile.cargarPerfil()
//                y este mismo módulo (cargarListaAmigos/cargarPanelAlumnos)
//                durante el arranque, ver AppState.precargarDatos -- cada
//                uno lanzaba su propia consulta 'in' por chunks sobre
//                'users', duplicando esas lecturas. Ahora el segundo
//                llamante reutiliza la misma promesa en vez de repetir la
//                consulta. Profile.cargarPerfil() (profile.js v10.13) ya
//                delega en esta función en vez de tener su propia copia
//                de la misma lógica.
// Versión: 3.59 - OPTIMIZACIÓN LECTURAS FIRESTORE (punto 1 de la ronda de
//                revisión): cargarListaAmigos() y cargarPanelAlumnos()
//                leían cada amigo DOS veces al cargar -- una dentro de
//                _limpiarAmigosHuérfanos() (la consulta 'in' a 'users'
//                ya trae el documento completo, no solo si existe, pero
//                se descartaba todo salvo el id) y otra justo después,
//                pidiendo cada amigo suelto con .doc(fid).get() para
//                sacar username/profile/etc. _limpiarAmigosHuérfanos()
//                devuelve ahora también docsPorId con lo ya leído; los
//                dos llamantes lo reutilizan y solo piden sueltos (como
//                red de seguridad) los que por lo que sea no vinieran
//                ahí -- p.ej. si esa comprobación falló del todo. Cada
//                amigo se lee una vez por carga en vez de dos.
// Versión: 3.58 - cargarEntrenadoPor() colorea cada tarjeta según el
//                nivel del entrenador (borde y fondo teñido, igual que ya
//                se hace en otros sitios con Gamification.getColorByLevel),
//                en vez de una tarjeta neutra para todos.
// Versión: 3.57 - (1) Reordenado a petición del usuario: en la pestaña
//                🎯 Entrenador, "Nueva sesión" + historial va primero y
//                "Mis alumnos" debajo (con muchos amigos, antes había que
//                bajar hasta el fondo para crear una sesión). (2) Caché de
//                "Mis alumnos" pasada de memoria a sessionStorage (como
//                cargarListaAmigos): sobrevive a recargar la app, y ya no
//                se ve NINGUNA animación de carga -- se pinta al instante
//                desde caché, y solo se vuelve a pedir cuando hay un
//                cambio real (ver _claveCacheAlumnos, y los listeners de
//                app.js, que ahora refrescan la caché aunque la pestaña no
//                esté visible en ese momento). (3) Nuevo
//                cargarEntrenadoPor()/dejarDeSerAlumno(): en el perfil
//                propio aparece "Entrenado por" con quién te ha aceptado
//                como alumno, y un botón para dejar de serlo sin dejar de
//                ser su amigo (ver Storage.dejarDeSerAlumno).
// Versión: 3.56 - FIX "Explorar usuarios se queda en CARGANDO para
//                siempre, nunca aparece nadie": _esperarPrimeraExplorar()
//                dependía por completo de que el listener onSnapshot de
//                _iniciarListenerExplorar llamara alguna vez a su éxito o
//                a su error -- si el canal de streaming en tiempo real de
//                Firestore está bloqueado por la red (algo que no afecta
//                a peticiones HTTPS normales como .get()), eso nunca
//                pasa, y no había ningún aviso ni caída a un estado de
//                error: la animación de "CARGANDO" se quedaba pegada sin
//                fin. Ahora, si el listener no responde en 9s, se hace un
//                .get() de una sola vez como red de seguridad para
//                rellenar la lista igual que si hubiera venido del
//                listener (y si ese .get() también falla, se pinta "no
//                hay usuarios" en vez de colgarse). Ver
//                _esperarPrimeraExplorar().
// Versión: 3.55 - Panel "Mis alumnos" (cargarPanelAlumnos): (1) FIX del
//                parpadeo "se ve cargando y de repente se ponen" -- el
//                texto fijo "Cargando..." se sustituye por la misma
//                animación de "CARGANDO" con letras de colores que ya usa
//                "Explorar usuarios" (_mostrarCargando/Utils._animarTextoDorado).
//                (2) el resultado se guarda ahora en memoria
//                (_alumnosPanelCache) y se reutiliza sin volver a leer
//                Firestore ni repetir la animación cada vez que se abre
//                la pestaña -- solo se refresca de verdad cuando cambia
//                algo real (invitar/cancelar/quitar aquí, o aceptar/
//                rechazar desde el otro lado, ver los listeners de
//                app.js). Se separa en cargarPanelAlumnos (datos) +
//                _renderPanelAlumnos (pintado) para poder repintar desde
//                caché sin volver a pedir nada.
// Versión: 3.54 - Nuevo panel "🎯 Entrenador → Mis alumnos": un entrenador
//                ya no puede enviar sesiones a cualquier amigo suyo sin
//                más -- tiene que invitarlo expresamente a "ser su
//                alumno" y ese amigo tiene que aceptar (nueva subpestaña
//                "🎯 Entrenador" en Comunidad > Solicitudes, para quien
//                recibe la invitación). Ver Storage.sendTrainerRequest /
//                acceptTrainerRequest / rejectTrainerRequest / removeAlumno
//                (storage.js) y session-invites.js::_precargarUsuarios,
//                que ahora solo deja elegir entre los alumnos aceptados.
...
const Friends = {
  ...
  _esperarPrimeraExplorar() {
    ...
    return new Promise(resolve => {
      ...
      this._exploreListenerResolvers.push(resolverUnaVez);
      // 🔧 FIX "se queda cargando para siempre, nunca aparece nadie": si
      // el canal en tiempo real de Firestore está bloqueado (algunas
      // redes/proxies/extensiones cortan el streaming pero sí dejan pasar
      // peticiones HTTPS normales), onSnapshot no llega a llamar NUNCA ni
      // a su éxito ni a su error -- antes eso dejaba la animación de
      // "CARGANDO" pegada para siempre, sin ningún aviso ni caída a un
      // estado de error. Red de seguridad: si a los 9s el listener no ha
      // respondido, se hace una lectura .get() de una sola vez (no
      // depende del canal de streaming) para rellenar la caché igual que
      // si hubiera venido del listener; si ese propio .get() también
      // falla, se marca cargando:false igualmente para que se pinte
      // "no hay usuarios" en vez de quedarse colgado sin fin.
      setTimeout(async () => {
        if (resuelto) return;
        try {
          const snap = await firebaseServices.db.collection('users').orderBy('username').limit(20).get();
          if (resuelto) return; // el listener respondió justo mientras pedíamos este .get()
          this._exploreCache.users = snap.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
          this._exploreCache.lastDoc = snap.docs[snap.docs.length - 1] || null;
          this._exploreCache.cargando = false;
        } catch (error) {
          console.error('Error en el .get() de respaldo de Explorar usuarios:', error);
          if (this._exploreCache) this._exploreCache.cargando = false;
        } finally {
          resolverUnaVez();
        }
  ...
  // 🔥 Antes pedía cada nivel con su propio getNivelDirecto() en paralelo
  // (hasta 10 peticiones HTTP simultáneas por lote) -- el coste de
  // lecturas de Firestore es el mismo (se factura por documento leído,
  // no por petición), pero tarda más en responder por los viajes de ida
  // y vuelta. Ahora se pide el lote entero en UNA sola consulta 'in' por
  // FieldPath.documentId(), igual que ya hace _limpiarAmigosHuérfanos
  // más abajo: mismo coste, menos latencia.
  async getNivelesDirectos(uids) {
    const niveles = {};
    const CHUNK = 10; // límite de Firestore para cláusulas 'in'
    for (let i = 0; i < uids.length; i += CHUNK) {
      const batch = uids.slice(i, i + CHUNK);
      try {
        const snapshot = await firebaseServices.db.collection('gamification')
          .where(firebaseServices.FieldPath.documentId(), 'in', batch)
          .get();
        const encontrados = {};
        snapshot.forEach(doc => { encontrados[doc.id] = doc.data().level || 1; });
        batch.forEach(uid => { niveles[uid] = encontrados[uid] || 1; });
      } catch (e) {
        console.warn('Error obteniendo niveles en lote:', e);
        batch.forEach(uid => { niveles[uid] = 1; });
      }
    }
    return niveles;
  },

  // 🔥 FIX lectura duplicada: promesa en vuelo por uid (mismo patrón que
  // SessionInvites._cargarHistorialSiHaceFalta). Profile.cargarPerfil() y
  // este mismo módulo (cargarListaAmigos/cargarPanelAlumnos) pueden lanzar
  // esta comprobación EN PARALELO durante el arranque -- ver
  // AppState.precargarDatos -- sobre el mismo friendIds recién leído de
  // users/{uid}. Sin esto, cada llamada repetía la misma consulta 'in'
  // por chunks. Ahora, si ya hay una limpieza en curso para ese uid, se
  // devuelve esa misma promesa en vez de lanzar otra consulta idéntica.
  //
  // NOTA (supuesto implícito de esta dedup): la clave es solo el uid, no
  // el contenido de friendIds. Si dos llamadas concurrentes para el mismo
  // uid pasaran listas de friendIds DISTINTAS (hoy no ocurre: los tres
  // llamantes actuales leen users/{uid} de fresco justo antes de llamar,
  // así que siempre coinciden), la segunda llamada recibiría en silencio
  // el resultado calculado con el friendIds de la PRIMERA, ignorando el
  // suyo propio. Si en el futuro se añade un llamante que pase una lista
  // cacheada/vieja en vez de una recién leída, esta función necesitará
  // invalidar o comparar friendIds además del uid.
  _limpiezaAmigosHuérfanosEnVuelo: {},

  async _limpiarAmigosHuérfanos(userId, friendIds) {
    if (!friendIds || friendIds.length === 0) return { validIds: [], changed: false, docsPorId: {} };
    if (this._limpiezaAmigosHuérfanosEnVuelo[userId]) {
      return this._limpiezaAmigosHuérfanosEnVuelo[userId];
    }
    const promise = this._limpiarAmigosHuérfanosInterno(userId, friendIds);
    this._limpiezaAmigosHuérfanosEnVuelo[userId] = promise;
    try {
      return await promise;
    } finally {
      delete this._limpiezaAmigosHuérfanosEnVuelo[userId];
    }
  },

  async _limpiarAmigosHuérfanosInterno(userId, friendIds) {
    const validIds = [];
    // 🔥 FIX lecturas duplicadas: esta consulta 'in' ya trae el documento
    // COMPLETO de cada amigo (Firestore no tiene una forma más barata de
    // comprobar "¿existe?" que traer todo el doc), pero antes se
    // descartaba todo salvo el doc.id -- cargarListaAmigos() y
    // cargarPanelAlumnos() volvían a pedir esos mismos documentos justo
    // después, uno por uno, para sacar username/profile/etc. Ahora se
    // guardan aquí y los dos llamantes los reutilizan: cada amigo se lee
    // una sola vez por carga en vez de dos.
    const docsPorId = {};
    const chunks = [];
    ...
    try {
      for (const chunk of chunks) {
        ...
        snapshot.forEach(doc => {
          ...
          docsPorId[doc.id] = { uid: doc.id, ...doc.data() };
        });
      }
    } catch (error) {
      console.warn('No se pudo comprobar amigos huérfanos (se deja la lista tal cual):', error);
      // docsPorId se devuelve vacío -- los llamantes tienen su propio
      // fallback para pedir sueltos los que no vengan aquí.
      return { validIds: friendIds, changed: false, docsPorId: {} };
    }
    ...
    return { validIds, changed, docsPorId };
  },
  ...
  initEventListeners() {
    ...
    // 🔥 Delegación equivalente para las invitaciones de entrenador/alumno
    // (panel "Mis alumnos" del entrenador + subpestaña "🎯 Entrenador" de
    // Solicitudes recibidas).
    document.addEventListener('click', (e) => {
      const target = e.target.closest('[data-trainer-action]');
      if (!target) return;
      const action = target.getAttribute('data-trainer-action');
      const uid = target.getAttribute('data-uid');
      const username = target.getAttribute('data-username');
      const requestId = target.getAttribute('data-request-id');
      if (action === 'invitar' && uid) this.invitarAlumno(uid, username || 'tu amigo');
      else if (action === 'cancelar-invitacion' && requestId) this.cancelarInvitacionAlumno(requestId);
      else if (action === 'quitar' && uid) this.quitarAlumno(uid);
      else if (action === 'aceptar' && uid && requestId) this.aceptarSolicitudEntrenador(requestId, uid);
      else if (action === 'rechazar' && requestId) this.rechazarSolicitudEntrenador(requestId);
      else if (action === 'dejar-de-ser-alumno' && uid) this.dejarDeSerAlumno(uid, username || 'ese entrenador');
    });
  ...
  async cargarSolicitudes(tipo) {
    ...
    try {
      ...
      for (const sol of solicitudes) {
        ...
        const pillEntrenador = otroUser?.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : '';
        html += `
          <div class="solicitud-item" data-uid="${otroUid}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${otroUid}')">
            <div class="resultado-info">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre" style="color:${colorNombre};">${Utils.escapeHTML(usernameFormatted)}${pillEntrenador}</div>
  ...
  async enviarSolicitud(toUid, toUsername) {
    ...
    if (!confirmed) return;
    // 🔥 FIX toasts contradictorios ("✅ enviada" y luego "❌ error" de
    // golpe): antes el refresco de listas iba en el MISMO try que el
    // envío -- si ese refresco fallaba (aunque la solicitud ya se hubiera
    // mandado bien), saltaba el catch y se veía un error encima del
    // éxito. Ahora la acción principal y su refresco posterior van en
    // tries separados: solo un fallo de verdad al enviar muestra el rojo.
    let ok;
    try {
      ok = await Storage.sendFriendRequest(AppState.currentUserId, toUid);
    } catch (error) {
      console.error('Error enviando solicitud:', error);
      Utils.showToast('Error al enviar solicitud', 'error');
      return;
    }
    if (!ok) { Utils.showToast('No se pudo enviar la solicitud', 'error'); return; }
    ...
    try {
      ...
      console.error('Error refrescando listas tras enviar solicitud:', error);
    }
  },
  ...
  async _refreshCurrentUserData() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        // 🔥 Se escribe en el contador de AMISTAD (no en el total
        // directo): el total combina también las invitaciones de
        // entrenador pendientes, alimentadas por su propio listener en
        // tiempo real -- ver AppState._recalcularSolicitudesPendientes.
        const { recibidas } = await this.obtenerSolicitudesPendientes();
        AppState.solicitudesAmistadPendientesCount = recibidas.length;
        AppState._recalcularSolicitudesPendientes();
      }
  ...
  async aceptarSolicitud(requestId, fromUid, toUid) {
    ...
    // 🔥 FIX toasts contradictorios (verde "aceptada" seguido de rojo
    // "error al aceptar"): antes TODO el trabajo posterior a aceptar
    // (crear la conversación de chat si no existía, refrescar listas de
    // amigos/solicitudes, actualizar el badge) iba en el MISMO try que la
    // aceptación -- si cualquiera de esos pasos fallaba (un hipo de red
    // al crear la conversación, por ejemplo), saltaba el catch y se veía
    // "Error al aceptar" aunque la solicitud SÍ se hubiera aceptado de
    // verdad. Ahora solo un fallo real al aceptar muestra el rojo; los
    // pasos de después van en su propio try y, si fallan, se quedan en
    // consola -- las listas se autocorrigen en el siguiente refresco.
    try {
    ...
    try {
      ...
      console.error('Error tras aceptar solicitud (creando conversación / refrescando listas):', error);
    }
  },
  ...
  async rechazarSolicitud(requestId) {
    ...
    try {
      ...
      console.error('Error refrescando listas tras rechazar solicitud:', error);
    }
  },
  ...
  async cancelarSolicitud(requestId) {
    ...
    Utils.showToast(yaNoExiste ? 'La solicitud ya no existe' : '✅ Solicitud cancelada', yaNoExiste ? 'info' : 'success');
    try {
      this._invalidarCacheUsuarios();
      await this.cargarTodosUsuarios(true);
      await this.actualizarBadgeSolicitudes();
      ...
      console.error('Error refrescando listas tras cancelar solicitud:', error);
    }
  },

  // ==================== ENTRENADOR / ALUMNOS ====================
  // 🔥 Invitaciones de "quiero ser tu entrenador" recibidas -- viven en su
  // propia subpestaña dentro de Comunidad > Solicitudes (junto a
  // Recibidas/Enviadas de amistad), porque cualquier usuario puede
  // recibir una aunque él mismo no sea entrenador.
  async cargarSolicitudesEntrenador() {
    const container = document.getElementById('listaSolicitudesEntrenador');
    if (!container || !AppState.currentUserId) return;
    try {
      const solicitudes = await Storage.getTrainerRequestsRecibidas(AppState.currentUserId);
      if (!solicitudes.length) {
        container.innerHTML = '<p style="text-align:center; padding:20px;">No tienes invitaciones de entrenador pendientes</p>';
        return;
      }
      let html = '';
      for (const sol of solicitudes) {
        const otroUser = await Storage.getUser(sol.from);
        const usernameFormatted = Utils.capitalizeUsername(otroUser?.username || 'Usuario');
        const userTag = otroUser?.username ? `@${Utils.escapeHTML(otroUser.username)}` : 'cuenta no disponible';
        const photoURL = otroUser?.profile?.photoURL;
        const nivel = await this.getNivelDirecto(sol.from);
        const colorNivel = Gamification.getColorByLevel(nivel);
        const avatarHTML = photoURL
          ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover; border:2px solid ${colorNivel};" onerror="Utils.avatarFallback(this)"><div class="nivel-badge" style="background: ${colorNivel}; color: white; text-shadow: 0 0 1px black;">${nivel}</div></div>`
          : `<div class="resultado-avatar-wrapper"><div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center; border:2px solid ${colorNivel};">👤</div><div class="nivel-badge" style="background: ${colorNivel}; color: white; text-shadow: 0 0 1px black;">${nivel}</div></div>`;
        html += `
          <div class="solicitud-item" data-uid="${sol.from}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${sol.from}')">
            <div class="resultado-info">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre" style="color:${colorNivel};">${Utils.escapeHTML(usernameFormatted)}</div>
                <div class="resultado-username">${Utils.escapeHTML(userTag)} quiere ser tu entrenador</div>
              </div>
            </div>
            <div class="solicitud-botones">
              <button class="btn-amigo" data-trainer-action="aceptar" data-request-id="${sol.id}" data-uid="${sol.from}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary);">✓ Aceptar</button>
              <button class="btn-amigo eliminar" data-trainer-action="rechazar" data-request-id="${sol.id}">✗ Rechazar</button>
            </div>
          </div>
        `;
      }
      ...
      console.error('Error cargando solicitudes de entrenador:', error);
      container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar invitaciones</p>';
    }
  },

  async aceptarSolicitudEntrenador(requestId, fromUid) {
    const ok = await Storage.acceptTrainerRequest(requestId, fromUid, AppState.currentUserId);
    if (!ok) { Utils.showToast('Error al aceptar', 'error'); return; }
    Utils.showToast('✅ Ahora ese entrenador puede enviarte sesiones', 'success');
    try {
      await this._refreshCurrentUserData();
      await this.cargarSolicitudesEntrenador();
      await this.actualizarBadgeSolicitudes();
    } catch (error) {
      console.error('Error refrescando tras aceptar invitación de entrenador:', error);
    }
  },

  async rechazarSolicitudEntrenador(requestId) {
    const ok = await Storage.rejectTrainerRequest(requestId);
    if (!ok) { Utils.showToast('Error al rechazar', 'error'); return; }
    Utils.showToast('Invitación rechazada', 'info');
    try {
      await this.cargarSolicitudesEntrenador();
      await this.actualizarBadgeSolicitudes();
    } catch (error) {
      console.error('Error refrescando tras rechazar invitación de entrenador:', error);
    }
  },

  // 🔥 Caché de "Mis alumnos" persistida en sessionStorage (sobrevive a
  // recargar la app, igual que cargarListaAmigos) en vez de solo en
  // memoria: se pinta al instante sin ninguna animación de carga, y solo
  // se vuelve a pedir a Firestore cuando hay un cambio de verdad
  // (invitar/cancelar/quitar aquí abajo, o aceptar/rechazar/revocar desde
  // el otro lado, o un cambio en la lista de amigos -- ver los listeners
  // de app.js, que llaman con forceRefresh=true).
  _claveCacheAlumnos() {
    return `alumnos_panel_${AppState.currentUserId}`;
  },

  // Panel "Mis alumnos" del entrenador (subpestaña Perfil > 🎯
  // Entrenador): parte de la propia lista de amigos y, para cada uno,
  // muestra si ya es alumno aceptado, si tiene una invitación pendiente,
  // o un botón para invitarlo. Solo visible para un entrenador que no sea
  // admin (ver app.js::setCurrentUser) -- el admin ya puede enviar
  // sesiones a cualquier usuario sin necesitar esta relación.
  async cargarPanelAlumnos(forceRefresh = false) {
    const container = document.getElementById('entrenadorAlumnosList');
    if (!container || !AppState.currentUserId) return;

    const cacheKey = this._claveCacheAlumnos();
    if (!forceRefresh) {
      const cached = sessionStorage.getItem(cacheKey);
      if (cached) {
        try {
          this._renderPanelAlumnos(JSON.parse(cached));
          return;
        } catch (e) {}
      }
    }

    // 🔥 Sin animación de "cargando": si no hay caché todavía (primera
    // vez en esta sesión del navegador), se deja el contenedor tal cual
    // hasta que llegan los datos -- ni parpadeo ni texto de "cargando".
    try {
      const userDoc = await firebaseServices.db.collection('users').doc(AppState.currentUserId).get();
      const userData = userDoc.data() || {};
      let friendIds = userData.friendIds || [];
      const { validIds, changed, docsPorId } = await this._limpiarAmigosHuérfanos(AppState.currentUserId, friendIds);
      if (changed) friendIds = validIds;
      const alumnosAceptados = userData.alumnosAceptados || [];
      const maxAlumnos = userData.maxAlumnos ?? null;

      let datos;
      if (!friendIds.length) {
        datos = { amigosData: [], alumnosAceptados: [], pendientePorUid: {}, niveles: {}, maxAlumnos };
      } else {
        // 🔥 FIX lecturas duplicadas: mismo motivo que en cargarListaAmigos
        // -- _limpiarAmigosHuérfanos ya trajo estos documentos completos.
        const amigosData = [];
        const faltantes = [];
        for (const fid of friendIds) {
          if (docsPorId[fid]) amigosData.push(docsPorId[fid]);
          else faltantes.push(fid);
        }
        for (let i = 0; i < faltantes.length; i += 10) {
          const lote = faltantes.slice(i, i + 10);
          const snaps = await Promise.all(lote.map(fid => firebaseServices.db.collection('users').doc(fid).get()));
          snaps.forEach(doc => { if (doc.exists) amigosData.push({ uid: doc.id, ...doc.data() }); });
        }
        const enviadas = await Storage.getTrainerRequestsEnviadas(AppState.currentUserId);
        const pendientePorUid = {};
        enviadas.forEach(r => { pendientePorUid[r.to] = r.id; });
        const niveles = await this.getNivelesDirectos(amigosData.map(a => a.uid));
        datos = { amigosData, alumnosAceptados, pendientePorUid, niveles, maxAlumnos };
      }

      sessionStorage.setItem(cacheKey, JSON.stringify(datos));
      this._renderPanelAlumnos(datos);
    } catch (error) {
      console.error('Error cargando panel de alumnos:', error);
      // Si ya había algo pintado (de una caché previa), se deja tal cual
      // en vez de taparlo con un mensaje de error.
      if (!container.innerHTML.trim()) {
        container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar tus amigos</p>';
      }
    }
  },

  _renderPanelAlumnos({ amigosData, alumnosAceptados, pendientePorUid, niveles, maxAlumnos }) {
    const container = document.getElementById('entrenadorAlumnosList');
    if (!container) return;
    // 🔥 A petición del usuario: un admin puede poner un límite de atletas
    // (maxAlumnos) a un entrenador. Al llegar a ese número de atletas YA
    // ACEPTADOS: (1) arriba del todo se muestra el recuento "X / Y
    // atletas" (en rojo si ya está al límite), y (2) a los amigos que
    // todavía no son atletas ni tienen invitación pendiente ya no se les
    // ofrece el botón "INVITAR", sino un aviso en rojo.
    const limiteAlcanzado = maxAlumnos != null && alumnosAceptados.length >= maxAlumnos;
    const cabeceraHTML = maxAlumnos != null
      ? `<p style="text-align:center; font-size:12px; margin-bottom:10px; color:${limiteAlcanzado ? '#e74c3c' : 'var(--text-secondary)'};">👥 ${alumnosAceptados.length} / ${maxAlumnos} atletas${limiteAlcanzado ? ' · límite alcanzado' : ''}</p>`
      : '';
    if (!amigosData.length) {
      container.innerHTML = cabeceraHTML + '<p style="text-align:center; padding:20px; color:var(--text-secondary); line-height:1.5;">Todavía no tienes amigos a quien invitar. Añade amigos desde Comunidad primero.</p>';
      return;
    }
    let html = '';
    for (const amigo of amigosData) {
      ...
      let accionHTML;
      if (alumnosAceptados.includes(amigo.uid)) {
        accionHTML = `<span style="color:var(--zone-2); font-weight:bold; font-size:11px; white-space:nowrap;">✅ ATLETA</span>
                      <button class="btn-amigo eliminar" data-trainer-action="quitar" data-uid="${amigo.uid}">QUITAR</button>`;
      } else if (pendientePorUid[amigo.uid]) {
        accionHTML = `<button class="btn-amigo eliminar" data-trainer-action="cancelar-invitacion" data-request-id="${pendientePorUid[amigo.uid]}">⏳ CANCELAR</button>`;
      } else if (limiteAlcanzado) {
        accionHTML = `<span style="color:#e74c3c; font-weight:bold; font-size:11px; white-space:nowrap;">🔒 Límite alcanzado</span>`;
      } else {
        accionHTML = `<button class="btn-amigo" data-trainer-action="invitar" data-uid="${amigo.uid}" data-username="${Utils.escapeHTML(usernameFormatted)}">🎯 INVITAR</button>`;
      }
      html += `
        <div class="resultado-busqueda" style="justify-content:space-between;" data-uid="${amigo.uid}">
          <div class="resultado-info">
            ${avatarHTML}
            <div>
          ...
          <div style="display:flex; gap:8px; align-items:center;">${accionHTML}</div>
        </div>
      `;
    }
    container.innerHTML = cabeceraHTML + html;
  },

  async invitarAlumno(uid, username) {
    // 🔧 FIX (petición del usuario): comprobación local del límite de
    // atletas antes de llamar siquiera a Storage.sendTrainerRequest, por
    // si la caché del panel (sessionStorage) estuviera desactualizada --
    // el límite real y definitivo se comprueba igualmente en
    // storage.js/sendTrainerRequest.
    const maxAlumnos = AppState.currentUserData?.maxAlumnos;
    const alumnosActuales = (AppState.currentUserData?.alumnosAceptados || []).length;
    if (maxAlumnos != null && alumnosActuales >= maxAlumnos) {
      Utils.showToast(`Ya tienes el máximo de ${maxAlumnos} atletas permitido`, 'error');
      this.cargarPanelAlumnos(true);
      return;
    }
    // 🔧 FIX v3.61 (diagnóstico, ver storage.js v3.34): sendTrainerRequest
    // ahora puede lanzar el error real de Firestore en vez de devolver
    // `false` a secas -- se muestra error.code en el toast para saber la
    // causa exacta (p.ej. "permission-denied") en vez de un mensaje ciego.
    try {
      const ok = await Storage.sendTrainerRequest(AppState.currentUserId, uid);
      if (!ok) { Utils.showToast('No se pudo enviar la invitación', 'error'); return; }
    } catch (error) {
      console.error('Error enviando invitación de entrenador:', error);
      Utils.showToast(`No se pudo enviar la invitación (${error.code || error.message || 'error desconocido'})`, 'error');
      return;
    }
    Utils.showToast(`✅ Invitación enviada a ${username}`, 'success');
    this.cargarPanelAlumnos(true); // cambio real: forzar refresco de la caché
  },

  async cancelarInvitacionAlumno(requestId) {
    try {
      await firebaseServices.db.collection('trainerRequests').doc(requestId).update({ status: 'cancelled' });
    } catch (error) {
      console.error('Error cancelando invitación de alumno:', error);
      Utils.showToast('Error al cancelar la invitación', 'error');
      return;
    }
    Utils.showToast('Invitación cancelada', 'info');
    this.cargarPanelAlumnos(true); // cambio real: forzar refresco de la caché
  },

  async quitarAlumno(uid) {
    // 🔥 v10.x: el nombre interno de la función/campos (alumno,
    // alumnosAceptados...) se mantiene por dentro para no tocar datos ya
    // guardados en Firestore -- a petición del usuario, solo cambia el
    // texto que ve la gente: "atleta(s)" en vez de "alumno(s)".
    const confirmado = await Utils.confirm('Quitar atleta', '¿Dejar de poder enviarle sesiones a este amigo? Tendría que aceptarte de nuevo si quieres volver a entrenarlo.');
    if (!confirmado) return;
    const ok = await Storage.removeAlumno(AppState.currentUserId, uid);
    if (!ok) { Utils.showToast('Error al quitar atleta', 'error'); return; }
    Utils.showToast('Atleta eliminado', 'info');
    this.cargarPanelAlumnos(true); // cambio real: forzar refresco de la caché
  },

  // ==================== "ENTRENADO POR" (lado del alumno) ====================
  // 🔥 En el propio perfil (subpestaña principal), si algún entrenador te
  // tiene aceptado, aquí aparece con quién y un botón para dejar de serlo
  // sin dejar de ser su amigo. Se apoya en AppState.currentUserData, que
  // el listener de "mis datos" de app.js ya mantiene al día en tiempo
  // real -- no hace falta ninguna lectura propia a Firestore salvo para
  // los nombres de los entrenadores.
  async cargarEntrenadoPor() {
    const container = document.getElementById('entrenadoPorContainer');
    if (!container || !AppState.currentUserId) return;
    const entrenadores = AppState.currentUserData?.entrenadoresAceptados || [];
    if (!entrenadores.length) { container.innerHTML = ''; return; }
    try {
      const datos = await Promise.all(entrenadores.map(uid => Storage.getUser(uid)));
      // 🔥 A petición del usuario: la tarjeta se colorea del nivel del
      // entrenador (gris nivel 2, morado/lila nivel 10...), igual que ya
      // se colorea el nombre en las invitaciones recibidas -- aquí además
      // se tiñe el borde y el fondo de la tarjeta entera, no solo el texto.
      const niveles = await this.getNivelesDirectos(entrenadores);
      let html = '<div style="color:var(--text-secondary); font-size:12px; margin-bottom:8px;">🎯 ENTRENADO POR</div>';
      entrenadores.forEach((uid, i) => {
        const u = datos[i];
        const nombre = Utils.capitalizeUsername(u?.username || 'Usuario');
        const nivel = niveles[uid] || 1;
        const color = Gamification.getColorByLevel(nivel);
        const photoURL = u?.profile?.photoURL;
        const avatarHTML = photoURL
          ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover; border:2px solid ${color};" onerror="Utils.avatarFallback(this)"><div class="nivel-badge" style="background: ${color}; color: white; text-shadow: 0 0 1px black;">${nivel}</div></div>`
          : `<div class="resultado-avatar-wrapper"><div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center; border:2px solid ${color};">👤</div><div class="nivel-badge" style="background: ${color}; color: white; text-shadow: 0 0 1px black;">${nivel}</div></div>`;
        html += `
          <div class="resultado-busqueda" style="justify-content:space-between; margin-bottom:8px; border:1px solid ${color}; background:${color}1a; border-radius:10px;">
            <div class="resultado-info">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre" style="color:${color};">${Utils.escapeHTML(nombre)}</div>
                <div class="resultado-username">Nivel ${nivel}</div>
              </div>
            </div>
            <button class="btn-amigo eliminar" data-trainer-action="dejar-de-ser-alumno" data-uid="${uid}" data-username="${Utils.escapeHTML(nombre)}">DEJAR DE SER SU ATLETA</button>
          </div>
        `;
      });
      container.innerHTML = html;
    } catch (error) {
      console.error('Error cargando entrenado por:', error);
    }
  },

  async dejarDeSerAlumno(trainerUid, nombre) {
    const confirmado = await Utils.confirm('Dejar de ser su atleta', `¿Seguro que ${nombre} deje de poder enviarte sesiones? Seguiríais siendo amigos igualmente.`);
    if (!confirmado) return;
    const ok = await Storage.dejarDeSerAlumno(trainerUid, AppState.currentUserId);
    if (!ok) { Utils.showToast('Error al dejar de ser su atleta', 'error'); return; }
    Utils.showToast(`Ya no eres atleta de ${nombre}`, 'info');
    await this._refreshCurrentUserData();
    this.cargarEntrenadoPor();
  },
  ...
  async cargarListaAmigos(forceRefresh = false) {
    ...
    try {
      ...
      // 🔥 FIX lecturas duplicadas: _limpiarAmigosHuérfanos ya trajo el
      // documento completo de cada amigo -- se reutiliza en vez de volver
      // a pedirlo. Solo si por lo que sea no vino aquí (p.ej. falló esa
      // comprobación, ver el catch de arriba) se pide suelto como red de
      // seguridad, igual que se hacía siempre.
      const amigosData = [];
      const faltantes = [];
      for (const fid of friendIds) {
        if (docsPorId[fid]) amigosData.push(docsPorId[fid]);
      ...
      for (let i = 0; i < faltantes.length; i += 10) {
        const batch = faltantes.slice(i, i + 10);
  ...
  _renderListaAmigos({ amigos, niveles }) {
    ...
    for (const amigo of amigos) {
      ...
      const avatarHTML = photoURL
        ...
        : `<div class="resultado-avatar-wrapper"><div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center; border:2px solid ${color};">👤</div><div class="nivel-badge" style="${badgeStyle}">${nivel}</div></div>`;
      const usernameFormatted = Utils.capitalizeUsername(amigo.username);
      const pillEntrenador = amigo.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : '';
      html += `
        <div class="resultado-busqueda" style="justify-content:space-between;" data-uid="${amigo.uid}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${amigo.uid}')">
          <div class="resultado-info">
            ${avatarHTML}
            <div>
              <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)}${pillEntrenador}</div>
              <div class="resultado-username">@${Utils.escapeHTML(amigo.username)}</div>
            </div>
          </div>
  ...
  async eliminarAmigo(friendUid) {
    ...
    try {
      ...
      this._eliminandoAmigoEnCurso.delete(friendUid);
      return;
    }
    ...
    try {
      ...
      console.error('Error refrescando listas tras eliminar amigo:', error);
  ...
  async abrirModalAmigo(uid) {
        ...
        <div style="background:var(--bg-card); border-radius:16px; border:2px solid ${levelColor}; overflow:hidden;">
          <div style="padding:20px;">
            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
              ${avatarHTML}
              <div>
                <div style="font-size:18px; font-weight:500; color:${levelColor};">${Utils.escapeHTML(userName)}${userData.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : ''}</div>
    ...
    try {
      ...
      const modalTitle = document.querySelector('#modalAmigo h3');
      if (modalTitle) {
        modalTitle.innerHTML = Utils.escapeHTML(Utils.capitalizeUsername(userData.username)) + (userData.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : '');
        modalTitle.style.display = 'block';
        modalTitle.style.margin = '0 0 16px 0';
        modalTitle.style.color = 'var(--accent-yellow)';
      }
      ...
      if (esAmigo) {
        botonAccion = `
          <div style="display:flex; align-items:center; gap:10px; justify-content:center;">
            <span style="font-size:14px; color:var(--zone-2);">✓ Ya son amigos</span>
            <button class="btn-amigo chat-btn" data-uid="${uid}" data-username="${Utils.escapeHTML(userName)}">💬 CONTACTAR</button>
          </div>
        `;
      } else {
  ...
  async _cargarPaginaUsuarios(esPrimera) {
    ...
    try {
      ...
      } else if (st.modo === 'entrenadores') {
        // 🔧 NUEVO (petición del usuario): modo "🎯 Ver solo entrenadores"
        // -- misma paginación que el resto de modos, pero consultando
        // Storage.getAllTrainers en vez de todos los usuarios.
        const result = await Storage.getAllTrainers(20, esPrimera ? null : st.lastDoc);
      ...
      // 🔧 NUEVO: la primera vez que se use el filtro "solo entrenadores"
      // en un proyecto de Firebase, esta consulta (isTrainer + orderBy
      // username) puede pedir crear un índice compuesto -- Firestore da
      // el enlace exacto en la consola/consola del navegador para
      // crearlo con un clic; aquí solo se avisa con un mensaje claro en
      // vez del genérico "Error al cargar".
      if (error.code === 'failed-precondition' && st.modo === 'entrenadores') {
        if (esPrimera) {
          await esperarFinAnimacion();
          container.innerHTML = '<p style="text-align:center; color:var(--zone-5); padding:20px; line-height:1.5;">Falta crear un índice en Firestore para esta búsqueda. Revisa la consola del navegador: Firebase deja un enlace para crearlo con un clic.</p>';
        } else {
          Utils.showToast('Falta un índice en Firestore para esto (ver consola)', 'error');
        }
      } else if (esPrimera) {
        await esperarFinAnimacion();
  ...
  async _aplicarModoBusqueda() {
    const st = this._usuariosState;
    if (!st) return;
    if (st.modo === 'entrenadores') return; // el filtro de entrenadores manda mientras esté activo
  ...
  // 🔧 NUEVO (petición del usuario): toggle "Ver solo entrenadores" en la
  // pantalla Amigos → Buscar. Mientras está activo, el cuadro de texto se
  // deshabilita (esta pantalla no combina búsqueda por nombre + filtro de
  // entrenador en la misma consulta) y se pagina con Storage.getAllTrainers.
  // pantalla Amigos → Buscar. Ya no es un checkbox: es el propio botón el
  // que se pulsa entero y queda "encendido" (clase .active, fondo
  // dorado). Mientras está activo, el cuadro de texto se deshabilita
  // (esta pantalla no combina búsqueda por nombre + filtro de entrenador
  // en la misma consulta) y se pagina con Storage.getAllTrainers.
  async toggleFiltroEntrenadores() {
    const btn = document.getElementById('filtroSoloEntrenadores');
    const input = document.getElementById('buscarAmigosInput');
    if (!btn) return;
    if (!this._usuariosState) await this.cargarTodosUsuarios(true);
    const st = this._usuariosState;
    if (!st) return;
    ...
    if (activar) {
      ...
      st.modo = 'entrenadores'; st.term = '';
    } else {
      if (input) input.disabled = false;
      st.modo = 'explorar'; st.term = '';
    }
    st.users = []; st.lastDoc = null; st.agotado = false;
    await this._cargarPaginaUsuarios(true);
  },

  _renderExplorarUsuarios(st) {
    const { users, niveles, friendIds, enviadas, recibidas, modo } = st;
    const container = document.getElementById('todosUsuariosList');
    if (!container) return;
    let html = '';
    for (const user of users) {
      if (user.uid === AppState.currentUserId) continue;
      // 🔧 FIX (petición del usuario): en modo "entrenadores" el objetivo
      // es un directorio para ENCONTRARLOS, así que ya no se ocultan los
      // que ya son amigos (antes desaparecían de la lista igual que en
      // el modo "explorar" normal, que sí los oculta a propósito porque
      // ahí el objetivo es encontrar gente nueva a la que añadir).
      const esAmigo = friendIds.includes(user.uid);
      if (modo !== 'entrenadores' && esAmigo) continue;
      ...
      const pillEntrenador = user.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : '';
      let boton = '';
      if (esAmigo) {
        boton = `<button class="btn-amigo chat-btn" data-uid="${user.uid}" data-username="${usernameFormatted}">💬 CONTACTAR</button>`;
  ...
  async actualizarBadgeSolicitudes() {
    if (!AppState.currentUserId) return;
    try {
      // 🔥 Refresco inmediato/optimista justo después de una acción local
      // (aceptar/rechazar/cancelar). Escribe en los MISMOS dos contadores
      // que alimentan los listeners onSnapshot en tiempo real
      // (AppState.solicitudesAmistadPendientesCount /
      // ...EntrenadorPendientesCount) en vez de tocar el total
      // directamente -- así, en cuanto esos listeners reciban su propia
      // confirmación del servidor, no pisan este valor con uno
      // desincronizado (ver AppState._recalcularSolicitudesPendientes).
      const [{ recibidas }, entrenadorRecibidas] = await Promise.all([
        this.obtenerSolicitudesPendientes(),
        Storage.getTrainerRequestsRecibidas(AppState.currentUserId)
      ]);
      AppState.solicitudesAmistadPendientesCount = recibidas.length;
      AppState.solicitudesEntrenadorPendientesCount = entrenadorRecibidas.length;
      AppState._recalcularSolicitudesPendientes();
...
console.log('✅ friends.js v3.60 - _limpiarAmigosHuérfanos() deduplica la promesa en vuelo por uid');
