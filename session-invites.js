...
const SessionInvites = {
    ...
    for (let dia = 1; dia <= diasEnMes; dia++) {
      ...
      html += `
        <div onclick="SessionInvites._elegirDia(${year},${month},${dia})" style="
          ...
          background:${esSeleccionado ? 'var(--level-color)' : 'var(--stat-bg)'};
          color:${esSeleccionado ? '#000' : 'var(--text-primary)'};
          border:1px solid ${esHoy && !esSeleccionado ? 'var(--level-color)' : 'var(--border-color)'};
  ...
  _actualizarTextoFechas() {
    ...
    txt.style.color = 'var(--level-color)';
    ...
    container.innerHTML = usuarios.map(u => {
      ...
      return `
        <div onclick="SessionInvites._toggleUsuario('${u.uid}')" style="
          ...
          background:${marcado ? 'color-mix(in srgb, var(--level-color) 12%, transparent)' : 'var(--stat-bg)'};
          border:1px solid ${marcado ? 'var(--level-color)' : 'var(--border-color)'};
  ...
  // ==================================================================
  //  REMITENTE (ADMIN): ENTERARSE EN TIEMPO REAL DE ACEPTACIONES/RECHAZOS
  // ==================================================================
  //
  // El listener de arriba (iniciarListener/unsubscribe) solo vigila las
  // invitaciones que ME LLEGAN (toUid == yo), para mostrar el modal de
  // aceptar/rechazar. Sin este segundo listener, quien ENVÍA la sesión
  // (el admin) nunca se enteraba en tiempo real de que fue aceptada o
  // rechazada: mostrarHistorial() solo lee Firestore una vez y se queda
  // en caché (_historialCache) hasta que se envía otra sesión, se borra
  // algo del historial, o se recarga la app. Con esto, en cuanto Firestore
  // confirma el cambio de estado, se invalida esa caché y se repinta el
  // panel si está abierto -- sin que el admin tenga que hacer nada.
  iniciarListenerEnviadas() {
    if (!AppState.currentUserId || !window.firebaseServices) return;
    this.detenerListenerEnviadas();
    this.unsubscribeEnviadas = firebaseServices.db.collection('sessionInvites')
      .where('fromUid', '==', AppState.currentUserId)
      .onSnapshot((snapshot) => {
        let huboCambioDeEstado = false;
        snapshot.docChanges().forEach(change => {
          // El snapshot inicial (al conectar el listener) reporta todos
          // los documentos existentes como 'added', nunca como
          // 'modified' -- así que este filtro ya evita, por sí solo,
          // mostrar toasts de cosas que cambiaron antes de abrir la app.
          if (change.type !== 'modified') return;
          const d = change.doc.data();
          if (d.status !== 'accepted' && d.status !== 'rejected') return;
          huboCambioDeEstado = true;
          const nombreSesion = d.sesion?.detalle?.nombre || d.sesion?.tipo || 'la sesión';
          const nombreUsuario = d.toUsername || 'Un usuario';
          if (d.status === 'accepted') {
            Utils.showToast(`✅ ${nombreUsuario} aceptó "${nombreSesion}"`, 'success');
          } else {
            Utils.showToast(`❌ ${nombreUsuario} rechazó "${nombreSesion}"`, 'info');
          }
        });

        if (huboCambioDeEstado) {
          this._historialCache = null; // fuerza a releer el historial la próxima vez que se pinte
          const panelAbierto = document.getElementById('tab-admin')?.classList.contains('active');
          if (panelAbierto) this.mostrarHistorial();
        }
      }, (error) => {
        console.error('Error en listener de sesiones enviadas (admin):', error);
      });
  },

  detenerListenerEnviadas() {
    if (this.unsubscribeEnviadas) {
      this.unsubscribeEnviadas();
      this.unsubscribeEnviadas = null;
    }
  },
      ...
      modal.innerHTML = `
        <div style="padding:20px; text-align:center;">
          ...
          <div style="background:var(--stat-bg); border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center; margin-bottom:20px;">
            <div style="font-weight:bold; color:var(--level-color); margin-bottom:8px;">Día de descanso</div>
    ...
    modal.innerHTML = `
      <div style="padding:20px; text-align:center;">
        ...
        <div style="background:var(--stat-bg); border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center; margin-bottom:12px;">
          <div style="font-weight:bold; color:var(--level-color); margin-bottom:6px;">${Utils.escapeHTML(detalle.nombre || sesion.tipo || 'Sesión')}</div>
