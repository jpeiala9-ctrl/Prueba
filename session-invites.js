...
const SessionInvites = {
  ...
  async _obtenerCalculoDestinatario(uid) {
    try {
      if (window.Storage && typeof Storage.getUltimoCalculo === 'function') {
        return await Storage.getUltimoCalculo(uid);
      }
    } catch (e) {
  ...
  // el admin (tipo + zona + distancia): ritmo objetivo (según su ritmo
  // base real), tiempo que le va a llevar esa distancia a SU ritmo, TSS
  // (duración personalizada × intensidad de la zona, misma fórmula que
  // usa el resto de la app) y calorías (peso × distancia, ~1kcal/kg/km,
  // estimación estándar de carrera). Si al usuario le faltan datos
  // (zonas sin calcular / peso sin rellenar en el perfil), esos campos
  // concretos quedan a null y se avisa en vez de inventar un número.
  _calcularPersonalizacion(zonaCodigo, distanciaKm, calculo, peso) {
    const resultado = { ritmoStr: null, duracionMin: null, tss: null, calorias: null };
    if (calculo && Array.isArray(calculo.zones) && calculo.zones.length && calculo.ritmoBase) {
      ...
      if (zona) {
        ...
        resultado.duracionMin = Math.max(1, Math.round(distanciaKm * paceDecimal));
  ...
  _renderPaso1() {
    ...
    modal.innerHTML = `
      ...
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        ...
        <input id="sgDistancia" type="number" step="0.1" min="0" style="width:100%; margin-bottom:10px;">

        <label style="font-size:11px; color:var(--text-secondary);">ZONA DE ENTRENAMIENTO</label>
        <select id="sgZona" style="width:100%; margin-bottom:6px;">
          ${this._ZONAS.map(z => `<option value="${z.codigo}">${z.codigo} · ${z.etiqueta}</option>`).join('')}
        </select>
        <p style="font-size:11px; color:var(--text-secondary); margin:0 0 10px; line-height:1.4;">
          ℹ️ Con la distancia y la zona es suficiente: el tiempo que le llevará, el ritmo, el TSS y las
  ...
  async _mostrarModalInvite(id, data) {
    ...
    const personalizado = this._calcularPersonalizacion(detalle.zona, distanciaKm, calculo, peso);
    ...
    modal.innerHTML = `
      <div style="padding:20px; text-align:center;">
        ...
        <div style="background:var(--stat-bg); border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:left; margin-bottom:12px;">
          <div style="font-weight:bold; color:var(--gold); margin-bottom:6px;">${Utils.escapeHTML(detalle.nombre || sesion.tipo || 'Sesión')}</div>
          <div style="font-size:12px; color:var(--text-secondary); display:flex; gap:14px; flex-wrap:wrap; margin-bottom:6px;">
            <span>🕒 ${tiempoTxt}</span>
        ...
        ${sinZonasCalculadas ? `<div style="font-size:11px; color:var(--gold); margin-bottom:12px; line-height:1.4;">⚠️ Todavía no has calculado tus zonas de entrenamiento, así que no podemos calcular tu ritmo/tiempo/TSS para esta sesión. Ve a Perfil → Entreno, calcúlalas y vuelve a abrir esta sesión.</div>` : ''}
        ...
        <div style="display:flex; gap:12px;">
          ...
          <button id="sessInviteAceptarBtn" class="action-button" style="flex:1; margin:0; background:var(--gold); color:#000;" ${sinZonasCalculadas ? 'disabled title="Calcula tus zonas primero"' : ''}>ACEPTAR</button>
        </div>
      </div>
    `;

    document.getElementById('sessInviteRechazarBtn').addEventListener('click', () => this._rechazar(id));
    const btnAceptar = document.getElementById('sessInviteAceptarBtn');
    if (!sinZonasCalculadas) {
      btnAceptar.addEventListener('click', () => this._aceptar(id, data, personalizado));
    }
  },
  ...
  async _aceptar(id, data, personalizado) {
    ...
    try {
      ...
      if (!personalizado) {
        ...
        personalizado = this._calcularPersonalizacion(data.sesion.detalle?.zona, data.sesion.detalle?.distanciaEstimada || 0, calculo, peso);
      }
      ...
      // ritmo/TSS/calorías (calculados con sus zonas y su peso), no lo
      // que puso el admin al crear la sesión.
