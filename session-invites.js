...
const SessionInvites = {
  ...
  // el admin (tipo + zona + distancia de la PARTE PRINCIPAL + minutos de
  // calentamiento/enfriamiento): ritmo objetivo (según su ritmo base
  // real), tiempo que le va a llevar esa distancia a SU ritmo (parte
  // principal), tiempo total de la sesión (calentamiento + parte
  ...
  //
  // BUG CORREGIDO: la distancia que pone el admin (p.ej. 8 km) es SOLO la
  // parte principal -- el calentamiento y el enfriamiento son aparte y se
  // corren en Z1 (recuperación), no en la zona de la parte principal.
  // Antes esos minutos de calentamiento/enfriamiento se sumaban al TIEMPO
  // total de la sesión pero NUNCA se convertían en km ni se sumaban a la
  // distancia: la app mostraba "8 km" aunque la sesión completa (con
  // calentamiento y enfriamiento incluidos) fuera más larga en la
  // práctica. Ahora se calcula cuántos km cubre el usuario en esos
  // minutos corriendo a SU ritmo de Z1, y esos km se suman a la parte
  // principal para dar la distancia TOTAL real de la sesión (p.ej. 8 km
  // de parte principal + 2 km de calentamiento a 5:00/km en 10' + 1 km de
  // enfriamiento a 5:00/km en 5' = 11 km en total).
  _calcularPersonalizacion(zonaCodigo, distanciaKm, calculo, peso, calentamientoMin = 0, enfriamientoMin = 0) {
    const resultado = {
      ...
      distanciaPartePrincipal: distanciaKm, distanciaCalentamientoKm: null, distanciaEnfriamientoKm: null,
      distanciaTotal: null
    };
    if (calculo && Array.isArray(calculo.zones) && calculo.zones.length && calculo.ritmoBase) {
      ...
      if (zona) {
        ...
        resultado.distanciaTotal = distanciaKm + resultado.distanciaCalentamientoKm + resultado.distanciaEnfriamientoKm;
    ...
    // si no hay zonas disponibles, como respaldo se usa solo la distancia
    // de la parte principal para no dejar las calorías a 0.
    if (peso) {
      const distanciaParaCalorias = resultado.distanciaTotal !== null ? resultado.distanciaTotal : distanciaKm;
    ...
    modal.innerHTML = `
      ...
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        ...
        <p style="font-size:11px; color:var(--text-secondary); text-align:center; margin:0 0 10px; line-height:1.4;">
          ℹ️ Esta distancia es SOLO la parte principal. El calentamiento y el enfriamiento
          (abajo, en minutos) se corren en Z1 y se convierten a km con el ritmo de Z1 de
          cada usuario, sumándose a la distancia y al tiempo total de la sesión.
        </p>
  ...
  async _mostrarModalInvite(id, data) {
    ...
    // 'distanciaEstimada' en la sesión tal y como la creó el admin es SOLO
    // la parte principal (ver _calcularPersonalizacion): el total real,
    // sumando lo que cubre el calentamiento/enfriamiento a ritmo de Z1,
    // se calcula aquí para CADA usuario.
    const distanciaParteePrincipal = detalle.distanciaEstimada || 0;
    const personalizado = this._calcularPersonalizacion(
      detalle.zona, distanciaParteePrincipal, calculo, peso,
    ...
    // se cae de vuelta a la distancia de la parte principal que puso el
    // admin, para no dejar el dato en blanco.
    const distanciaMostrar = personalizado.distanciaTotal !== null ? personalizado.distanciaTotal : distanciaParteePrincipal;
    const distanciaTxt = parseFloat(distanciaMostrar.toFixed(2));
    ...
    modal.innerHTML = `
      <div style="padding:20px; text-align:center;">
        ...
        ${(!peso && distanciaParteePrincipal) ? `<div style="font-size:11px; color:var(--text-secondary); margin-bottom:12px; line-height:1.4;">ℹ️ Añade tu peso en tu perfil para ver las calorías estimadas de esta sesión.</div>` : ''}
  ...
  async _aceptar(id, data, personalizado) {
    ...
    try {
      ...
      //
      // 'distanciaEstimada' se sobreescribe con la distancia TOTAL
      // (parte principal + calentamiento + enfriamiento convertidos a km a
      // ritmo de Z1) en vez de dejar el valor original del admin, que solo
      // era la parte principal -- así queda igual que el campo
      // 'distanciaEstimada' que genera el resto de la app (PlanGenerator:
      // ver calcularMetricasSesion en calendar.js), que siempre representa
      // el total de la sesión, no solo un tramo.
