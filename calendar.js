...
const PlanGenerator = {
  ...
  async insertarSesionManualEnPlan(diaGlobal, tipo, duracion, detalle) {
    ...
    try {
      ...
      if (sesiones.some(s => s.diaGlobal === diaGlobal)) {
  ...
  renderizarMes() {
    ...
    // 🔥 El "+" se comporta distinto según el tipo de plan:
    // - Plan GENERADO (cerrado, con fechas fijas): nunca tiene huecos de
    //   verdad -- todos los días ya tienen sesión, aunque sea "descanso".
    //   Ahí el "+" solo tiene sentido en el caso raro de un día
    //   realmente sin sesión, y solo hasta hoy (no tiene sentido
    //   registrar un entreno de un día futuro que aún no ha llegado).
    // - Plan PERSONALIZADO (el que se crea vacío para todo el año en
    //   curso e ir recibiendo sesiones sueltas del entrenador): TODO día
    //   desde HOY hasta el 31 de diciembre de ese año que no tenga una
    //   sesión de verdad asignada por el entrenador ha de tener el "+" --
    //   esto incluye tanto los días totalmente vacíos como los que de
    //   momento están en "descanso" por defecto. En cuanto el entrenador
    //   asigna una sesión a ese día, esa sesión sustituye al "+".
    const esPersonalizado = AppState.planActualTipo === 'personalizado';
    let finAñoPersonalizado = null;
    if (esPersonalizado && this._fechaInicioPlan) {
      // 🔥 Si el entrenador manda una sesión para más adelante del 31 de
      // diciembre del año en que se creó el plan (ya en el año
      // siguiente), el rango del "+" se amplía automáticamente hasta el
      // 31 de diciembre de ESE año más lejano -- así el "+" sigue
      // cubriendo todos los huecos hasta donde de verdad llega el plan,
      // no se queda cortado en el 31/12 original.
      ...
      finAñoPersonalizado = new Date(añoLimite, 11, 31);
    }
    ...
    for (let dia = 1; dia <= totalDiasMes; dia++) {
      ...
      // Helper: ¿toca mostrar el "+" en este día? (solo cuando ese día no
      // tiene una sesión de verdad asignada -- ni real ni "descanso").
      const diaElegibleParaMas = esPersonalizado
        ? (fecha >= hoy && finAñoPersonalizado && fecha <= finAñoPersonalizado)
        : (this._fechaInicioPlan && fecha >= this._fechaInicioPlan && fecha <= hoy);
