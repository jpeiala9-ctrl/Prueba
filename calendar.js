  ...
  renderizarMes() {
    ...
    for (let dia = 1; dia <= totalDiasMes; dia++) {
      ...
      if (!sesion) {
        ...
        // 🔥 FIX bug reportado: el "+" no salía en el plan personalizado.
        // Causa real: _fechaFinPlan no es "hasta cuándo dura el plan",
        // sino "hasta la fecha de la ÚLTIMA sesión que ya existe" en el
        // array (se calcula como fechaInicio + el mayor diaGlobal
        // encontrado). En el plan generado esto no se nota porque TODOS
        // los días tienen sesión (aunque sea "descanso"), así que esa
        // fecha siempre llega hasta el final de verdad -- pero en el
        // plan personalizado, que solo tiene las pocas sesiones sueltas
        // que manda el admin, esa fecha se queda corta en la última de
        // esas sesiones, y cualquier día de hoy en adelante quedaba
        // "fuera de rango" sin poder registrar nada. El "+" ya no exige
        // estar dentro de ese rango tan corto -- solo que el día sea
        // desde que empezó el plan hasta hoy (da igual hasta dónde
        // lleguen las sesiones ya existentes).
        const sePuedeRegistrar = this._fechaInicioPlan && fecha >= this._fechaInicioPlan && fecha <= hoy;
        const diaGlobalVacio = sePuedeRegistrar ? Math.floor((fecha - this._fechaInicioPlan) / 86400000) + 1 : null;
        const botonMas = sePuedeRegistrar
          ? `<button class="btn-dia-no-programado" data-dia-vacio="${diaGlobalVacio}" title="Registrar entreno no programado" style="position:absolute; bottom:2px; right:2px; width:18px; height:18px; line-height:16px; padding:0; border-radius:50%; border:1px solid var(--border-color-light); background:var(--stat-bg); color:var(--text-secondary); font-size:13px; cursor:pointer;">+</button>`
          : '';
        html += `<div class="calendario-dia calendario-dia-vacio${dentroDelPlan ? ' sesion-descanso' : ''}${esHoy}" style="position:relative;"><div class="dia-numero">${dia}</div>${botonMas}</div>`;
        continue;
      }
