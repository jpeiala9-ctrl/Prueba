...
const PlanGenerator = {
  ...
  ONDULACION_PATRONES: [
    { intensidad: 0.85, volumen: 1.15 },
    { intensidad: 1.0, volumen: 1.0 },
    { intensidad: 1.15, volumen: 0.85 },
    { intensidad: 0.9, volumen: 0.92 }
  ],
  ...
  // También se ha estrechado ligeramente la amplitud de la onda de
  // volumen (antes 0.8–1.2, ahora 0.85–1.15): una oscilación de ±20%
  // semana a semana es más agresiva de lo habitual en periodización real;
  // ±15% ya da la alternancia carga/recuperación sin exagerar el salto.
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        volumenSemana = Math.min(TECHO_DURO, Math.max(180, volumenSemana));
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (realDistance !== null && realDurationMs !== null) {
            ...
            metricas = this.calcularMetricasSesion(sesion);
        ...
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            ...
            if (distanciaReal !== null) {
              ...
              const metricas = this.calcularMetricasSesion(sesion);
              distanciaRemovida = metricas.distanciaTotal;
