// ==================== gamification.js ====================
// Versión: 5.21 - RONDA DE OPTIMIZACIÓN FIRESTORE: getData(uid) ahora tiene
//                caché en memoria con TTL de 30s (_cacheMemoria) -- evita
//                lecturas repetidas de gamification/{uid} cuando varias
//                pantallas piden los mismos datos en pocos segundos
//                (Dashboard, Perfil, Amigos, Muro, Sponsors...). Todas las
...
//                repairMyProfile) invalidan la entrada de caché justo
//                después de escribir, para que la siguiente lectura nunca
//                devuelva datos obsoletos. clearCache(uid) (llamada desde
//                fuera, p.ej. al cerrar sesión) también la limpia.
...
const Gamification = {
  // 🔥 FIX (ronda de optimización Firestore): caché en memoria de getData(uid)
  // con TTL de 30s. getData(uid) se llama desde Dashboard, Perfil, Amigos,
  // Muro, abrirModalAmigo, panel de alumnos, invitaciones y
  // Sponsors.comprobarUmbralZapatilla -- antes cada llamada suponía una
  // lectura nueva de gamification/{uid}, aunque varias de esas llamadas
  // ocurren a segundos de diferencia dentro de la misma sesión de uso.
  // Con el TTL corto seguimos viendo cambios reales (subida de nivel,
  // nueva sesión) casi al instante, pero evitamos ráfagas de lecturas
  // idénticas. clearCache(uid) invalida también esta entrada.
  _cacheMemoria: {},
  _CACHE_TTL_MS: 30000,
  ...
  async clearCache(uid) {
    ...
    try {
      ...
      delete this._cacheMemoria[uid]; // 🔥 FIX: invalidar también la caché en memoria de getData
  ...
  async getData(uid) {
    ...
    // 🔥 FIX: hit de caché en memoria (TTL 30s) -- evita leer Firestore otra
    // vez si getData(uid) ya se pidió hace poco para el mismo uid.
    const cached = this._cacheMemoria[uid];
    ...
    try {
      ...
      this._cacheMemoria[uid] = { data: resultado, timestamp: Date.now() };
      return resultado;
    } catch (error) {
...
console.log('✅ gamification.js v5.20 - reparación de zapatilla/derivados automática y silenciosa la primera vez que se entra a Perfil (sin botón manual)');