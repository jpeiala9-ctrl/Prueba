// ==================== gamification.js ====================
// Versión: 5.22 - CACHÉ EN VIVO (sustituye al TTL de 30s de la v5.21):
//                getData(uid) ya no expira nunca por tiempo -- ahora la
//                alimenta un listener onSnapshot sobre gamification/{uid}
//                (iniciarListener(uid), arrancado desde
//                AppState.setCurrentUser y parado desde
//                AppState.detenerListeners, ver app.js v4.66). Motivo: con
//                TTL, cambiar el nivel/insignias de alguien (a mano desde
//                consola, o desde otro dispositivo del propio usuario)
//                podía tardar hasta 30s en reflejarse en pantalla. Con el
//                listener es instantáneo Y cuesta menos lecturas (1 lectura
//                inicial + solo cambios reales, en vez de 1 lectura cada
//                vez que el TTL expiraba con la pantalla abierta). Mientras
//                el listener aún no ha recibido su primer snapshot,
//                getData(uid) cae a una lectura puntual de fallback (con la
//                misma autolimpieza de récords sin GPS y creación de
//                documento por defecto que ya hacía antes). Todas las
//                funciones de este archivo que escriben en el documento
//                (setCurrentShoe, addKilometersToShoe,
//                removeKilometersFromShoe, _fijarKmZapatilla,
//                repararTodoDesdeHistorialSiHaceFalta, updateAfterSession,
//                removeSession, repararBadgeGPS, actualizarRecordsPorTramos,
//                repairMyProfile) siguen invalidando la entrada de caché
//                justo después de escribir (ya no es estrictamente
//                necesario -- el propio listener la reemplazará con el
//                siguiente snapshot -- pero no hace daño dejarlo como red
//                de seguridad). clearCache(uid) (llamada desde fuera, p.ej.
//                al cerrar sesión) limpia la caché Y para el listener.
...
const Gamification = {
  // 🔥 CAMBIO (v5.22, caché en vivo): _cacheMemoria ya no guarda
  // { data, timestamp } con TTL -- ahora es un objeto simple { uid: data }
  // que mantiene actualizado iniciarListener(uid) vía onSnapshot.
  // getData(uid) se llama desde Dashboard, Perfil, Amigos, Muro,
  // abrirModalAmigo, panel de alumnos, invitaciones y
  // Sponsors.comprobarUmbralZapatilla: con el listener en marcha, todas
  // esas llamadas son instantáneas y no cuestan ninguna lectura extra, y
  // los cambios reales (subida de nivel, nueva sesión, edición manual
  // desde consola) se ven al momento en vez de esperar a que expire un TTL.
  _cacheMemoria: {},
  _unsubscribeGamification: null,

  // Arranca (de forma idempotente) el listener en vivo de
  // gamification/{uid}. Se llama desde AppState.setCurrentUser justo
  // después de iniciarListeners(). Si ya había un listener activo (p.ej.
  // llamada repetida), no hace nada.
  iniciarListener(uid) {
    if (!uid || this._unsubscribeGamification) return;
    this._unsubscribeGamification = firebaseServices.db.collection('gamification').doc(uid)
      .onSnapshot(snap => {
        if (snap.exists) this._cacheMemoria[uid] = snap.data();
      }, error => {
        console.warn('Error en el listener en vivo de gamificación:', error);
      });
  },

  // Para el listener en vivo y limpia toda la caché en memoria. Se llama
  // desde AppState.detenerListeners (logout) y desde clearCache(uid).
  detenerListener() {
    if (this._unsubscribeGamification) {
      this._unsubscribeGamification();
      this._unsubscribeGamification = null;
    }
    this._cacheMemoria = {};
  },
  ...
  async clearCache(uid) {
    ...
    try {
      ...
      delete this._cacheMemoria[uid];
      this.detenerListener(); // 🔥 CAMBIO v5.22: al limpiar la caché (p.ej. logout) se cierra también el listener en vivo
  ...
  async getData(uid) {
    ...
    // 🔥 CAMBIO v5.22: hit de caché en vivo -- si iniciarListener(uid) ya
    // recibió su primer snapshot, esto es instantáneo y no cuesta ninguna
    // lectura de Firestore.
    if (this._cacheMemoria[uid]) return this._cacheMemoria[uid];
    // Fallback: el listener todavía no ha disparado su primer snapshot (o
    // no se llegó a arrancar para este uid). Lectura puntual, con la misma
    // autolimpieza de récords sin GPS y creación de documento por defecto
    // de siempre. En cuanto el listener reciba su primer snapshot,
    // sustituirá esta entrada.
    try {
      const doc = await firebaseServices.db.collection('gamification').doc(uid).get();
      let resultado;
      if (doc.exists) {
        const data = doc.data();
        resultado = await this._limpiarRecordsNoGPS(uid, data);
      } else {
        ...
        resultado = defaultData;
      }
      this._cacheMemoria[uid] = resultado;
      return resultado;
    } catch (error) {
  ...
  async actualizarRecordsPorTramos(uid, trackPoints) {
    ...
    try {
      ...
      if (mejorados.length > 0) {
        ...
        delete this._cacheMemoria[uid]; // 🔥 FIX: invalidar caché de getData tras escribir
      }
  ...
  async repararTodoDesdeHistorialSiHaceFalta(uid, gamificationDataYaLeida = null) {
    ...
    try {
      ...
      const newLevel = this.getLevelByDistance(derivados.totalDistance);
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // volver a leer nada.
      delete this._cacheMemoria[uid]; // 🔥 FIX: invalidar caché de getData tras la transacción
  ...
  async _recalcularDerivadosDesdeHistorial(uid) {
    const vacio = {
      ...
      // 🔥 v5.17: ver más abajo -- ahora esta función también recalcula
      // estos 7 campos desde CERO a partir del historial real.
      totalDistance: 0, totalSessions: 0, totalZone4Minutes: 0, totalZone5Minutes: 0,
      countLongRuns: 0, countIntervals: 0, countStrengthRuns: 0,
      // 🔥 v5.19: ver más abajo -- los km de la zapatilla actual también se
      // recalculan desde la verdad, igual que totalDistance.
      currentShoeKm: 0
    };
    try {
      // 🔥 v5.19: FIX de fondo -- "el nivel ya se corrigió pero la
      // zapatilla se quedó con los km viejos". El usuario detectó que, tras
      // la auto-corrección de totalDistance (v5.17/5.18), su nivel bajó de
      // 6 a 3 (la distancia real de verdad era mucho menor que la que
      // llevaba acumulada por desviación) pero currentShoe.km se quedó
      // igual, con los mismos 700 y pico km de antes. Causa: a diferencia
      // de totalDistance (que este método reconstruye entero desde
      // globalFeed), currentShoe.km siempre se había gestionado con
      // FieldValue.increment/decrement puntual, sesión a sesión (ver
      // addKilometersToShoe/removeKilometersFromShoe más abajo) -- el
      // mismo patrón frágil que causaba la desviación de totalDistance
      // antes de v5.17, y que nunca llegó a corregirse aquí. Cualquier
      // desajuste histórico entre un marcado y su desmarcado (p.ej. si al
      // desmarcar fallaba la lectura de la distancia real guardada en el
      // muro y se caía al fallback de la distancia ESTIMADA del plan --
      // ver calendar.js, `distanciaRemovida` en el flujo de desmarcado) se
      // quedaba pegado para siempre en la zapatilla, sin ninguna forma de
      // autocorregirse, aunque totalDistance ya se hubiera arreglado solo.
      // Ahora se recalcula igual: sumando la distancia real de todas las
      // sesiones que de verdad existen en globalFeed desde que esta
      // zapatilla es la actual (el 'changedAt' de la última entrada de
      // shoeHistory; si nunca se ha cambiado de zapatilla, cuenta desde el
      // principio, igual que totalDistance).
      let shoeHistory = [];
      try {
      ...
      // 🔥 v5.17: FIX "insignia de zona imposible" reportado por el
      // usuario -- llevaba, según la propia app, 40 min acumulados de Z4
      // (tarjeta "Zonas · últimos 30 días", YA arreglada en la ronda
      // anterior para contar bien los bloques 'extras') y aun así le
      // saltó la insignia "60 min en Z4" al completar una sesión de Z2 sin
      // nada de Z4. Causa real: `totalZone4Minutes`/`totalZone5Minutes` (y
      // `totalDistance`/`totalSessions`/`countLongRuns`/`countIntervals`/
      // `countStrengthRuns`) NUNCA se recalculaban desde la verdad -- solo
      // se sumaban en updateAfterSession() y se restaban en removeSession()
      // sesión a sesión, así que cualquier desajuste puntual entre un
      // marcado y su desmarcado correspondiente (p.ej. si la sesión se
      // corrigió/regeneró entre medias, algo que ha pasado varias veces
      // durante las pruebas de esta app) se quedaba pegado para siempre en
      // el acumulado, sin ninguna forma de autocorregirse -- exactamente
      // el tipo de "insignia que sale de la nada" que se estaba reportando.
      // Ahora esta función (que YA se ejecuta en cada `removeSession`, ver
      // más abajo) también sirve para RECONSTRUIR estos 7 campos desde
      // CERO, sumando el desglose real de TODAS las sesiones que de verdad
      // existen ahora mismo en globalFeed -- la fuente de la verdad, no un
      // contador que se puede desviar. Object.assign(newData, derivados) en
      // removeSession() ya sustituye el valor calculado por resta con este,
      // más fiable, así que "desmarcar y volver a marcar" cualquier sesión
      // (cualquiera, no hace falta que sea la de la insignia) sirve como
      // "reinicio" inmediato de estos contadores para el usuario que ya
      // los tuviera desviados.
      let totalDistance = 0, totalSessions = 0;
      let totalZone4Minutes = 0, totalZone5Minutes = 0;
      let countLongRuns = 0, countIntervals = 0, countStrengthRuns = 0;
      // 🔥 v5.19: suma aparte, solo de las sesiones desde que la zapatilla
      // actual es la actual (ver cutoffDate más arriba).
      let currentShoeKm = 0;
      ...
      snap.forEach(doc => {
        ...
        // 🔥 v5.17 (ver comentario largo más arriba): recompuesto desde la
        // verdad de cada sesión que de verdad existe, no desde un
        // contador acumulado aparte.
        totalSessions++;
        totalDistance += dist;
        // 🔥 v5.19: solo cuenta para la zapatilla actual si la sesión es de
        // cuando esta zapatilla ya era la actual (o si nunca se ha
        // cambiado de zapatilla, en cuyo caso cutoffDate es null y cuenta
        // todo, igual que totalDistance).
        if (!cutoffDate || fecha >= cutoffDate) currentShoeKm += dist;
        if (d.trainingType === 'largo') countLongRuns++;
        else if (d.trainingType === 'series') countIntervals++;
        else if (d.trainingType === 'strength') countStrengthRuns++;
        if (d.desglose && window.PlanGenerator?._sumarMinutosPorZona) {
          totalZone4Minutes += PlanGenerator._sumarMinutosPorZona(d.desglose, 'Z4');
          totalZone5Minutes += PlanGenerator._sumarMinutosPorZona(d.desglose, 'Z5');
        }
      ...
      if (dias.length === 0) return { ...vacio, personalRecords, currentShoeKm };
      ...
      return {
        streakDays, lastSessionDate, daysOfWeek, earlyBirdCount, maxDistanceSingle, monthStreak, lastMonth, bestStreakDays, personalRecords,
        totalDistance, totalSessions, totalZone4Minutes, totalZone5Minutes, countLongRuns, countIntervals, countStrengthRuns,
        currentShoeKm
      };
    } catch (e) {
...
console.log('✅ gamification.js v5.22 - getData(uid) con caché en vivo (listener onSnapshot), sin TTL');