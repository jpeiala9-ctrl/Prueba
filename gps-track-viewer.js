// ==================== gps-track-viewer.js ====================
// Versión: 2.7 - Mapa robusto ante orientación (doble rAF + listener de
//                resize/orientationchange, igual que el mini-mapa del Muro
//                y el mapa en vivo del tracker)
// ====================
...
const GPSTrackViewer = {
  ...
  _crearModal(entry, ciudad) {
    ...
    modal.innerHTML = `
      ...
      <div id="gpsViewerMap" style="height:300px; flex-shrink:0; background:var(--stat-bg);"></div>
      <div style="padding:16px 20px; background:var(--bg-primary); border-top:1px solid var(--border-color);">
        ...
        <div style="display:flex; justify-content:center;">
          <button id="closeGpsViewerBtn" class="action-button" style="width:auto; padding:0 24px; margin:0;">CERRAR</button>
        </div>
      </div>
    `;
  ...
  async _initMap(entry) {
    ...
    // que Leaflet mida el contenedor (mismo arreglo que ya usa el mini-mapa
    // del Muro para el "medio mapa negro"): si el mapa se crea en el mismo
    // tick en que se insertó el HTML del modal, el contenedor puede no
    // tener aún su tamaño final -- y ese tamaño incorrecto queda mal
    // "grabado" en Leaflet aunque luego se llame a invalidateSize(), hasta
    // que algo (como rotar el móvil, que fuerza un resize real) le hace
    // recalcularlo de verdad. Con un doble requestAnimationFrame nos
    // aseguramos de que ya hubo al menos un pintado completo antes de
    // inicializar, sin depender de tener que rotar la pantalla para que se
    // vea bien.
    await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    if (!document.body.contains(mapContainer)) return; // se cerró el visor mientras esperábamos
    ...
    try {
      ...
      setTimeout(() => {
        if (!this.map || !document.body.contains(mapContainer)) return;
        encuadrar();
      }, 300);

      // Recalcular si el usuario rota el móvil (o cambia el tamaño de
      // ventana) mientras el visor está abierto, igual que ya hace el
      // mapa en vivo del tracker. Antes esto no existía aquí: el mapa se
      // quedaba con el tamaño (a veces incorrecto) que tenía al abrirse, y
      // solo se veía bien "por accidente" tras rotar, porque el propio
      // Leaflet escucha el evento nativo 'resize' de la ventana y se
      // repinta solo -- pero únicamente en ESE momento, no antes.
      this._onOrientationOrResize = () => setTimeout(encuadrar, 200);
      window.addEventListener('orientationchange', this._onOrientationOrResize);
      window.addEventListener('resize', this._onOrientationOrResize);
    } catch(e) {
...
console.log('✅ GPSTrackViewer v2.7 - Mapa robusto ante orientación (doble rAF + listener resize/orientationchange)');
