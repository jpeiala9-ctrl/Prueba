// ==================== app.js - VERSIÓN FINAL CON CACHÉ DE HTML Y ESTRUCTURA BASE ====================
// VERSIÓN: 5.1 - Crea contenedores aunque no haya caché
// ====================
...
const Utils = {
  ...
  parseTime(t) {
    ...
    if (!t.includes(':')) {
      ...
      if (minutos >= 10 && minutos <= 120) return minutos;
      return NaN;
    }
    const parts = t.split(':');
  ...
  handleFirebaseError(error) {
    ...
    switch(error.code) {
      ...
      case 'auth/invalid-email': message = 'Correo electrónico no válido'; break;
      case 'auth/weak-password': message = 'La contraseña debe tener al menos 6 caracteres'; break;
      case 'auth/user-not-found':
      case 'auth/wrong-password': message = 'Usuario o contraseña incorrectos'; break;
      case 'auth/too-many-requests': message = 'Demasiados intentos. Intenta más tarde'; break;
      case 'auth/network-request-failed': message = 'Error de conexión. Comprueba tu red'; break;
      case 'auth/requires-recent-login': message = 'Esta operación requiere autenticación reciente. Vuelve a iniciar sesión'; break;
      case 'permission-denied': message = 'No tienes permisos para esta acción'; break;
      case 'unavailable': message = 'Servicio no disponible. Intenta más tarde'; break;
      default: message = 'Error inesperado. Inténtalo de nuevo más tarde';
    }
...
const AppState = {
  ...
  historialPagination: { lastDoc: null, hasMore: true, loading: false },
  ...
  _actualizarBadgeComunidad() {
    ...
    if (!badge) return;
    const total = (this.solicitudesPendientesCount || 0) + (this.mensajesAmigosNoLeidos || 0);
    if (total > 0) {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (userData && !userData.username_lowercase && userData.username) {
      ...
      firebaseServices.db.collection('users').doc(uid).update({ username_lowercase: newLowercase }).catch(e => console.warn(e));
    ...
    if (uid) {
      ...
      if (this.isAdmin) localStorage.setItem('ri5_is_admin', 'true');
      else localStorage.removeItem('ri5_is_admin');
    } else {
    ...
    if(t) {
      if(this.isPremium && this.premiumExpiryDate && new Date() <= this.premiumExpiryDate) t.classList.add('premium');
      else t.classList.remove('premium');
    }
    if (this.currentUser) {
    ...
    if (soporteTab) soporteTab.innerHTML = this.isAdmin ? '🛠️ Administración' : '💬 Soporte';
    this.actualizarInterfazPremium();
    this.verificarExpiracionPremium();
    const adminTab = document.getElementById('adminTabButton');
    if (adminTab) adminTab.style.display = this.isAdmin ? 'inline-block' : 'none';
    this.actualizarBotonCalcular();
    if (uid && userData) localStorage.setItem('ri5_user_data', JSON.stringify(userData));
    else localStorage.removeItem('ri5_user_data');
    if (uid) {
  ...
  precargarDatos() {
    ...
    if (window.Profile) Profile.cargarPerfil().catch(e => console.warn(e));
    if (window.Friends) {
      Friends.cargarListaAmigos().catch(e => console.warn(e));
      if (document.getElementById('todosUsuariosList')) Friends.cargarTodosUsuarios(true).catch(e => console.warn(e));
    }
    if (window.UI && UI.cargarMensajesRecibidos) UI.cargarMensajesRecibidos().catch(e => console.warn(e));
  },
  ...
  verificarExpiracionPremium() {
    ...
    if (this.premiumExpiryDate) {
      ...
      if (diasRestantes <= 7 && diasRestantes > 0) {
        if (banner) { banner.style.display = 'block'; if (message) message.innerText = `⚠️ Tu premium expira en ${diasRestantes} días`; }
      } else if (diasRestantes <= 0) {
        if (banner) { banner.style.display = 'block'; if (message) message.innerText = `⚠️ Tu premium ha expirado`; }
  ...
  async incrementarCalculo() {
    ...
    try {
      await firebaseServices.db.collection('users').doc(this.currentUserId).update({ calculosMes: firebaseServices.FieldValue.increment(1) });
      this.calculosMes++;
    } catch (error) {
  ...
  resetHistorialPagination() {
    this.historialPagination = { lastDoc: null, hasMore: true, loading: false };
  },

  iniciarListeners() {
    this.detenerListeners();
    if (!this.currentUserId) return;
    this.unsubscribeFriendRequests = firebaseServices.db.collection('friendRequests')
      .where('to', '==', this.currentUserId).where('status', '==', 'pending')
      .onSnapshot((snapshot) => {
        ...
        if (this.solicitudesPendientesCount !== count) {
          ...
          if (activeTab === 'amigos') {
            ...
            if (activeAmigosTab === 'solicitudes' && window.Friends) Friends.cargarSolicitudesRecibidas();
          }
        }
      }, (error) => console.error('Error en listener de solicitudes:', error));
    this.unsubscribeConversations = firebaseServices.db.collection('conversations')
      .where('participants', 'array-contains', this.currentUserId)
      .onSnapshot(async () => {
        if (window.Chat) await Chat.updateUnreadBadge();
        this.actualizarBadgeChat();
      }, (error) => console.error('Error en listener de conversaciones:', error));
    this.unsubscribeMensajesSoporte = firebaseServices.db.collection('users').doc(this.currentUserId).collection('mensajes')
      .onSnapshot(async (snapshot) => {
        let noLeidos = 0;
        snapshot.forEach(doc => {
          const data = doc.data();
          if (!data.leido && data.toUid === this.currentUserId) noLeidos++;
        });
        ...
        if (soporteTab && soporteTab.classList.contains('active') && window.UI) {
          UI.cargarMensajesRecibidos();
          UI.cargarMensajesEnviados();
        }
      }, (error) => console.error('Error en listener de mensajes de soporte:', error));
  },

  detenerListeners() {
    if (this.unsubscribeFriendRequests) { this.unsubscribeFriendRequests(); this.unsubscribeFriendRequests = null; }
    if (this.unsubscribeConversations) { this.unsubscribeConversations(); this.unsubscribeConversations = null; }
    if (this.unsubscribeMensajesSoporte) { this.unsubscribeMensajesSoporte(); this.unsubscribeMensajesSoporte = null; }
  },
...
const Admin = {
  usersPagination: { lastDoc: null, hasMore: true, loading: false, searchTerm: '' },
  currentEditUserId: null,
  filtrosActuales: { premium: 'all', from: '', to: '', sort: 'username' },
  messagesPagination: { lastDoc: null, hasMore: true, loading: false, allMessagesCache: [] },
  _unsubscribeMensajes: null,
  _profileCache: {},

  async cargarEstadisticas() {
    try {
      const [totalUsers, premiumUsers, newUsers, sessionsToday] = await Promise.all([
        Storage.contarUsuarios(),
        Storage.contarUsuariosPremium(),
        Storage.contarUsuariosNuevos(7),
        Storage.contarSesionesHoy()
      ]);
      ...
      const elSessions = document.getElementById('statSessionsToday');
      ...
      if (elSessions) elSessions.textContent = sessionsToday;
  ...
  async cargarUsuarios(reset = false) {
    ...
    if (!container) { this.usersPagination.loading = false; return; }
    if (reset) {
    ...
    try {
      ...
      if (searchTerm) {
        ...
        let query1 = firebaseServices.db.collection('users').orderBy('username_lowercase').startAt(termLower).endAt(termLower + '\uf8ff').limit(50);
        let query2 = firebaseServices.db.collection('users').orderBy('username').startAt(searchTerm).endAt(searchTerm + '\uf8ff').limit(50);
        const [snapshot1, snapshot2] = await Promise.all([query1.get(), query2.get()]);
        const usersMap = new Map();
        snapshot1.forEach(doc => { const user = doc.data(); usersMap.set(doc.id, { uid: doc.id, ...user }); });
        snapshot2.forEach(doc => { const user = doc.data(); if (!usersMap.has(doc.id)) usersMap.set(doc.id, { uid: doc.id, ...user }); });
        ...
        let query = firebaseServices.db.collection('users').orderBy('username_lowercase').limit(20);
        if (this.usersPagination.lastDoc && !reset) query = query.startAfter(this.usersPagination.lastDoc);
        const snapshot = await query.get();
        if (snapshot.empty) {
          if (reset) container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay usuarios</p>';
          else { this.usersPagination.hasMore = false; if (container.innerHTML && !container.innerHTML.includes('No hay más usuarios')) container.innerHTML += '<p style="text-align:center; padding:20px; color: var(--text-secondary);">No hay más usuarios</p>'; }
          this.usersPagination.loading = false;
          this._actualizarBotonCargarMas();
          return;
        }
        ...
        usersData = snapshot.docs.map(doc => { const user = doc.data(); return { uid: doc.id, ...user }; });
      }
      ...
      let usersDataProcessed = usersData.map(u => {
        ...
        const esNuevo = !usuariosVistos.includes(uid) && (user.created && new Date(user.created) > new Date(Date.now() - 7*24*60*60*1000));
      ...
      if (this.filtrosActuales.premium !== 'all') {
        ...
        usersDataProcessed = usersDataProcessed.filter(u => u.user.premium === isPremium);
      }
      if (this.filtrosActuales.from) {
        const fromDate = new Date(this.filtrosActuales.from); fromDate.setHours(0,0,0,0);
        usersDataProcessed = usersDataProcessed.filter(u => { if (!u.user.created) return false; return new Date(u.user.created) >= fromDate; });
      }
      if (this.filtrosActuales.to) {
        const toDate = new Date(this.filtrosActuales.to); toDate.setHours(23,59,59,999);
        usersDataProcessed = usersDataProcessed.filter(u => { if (!u.user.created) return false; return new Date(u.user.created) <= toDate; });
      }
      if (this.filtrosActuales.sort === 'created') usersDataProcessed.sort((a, b) => new Date(b.user.created) - new Date(a.user.created));
      else if (this.filtrosActuales.sort === 'premium') usersDataProcessed.sort((a, b) => (b.user.premium ? 1 : 0) - (a.user.premium ? 1 : 0));
      else usersDataProcessed.sort((a, b) => a.username.localeCompare(b.username));
      this.renderUsersList(usersDataProcessed);
      if (searchTerm) this.usersPagination.hasMore = false;
      else this.usersPagination.hasMore = hasMore;
      if (!reset && !searchTerm) {
      ...
      if (reset) container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--zone-5);">Error al cargar usuarios. Revisa la consola.</p>';
      else { if (container.innerHTML && !container.innerHTML.includes('Error al cargar más')) container.innerHTML += '<p style="text-align:center; padding:20px; color: var(--zone-5);">Error al cargar más usuarios.</p>'; }
    } finally { this.usersPagination.loading = false; this._actualizarBotonCargarMas(); }
  },
  ...
  renderUsersList(usersData) {
    ...
    for (const u of usersData) {
      const premiumBadge = u.premium === 'SÍ' ? '<span class="usuario-premium-badge">PREMIUM</span>' : '<span class="usuario-premium-badge normal">GRATIS</span>';
      html += `
    ...
    container.querySelectorAll('.ver-perfil-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const uid = btn.dataset.uid; Admin.verPerfil(uid); }); });
    container.querySelectorAll('.mensaje-usuario-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const uid = btn.dataset.uid; const username = btn.dataset.username; Admin.abrirMensajeUsuario(uid, username); }); });
    container.querySelectorAll('.premium-usuario-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const uid = btn.dataset.uid; const username = btn.dataset.username; const esPremium = btn.dataset.premium === 'true'; const expires = btn.dataset.expires; Admin.abrirModalPremium(uid, username, esPremium, expires); }); });
    container.querySelectorAll('.eliminar-usuario-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const uid = btn.dataset.uid; const username = btn.dataset.username; Admin.eliminarUsuario(uid, username); }); });
    container.querySelectorAll('.usuario-header').forEach(header => { header.addEventListener('click', (e) => { const item = header.closest('.usuario-item'); if (item) Admin.toggleUsuario(item, item.dataset.uid); }); });
    this._actualizarBotonCargarMas();
  },
  ...
  toggleUsuario(element, uid) {
    ...
    if (element.classList.contains('nuevo')) {
      const usuariosVistos = JSON.parse(localStorage.getItem('admin_usuarios_vistos') || '[]');
      if (!usuariosVistos.includes(uid)) { usuariosVistos.push(uid); localStorage.setItem('admin_usuarios_vistos', JSON.stringify(usuariosVistos)); }
  ...
  cargarMasUsuarios() {
    if (this.usersPagination.hasMore && !this.usersPagination.loading) this.cargarUsuarios(false);
    else if (!this.usersPagination.hasMore) Utils.showToast('No hay más usuarios para cargar', 'info');
  },

  buscarUsuarios: Utils.debounce(function() {
    const searchEl = document.getElementById('adminUserSearch2');
    if (searchEl) document.getElementById('adminUserSearch').value = searchEl.value;
  ...
  async verPerfil(uid) {
    ...
    try {
      ...
      if (cached) {
        try { const { data, timestamp } = JSON.parse(cached); if (Date.now() - timestamp < 30000) { this._renderProfile(uid, data.userData, data.gam, data.entrenos); return; } } catch (e) {}
      }
      const [userDoc, gam, entrenosSnapshot] = await Promise.all([
        ...
        firebaseServices.db.collection('globalFeed').where('userId', '==', uid).orderBy('timestamp', 'desc').limit(5).get()
      ]);
      if (!userDoc.exists) { Utils.showToast('Usuario no encontrado', 'error'); modal.classList.remove('active'); return; }
      const userData = userDoc.data();
      const entrenos = entrenosSnapshot;
      const cacheData = { userData, gam, entrenos: entrenos.docs.map(d => ({ id: d.id, ...d.data() })) };
    ...
    } catch (error) { console.error('Error viendo perfil:', error); Utils.showToast('Error al cargar perfil', 'error'); document.getElementById('adminUserModal').classList.remove('active'); }
  },
    ...
    const html = `
      ...
      <div class="admin-modal-entrenos">
        ...
        ${entrenos.empty ? '<p style="color:var(--text-secondary);">Sin entrenamientos</p>' :
          entrenos.docs.map(d => `<div class="entreno-item">${d.data().trainingType || 'Sesión'} · ${d.data().distancia?.toFixed(2) || '0'} km · ${d.data().duration || '?'} min</div>`).join('')}
      </div>
    `;
    ...
    try {
      switch (tipo) {
        case 'total':
          ...
          const totalSnap = await firebaseServices.db.collection('users').orderBy('username_lowercase').limit(100).get();
          usuarios = totalSnap.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
          break;
        case 'premium':
          titulo = '⭐ Usuarios Premium';
          const premiumSnap = await firebaseServices.db.collection('users').where('premium', '==', true).orderBy('username_lowercase').limit(100).get();
          usuarios = premiumSnap.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
          break;
        case 'new':
          titulo = '🆕 Usuarios nuevos (últimos 7 días)';
          const fecha = new Date(); fecha.setDate(fecha.getDate() - 7); const fechaStr = fecha.toISOString();
          const newSnap = await firebaseServices.db.collection('users').where('created', '>=', fechaStr).orderBy('created', 'desc').limit(100).get();
        ...
        case 'sessions':
          ...
          if (userIds.length === 0) usuarios = [];
          else {
            const userPromises = [];
            for (let i = 0; i < userIds.length; i += 10) {
              const batch = userIds.slice(i, i + 10);
              userPromises.push(firebaseServices.db.collection('users').where('__name__', 'in', batch).get());
            }
            const userSnaps = await Promise.all(userPromises);
            usuarios = userSnaps.flatMap(snap => snap.docs.map(doc => ({ uid: doc.id, ...doc.data() })));
          }
          break;
        default: Utils.showToast('Filtro no válido', 'error'); Utils.hideLoading(); return;
      }
  ...
  abrirMensajeUsuario(uid, username) {
    ...
    if (texto && texto.trim()) this.enviarMensajeSoporteAdmin(uid, texto.trim());
  },
  ...
  async enviarMensajeUsuario(uid, texto) {
    ...
    Utils.showLoading();
    try {
      await Storage.enviarMensajeSoporte(AppState.currentUserId, uid, texto);
  ...
  async guardarPremium() {
    ...
    if (!expiry) { const d = new Date(); d.setMonth(d.getMonth() + 1); expiry = d.toISOString(); }
    else expiry = new Date(expiry + 'T23:59:59').toISOString();
    Utils.showLoading();
    try {
  ...
  async eliminarMensajeSoporte(mensajeId) {
    ...
    const confirmado = await Utils.confirm('ELIMINAR MENSAJE', '¿Eliminar este mensaje permanentemente? También se eliminará de la bandeja del usuario.');
    if (!confirmado) return;
    Utils.showLoading();
    try {
      const doc = await firebaseServices.db.collection('soporteMensajes').doc(mensajeId).get();
      if (!doc.exists) { Utils.showToast('El mensaje ya no existe', 'warning'); Utils.hideLoading(); return; }
      const data = doc.data();
      ...
      if (fromUid && fromUid !== adminUid) {
        const snapshot = await firebaseServices.db.collection('users').doc(fromUid).collection('mensajes')
          .where('timestamp', '==', data.timestamp).where('texto', '==', data.texto).get();
        const batch = firebaseServices.db.batch();
        snapshot.forEach(doc => batch.delete(doc.ref));
        await batch.commit();
      }
      if (toUid && toUid !== adminUid) {
        const snapshot = await firebaseServices.db.collection('users').doc(toUid).collection('mensajes')
          .where('timestamp', '==', data.timestamp).where('texto', '==', data.texto).get();
        const batch = firebaseServices.db.batch();
        snapshot.forEach(doc => batch.delete(doc.ref));
        await batch.commit();
      ...
      this.cargarMensajesUsuarios(true);
    } catch (error) { console.error('Error eliminando mensaje:', error); Utils.showToast('Error al eliminar mensaje', 'error'); }
    finally { Utils.hideLoading(); }
  },
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    if (!container) return;
    if (reset) container.innerHTML = '<div style="text-align:center; padding:40px; color: var(--text-secondary);">⏳ Cargando mensajes...</div>';
    try {
      ...
      for (const msg of mensajes) {
        ...
        html += `
          <div class="mensaje-soporte-item ${!leido ? 'no-leido' : ''}" data-id="${msg.id}">
            <div class="mensaje-soporte-header">
              <span class="mensaje-soporte-usuario">${nombre} ${badge}</span>
            ...
            <div class="mensaje-soporte-acciones">
              ...
              <button class="eliminar" data-id="${msg.id}">Eliminar</button>
            </div>
          </div>
        `;
      }
      ...
      container.querySelectorAll('.mensaje-soporte-item').forEach(el => {
        el.addEventListener('click', async (e) => {
          ...
          if (id && el.classList.contains('no-leido')) {
            await Storage.marcarMensajeSoporteLeido(id, msg.fromUid);
      ...
      container.querySelectorAll('.responder').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          const respuesta = prompt(`Responder a ${username}:`);
          if (respuesta && respuesta.trim()) this.enviarMensajeSoporteAdmin(uid, respuesta.trim());
        });
      });
      container.querySelectorAll('.eliminar').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          if (id) this.eliminarMensajeSoporte(id);
        });
      });
    } catch (error) {
  ...
  async enviarMensajeSoporteAdmin(usuarioUid, texto) {
    ...
    if (ok) { Utils.showToast('✅ Mensaje enviado', 'success'); this.cargarMensajesUsuarios(true); }
    else Utils.showToast('Error al enviar mensaje', 'error');
  },
  ...
  async enviarMensajeATodos() {
    ...
    try {
      ...
      for (const userDoc of users) {
        try { await Storage.enviarMensajeSoporte(AppState.currentUserId, userDoc.id, texto); enviados++; }
...
const UI = {
  ...
  validarCampo(c) {
    ...
    if(!AppState || !AppState.camposTocados[c]) { err.classList.remove('visible'); el.classList.remove('error'); return true; }
    let ok = true;
    if(c === 'age') { const a = parseInt(el.value); ok = !isNaN(a) && a >= 14 && a <= 85; }
    else if(c === 'time') { const t = Utils.parseTime(el.value); ok = !isNaN(t) && t >= 10 && t <= 90; }
    if(!ok) { err.innerText = c === 'age' ? 'Edad 14-85' : 'Formato MM:SS o solo minutos (ej. 27)'; err.classList.add('visible'); el.classList.add('error'); }
  ...
  async switchTab(tab) {
    if (window.Chat && window.Chat.closeChat) window.Chat.closeChat();
    if (tab !== 'muro' && window.Wall) Wall.detenerListener();
    ...
    for(let b of tabs) {
      if(b.textContent.includes(tab === 'entreno' ? 'ENTRENO' :
                               ...
                               tab === 'perfil' ? 'PERFIL' :
                               tab === 'amigos' ? 'AMIGOS' :
                               tab === 'muro' ? 'MURO' : 'ADMIN')) { b.classList.add('active'); break; }
    }
    const tabEl = document.getElementById(`tab-${tab}`);
    if (tabEl) tabEl.classList.add('active');
    try {
      if(tab === 'historial') { if (AppState) AppState.resetHistorialPagination(); this.cargarHistorialCompleto(true).catch(e => console.warn(e)); }
      if(tab === 'plan') { this.cargarHistorialPlanes().catch(e => console.warn(e)); }
      if(tab === 'soporte') { this.cargarMensajesRecibidos().catch(e => console.warn(e)); this.cargarMensajesEnviados().catch(e => console.warn(e)); }
      if(tab === 'perfil') {
        if (window.Profile) Profile.cargarPerfil(false).catch(e => console.warn(e));
        if (AppState.isAdmin) setTimeout(() => { if (document.getElementById('soporte-admin-panel')) Admin.cambiarSubtab('control'); }, 100);
      }
      if(tab === 'amigos') {
        if (window.Friends) {
          Friends.actualizarBadgeSolicitudes().catch(e => console.warn(e));
          const activeAmigosTab = document.querySelector('.amigos-tab.active');
          if (activeAmigosTab) {
            const tabText = activeAmigosTab.textContent.toLowerCase();
            if (tabText.includes('buscar')) { if (!Friends.todosUsuariosPagination.lastDoc) Friends.cargarTodosUsuarios(true).catch(e => console.warn(e)); }
            else if (tabText.includes('solicitudes')) Friends.cargarSolicitudesRecibidas().catch(e => console.warn(e));
            else if (tabText.includes('mis amigos')) Friends.cargarListaAmigos().catch(e => console.warn(e));
          } else Friends.cargarTodosUsuarios(true).catch(e => console.warn(e));
        }
      }
      if(tab === 'muro') { if (window.Wall) { Wall.detenerListener(); Wall.init(); } }
      if(tab === 'admin' && AppState && AppState.isAdmin) { Admin.cargarUsuarios(true).catch(e => console.warn(e)); Admin.cargarEstadisticas().catch(e => console.warn(e)); }
    } catch (error) { console.error(`Error cargando pestaña ${tab}:`, error); Utils.showToast('Error al cargar contenido', 'error'); }
    this.guardarEstado();
  },
    ...
    container.querySelectorAll('.mensaje-item').forEach(el => {
      ...
      if (header) {
        header.addEventListener('click', (e) => {
          ...
          if (!el.classList.contains('nuevo') && AppState.currentUserId) Storage.marcarLeido(AppState.currentUserId, idx);
          if (el.classList.contains('nuevo')) {
            const vistos = JSON.parse(sessionStorage.getItem(`user_mensajes_vistos_${tipoMsg}`) || '[]');
            if (!vistos.includes(mensajeId)) { vistos.push(mensajeId); sessionStorage.setItem(`user_mensajes_vistos_${tipoMsg}`, JSON.stringify(vistos)); }
      ...
      if (responderBtn) {
        responderBtn.addEventListener('click', (e) => {
          ...
          if (onToggleCallback) eval(onToggleCallback);
        });
      }
    });
  },
  ...
  async cargarMensajesRecibidos() {
    ...
    this.renderMessageList(container, recibidos, 'recibido', null);
    this.actualizarBadgeMensajes();
  },
  ...
  async cargarMensajesEnviados() {
    ...
    this.renderMessageList(container, enviados, 'enviado', null);
  },
  ...
  async borrarMensajeUsuario(idx, tipo) {
    ...
    if (!mensaje) { Utils.showToast('Mensaje no encontrado', 'error'); return; }
    const confirmed = await Utils.confirm('Eliminar mensaje', '¿Eliminar este mensaje permanentemente?');
    if (!confirmed) return;
    Utils.showLoading();
    try {
      await firebaseServices.db.collection('users').doc(AppState.currentUserId).collection('mensajes').doc(mensaje.id).delete();
      Utils.showToast('✅ Mensaje eliminado', 'success');
  ...
  async enviarMensajeUsuario() {
    ...
    if (!texto) { Utils.showToast('Escribe un mensaje', 'warning'); return; }
    const adminUid = await Storage.getAdminUid();
    if (!adminUid) { Utils.showToast('No se pudo encontrar al administrador', 'error'); return; }
    const ok = await Storage.enviarMensajeSoporte(AppState.currentUserId, adminUid, texto);
    if (ok) { document.getElementById('mensajeUsuario').value = ''; Utils.showToast('✅ Mensaje enviado a soporte', 'success'); await this.cargarMensajesEnviados(); await this.cargarMensajesRecibidos(); }
    else Utils.showToast('Error al enviar mensaje', 'error');
  },
  ...
  actualizarBadgeMensajes() {
    ...
    if (tab) { if (AppState.mensajesNoLeidos > 0) tab.classList.add('soporte-unread'); else tab.classList.remove('soporte-unread'); }
  },
  ...
  guardarEstado() {
    ...
    const estado = { activeTab, planVisible, planId: AppState.planActualId, trimestre: AppState.trimestreActual };
    sessionStorage.setItem('ri5_estado', JSON.stringify(estado));
    if (planVisible && AppState.planActualId) {
      const estadoPlan = { planId: AppState.planActualId, trimestre: AppState.trimestreActual, visible: true };
      localStorage.setItem(`ri5_plan_${uid}`, JSON.stringify(estadoPlan));
    } else localStorage.removeItem(`ri5_plan_${uid}`);
  },
  ...
  async restaurarEstado() {
    ...
    if (storedPlan) {
      try {
        ...
        if (visible && planId) {
          ...
          if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
            ...
            if (AppState) { AppState.planActualId = planId; AppState.trimestreActual = trimestre || 0; AppState.sesionesRealizadas = planExiste.sesionesRealizadas || {}; AppState.feedbackSesiones = planExiste.feedback || {}; AppState.planGeneradoActual = planExiste.params; }
            const calendario = document.getElementById("calendarioEntreno");
            const cuestionario = document.getElementById("cuestionarioEntreno");
      ...
      } catch (e) { console.warn('Error restaurando plan desde localStorage:', e); }
    }
    ...
    try {
      ...
      if (estado.planVisible && estado.planId && AppState && AppState.currentUserId) {
        const planExiste = await Storage.getPlanCompleto(AppState.currentUserId, estado.planId);
        if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
          if (AppState) { AppState.trimestreActual = estado.trimestre || 0; AppState.planActualId = estado.planId; AppState.sesionesRealizadas = planExiste.sesionesRealizadas || {}; AppState.feedbackSesiones = planExiste.feedback || {}; AppState.planGeneradoActual = planExiste.params; }
          this.cargarPlanDesdeHistorial(estado.planId).catch(e => console.warn(e));
        }
      }
...
// ==================== DASHBOARD CON CACHÉ DE HTML ====================

// ----- Restaurar dashboard desde caché (se llama inmediatamente al recargar) -----
function restaurarDashboardDesdeCache() {
  const tabInicio = document.getElementById('tab-inicio');
  if (!tabInicio) return;

  // Si hay caché, usarla
  const cachedHTML = localStorage.getItem('ri5_dashboard_html');
  if (cachedHTML && cachedHTML.length > 100) {
    tabInicio.innerHTML = cachedHTML;
    console.log('📦 Dashboard restaurado desde caché HTML (instantáneo)');
    // Re-inicializar eventos de zonas y consejos
    document.querySelectorAll('.zona-item').forEach(el => {
      el.onclick = function() { this.classList.toggle('expanded'); };
    });
    initDashboardConsejo();
    document.getElementById('chartBtnSemana')?.addEventListener('click', () => switchDashboardChart('semana'));
    document.getElementById('chartBtnMes')?.addEventListener('click', () => switchDashboardChart('mes'));
    document.getElementById('dashboardCuriosity')?.addEventListener('click', changeDashboardConsejo);
    return;
  }

  // Si NO hay caché, inyectar una estructura base con todos los contenedores
  tabInicio.innerHTML = `
    <div id="dashboardHero" style="position:relative; overflow:hidden; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:20px; padding:20px 20px 16px; margin-bottom:16px; display:flex; align-items:center; gap:16px;">
      <div id="dashboardAvatar" style="width:68px; height:68px; border-radius:50%; flex-shrink:0; background:var(--bg-primary); border:2px solid var(--gold); display:flex; align-items:center; justify-content:center; font-size:32px; overflow:hidden;">👤</div>
      <div style="flex:1; min-width:0;">
        <div id="dashboardGreeting" style="font-size:11px; color:var(--text-secondary); letter-spacing:1px; text-transform:uppercase; margin-bottom:2px;">Cargando...</div>
        <div id="dashboardUsername" style="font-size:20px; font-weight:600; color:var(--text-primary); letter-spacing:0.5px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">—</div>
        <div id="dashboardCity" style="font-size:12px; color:var(--text-secondary); margin-top:2px;"></div>
        <div id="dashboardShoe" style="display:inline-flex; align-items:center; gap:6px; margin-top:8px; padding:4px 10px; background:rgba(192,160,96,0.1); border:1px solid rgba(192,160,96,0.3); border-radius:20px; font-size:11px; color:var(--gold);">
          <span>👟</span><span id="dashboardShoeName">—</span>
          <span style="color:var(--text-secondary);" id="dashboardShoeKm"></span>
        </div>
      </div>
      <div id="dashboardLevelBadge" style="position:absolute; top:14px; right:14px; background:var(--bg-primary); border:1px solid var(--border-color); border-radius:10px; padding:4px 10px; text-align:center;">
        <div style="font-size:9px; color:var(--text-secondary); letter-spacing:1px; text-transform:uppercase;">Nivel</div>
        <div id="dashboardLevel" style="font-size:22px; font-weight:300; color:var(--gold); line-height:1;">—</div>
      </div>
    </div>

    <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:16px;">
      <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; text-align:center;">
        <div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:1px; margin-bottom:6px;">Esta semana</div>
        <div id="dashboardKmSemana" style="font-size:32px; font-weight:300; color:var(--gold); line-height:1;">0</div>
        <div style="font-size:11px; color:var(--text-secondary); margin-top:4px;">kilómetros</div>
      </div>
      <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; text-align:center;">
        <div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:1px; margin-bottom:6px;">Sesiones</div>
        <div id="dashboardSesionesSemana" style="font-size:32px; font-weight:300; color:var(--gold); line-height:1;">0</div>
        <div style="font-size:11px; color:var(--text-secondary); margin-top:4px;">esta semana</div>
      </div>
    </div>

    <div id="dashboardUltimaSesion" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
        <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">Última sesión</span>
        <span style="font-size:18px;">🏃</span>
      </div>
      <div id="dashboardUltimaSesionContent" style="color:var(--text-secondary); font-size:13px; cursor:pointer;">Sin sesiones registradas aún.</div>
    </div>

    <div id="dashboardProximaSesion" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
        <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">Próxima sesión</span>
        <span style="font-size:18px;">📅</span>
      </div>
      <div id="dashboardProximaSesionContent" style="color:var(--text-secondary); font-size:13px; cursor:pointer;">Sin plan activo.</div>
    </div>

    <div id="dashboardChartCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:4px;">
        <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">📈 Progreso</span>
        <div style="display:flex; gap:6px;">
          <button id="chartBtnSemana" onclick="switchDashboardChart('semana')" style="font-size:10px; padding:3px 10px; border-radius:20px; border:1px solid var(--gold); background:var(--gold); color:#000; cursor:pointer; font-weight:600; letter-spacing:0.5px;">7D</button>
          <button id="chartBtnMes" onclick="switchDashboardChart('mes')" style="font-size:10px; padding:3px 10px; border-radius:20px; border:1px solid var(--border-color); background:transparent; color:var(--text-secondary); cursor:pointer; letter-spacing:0.5px;">30D</button>
        </div>
      </div>
      <div id="dashboardChartContent" style="margin-top:8px;">
        <canvas id="dashboardKmChart" height="140" style="width:100%;"></canvas>
      </div>
    </div>

    <div id="dashboardZonasCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
        <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">❤️ Mis zonas</span>
        <span style="font-size:18px;">📊</span>
      </div>
      <div id="dashboardZonasContent" style="color:var(--text-secondary); font-size:13px;">Cargando zonas...</div>
    </div>

    <div id="dashboardPlanProgresoCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px; display:none;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
        <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">📋 Progreso del plan</span>
        <span id="dashboardPlanProgresoPct" style="font-size:13px; font-weight:700; color:var(--gold);">0%</span>
      </div>
      <div style="width:100%; height:8px; background:var(--bg-primary); border-radius:10px; overflow:hidden;">
        <div id="dashboardPlanProgresoBar" style="height:100%; width:0%; background:var(--gold); border-radius:10px;"></div>
      </div>
      <div id="dashboardPlanProgresoTexto" style="font-size:12px; color:var(--text-secondary); margin-top:8px;"></div>
    </div>

    <div class="curiosity" id="dashboardCuriosity" onclick="changeDashboardConsejo()" style="margin-bottom:16px;">
      <span id="dashboardConsejoText">> CONSEJO DEL DÍA_</span>
      <small>// pulsa para nuevo consejo</small>
    </div>
  `;
  console.log('📦 Creada estructura base del dashboard (sin caché)');
  initDashboardConsejo();
}

// ----- Guardar el HTML del dashboard en caché (solo si se genera correctamente) -----
function guardarDashboardEnCache() {
  const tabInicio = document.getElementById('tab-inicio');
  if (!tabInicio) return;
  const html = tabInicio.innerHTML;
  if (html && html.length > 200 && !html.includes('Error al cargar el dashboard')) {
    localStorage.setItem('ri5_dashboard_html', html);
    console.log('✅ HTML del dashboard guardado en caché');
  } else {
    console.warn('⚠️ No se guardó el HTML porque parece vacío o con error');
  }
}
...
function obtenerSaludo() {
  ...
  if (h >= 6  && h < 14) return '☀️ Buenos días';
  if (h >= 14 && h < 21) return '🌤 Buenas tardes';
  return '🌙 Buenas noches';
}

function fmtRitmo(minKm) {
...
function abrirModalDetalleSesion(sesionData, esPlan = false) {
...
async function cargarDashboard() {
  try {
    ...
    if (greetEl) greetEl.textContent = obtenerSaludo();
    ...
    try {
      ...
      if (nameEl) {
        const nombre = userData.username ? Utils.capitalizeUsername(userData.username) : '—';
        nameEl.textContent = nombre;
      }
      const cityEl = document.getElementById('dashboardCity');
      if (cityEl && profile.city) cityEl.innerHTML = `📍 ${Utils.escapeHTML(profile.city)}`;
      const avatarEl = document.getElementById('dashboardAvatar');
      if (avatarEl && profile.photoURL) {
        const yaTieneEstaFoto = avatarEl.querySelector('img')?.src === profile.photoURL;
        if (!yaTieneEstaFoto) {
          await new Promise(resolve => { const preImg = new Image(); preImg.onload = resolve; preImg.onerror = resolve; preImg.src = profile.photoURL; });
          avatarEl.innerHTML = `<img src="${Utils.escapeHTML(profile.photoURL)}" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">`;
        }
      }
      try {
        const gamDoc = await firebaseServices.db.collection('gamification').doc(AppState.currentUserId).get();
        const gam = gamDoc.exists ? gamDoc.data() : null;
        if (gam) {
          const lvlEl = document.getElementById('dashboardLevel');
          const lvlBadge = document.getElementById('dashboardLevelBadge');
          const levelColor = Gamification.getColorByLevel(gam.level || 1);
          if (lvlEl) lvlEl.textContent = gam.level || 1;
          if (lvlEl) lvlEl.style.color = levelColor;
          if (lvlBadge) lvlBadge.style.borderColor = levelColor + '60';
          const shoe = await Gamification.getCurrentShoe(AppState.currentUserId);
          const shoeNameEl = document.getElementById('dashboardShoeName');
          const shoeKmEl = document.getElementById('dashboardShoeKm');
          if (shoe && shoe.name) {
            if (shoeNameEl) shoeNameEl.textContent = shoe.name;
            if (shoeKmEl) shoeKmEl.textContent = `· ${(shoe.km || 0).toFixed(0)} km`;
          } else {
            if (shoeNameEl) shoeNameEl.textContent = 'Sin zapatilla';
          }
        }
      } catch(e) {}
    } catch(e) { console.warn('Dashboard: error cargando usuario', e); }
    ...
    try {
      ...
      const inicioSemana = kmInicioSemana();
      let kmSemana = 0, sesionesSemana = 0;
      entries.forEach(e => {
        const fecha = e.timestamp?.toDate ? e.timestamp.toDate() : new Date(e.timestamp);
        if (fecha >= inicioSemana) {
          const km = parseFloat(e.gpsDistanceKm || e.distancia || 0);
          if (isFinite(km)) kmSemana += km;
          sesionesSemana++;
        }
      });
      const kmEl = document.getElementById('dashboardKmSemana');
      if (kmEl) kmEl.textContent = kmSemana > 0 ? kmSemana.toFixed(1) : '0';
      const sesEl = document.getElementById('dashboardSesionesSemana');
      if (sesEl) sesEl.textContent = sesionesSemana;
      if (entries.length > 0) {
        const ult = entries[0];
        const fecha = ult.timestamp?.toDate ? ult.timestamp.toDate() : new Date(ult.timestamp);
        const diasAtras = Math.floor((Date.now() - fecha) / 86400000);
        const cuandoStr = diasAtras === 0 ? 'Hoy' : diasAtras === 1 ? 'Ayer' : `Hace ${diasAtras} días`;
        const tipo = ult.trainingName || ult.trainingType || 'Sesión';
        const dist = parseFloat(ult.gpsDistanceKm || ult.distancia || 0);
        const distStr = dist > 0 ? `${dist.toFixed(2)} km` : '';
        const durMs = ult.gpsDurationMs || (ult.duration ? ult.duration * 60000 : 0);
        const durStr = durMs > 0 ? fmtMs(durMs) : ult.duration ? `${ult.duration} min` : '';
        const tipoEmoji = { rodaje:'🏃', tempo:'⚡', series:'🔁', largo:'📏', strength:'💪' }[ult.trainingType] || '🏃';
        const gpsBadge = ult.hasGPS ? '<span style="font-size:10px; font-weight:600; letter-spacing:0.5px; color:#c0a060; background:rgba(192,160,96,0.12); border:1px solid rgba(192,160,96,0.25); border-radius:20px; padding:2px 8px; margin-left:6px;">📍 GPS</span>' : '';
        const ultEl = document.getElementById('dashboardUltimaSesionContent');
        if (ultEl) {
          ultEl.innerHTML = `
            <div style="display:flex; align-items:flex-start; gap:12px;">
              <span style="font-size:28px; line-height:1;">${tipoEmoji}</span>
              <div style="flex:1;">
                <div style="font-size:15px; font-weight:500; color:var(--text-primary); margin-bottom:4px;">
                  ${Utils.escapeHTML(tipo)} ${gpsBadge}
                </div>
                <div style="font-size:12px; color:var(--text-secondary);">${cuandoStr}</div>
                <div style="display:flex; gap:16px; margin-top:10px; flex-wrap:wrap;">
                  ${distStr ? `<div style="text-align:center;"><div style="font-size:18px; font-weight:600; color:var(--gold);">${distStr}</div><div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.5px;">distancia</div></div>` : ''}
                  ${durStr  ? `<div style="text-align:center;"><div style="font-size:18px; font-weight:600; color:var(--gold);">${durStr}</div><div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.5px;">tiempo</div></div>` : ''}
                  ${dist > 0 && durMs > 0 ? `<div style="text-align:center;"><div style="font-size:18px; font-weight:600; color:var(--gold);">${fmtRitmo((durMs/1000/60)/dist)}</div><div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.5px;">min/km</div></div>` : ''}
                </div>
              </div>
            </div>
          `;
          ultEl.style.cursor = 'pointer';
          ultEl.onclick = () => {
            const sesionObj = {
              tipo: ult.trainingType || 'rodaje',
              duracion: ult.duration || 45,
              detalle: {
                nombre: ult.trainingName || ult.trainingType || 'Sesión',
                objetivo: 'Sesión registrada con GPS',
                porque: 'Entrenamiento completado',
                zona: ult.zone || 'Z2',
                ritmoObjetivo: dist > 0 && durMs > 0 ? fmtRitmo((durMs/1000/60)/dist) : '--:--',
                sensacion: '—',
                estructura: '',
                pasosDetallados: [],
                distanciaEstimada: dist,
                tssEstimada: ult.tss || 0
              }
            };
            abrirModalDetalleSesion(sesionObj, true);
          };
        }
      }
    } catch(e) { console.warn('Dashboard: error cargando entrenamientos', e); }

    // 3. PRÓXIMA SESIÓN DEL PLAN
    try {
      let planId = AppState.planActualId;
      if (!planId) {
        const planSnap = await firebaseServices.db.collection('users').doc(AppState.currentUserId).collection('planes')
          .orderBy('fechaCreacion', 'desc').limit(1).get();
        if (!planSnap.empty) { planId = planSnap.docs[0].id; AppState.planActualId = planId; }
      }
      if (planId) {
        const planDoc = await firebaseServices.db.collection('users').doc(AppState.currentUserId).collection('planes').doc(planId).get();
        if (planDoc.exists) {
          ...
          for (const s of sesiones) {
            let fechaSesion = s.fecha ? new Date(s.fecha) : null;
            if (!fechaSesion && s.diaGlobal) {
              const fechaInicio = plan.params?.fechaInicio ? new Date(plan.params.fechaInicio) : new Date();
              fechaSesion = new Date(fechaInicio);
              fechaSesion.setDate(fechaSesion.getDate() + (s.diaGlobal - 1));
            }
            if (!fechaSesion) continue;
            fechaSesion.setHours(0,0,0,0);
            if (fechaSesion >= hoy && !s.realizada) {
              if (!proxima || fechaSesion < new Date(proxima.fecha)) {
                proxima = { ...s, fecha: fechaSesion };
              }
            }
          }
          const proxEl = document.getElementById('dashboardProximaSesionContent');
          if (proxEl && proxima) {
            const fp = new Date(proxima.fecha);
            const dias = Math.round((fp - hoy) / 86400000);
            const cuandoStr = dias === 0 ? 'Hoy' : dias === 1 ? 'Mañana' : `En ${dias} días`;
            const tipoEmoji = { rodaje:'🏃', tempo:'⚡', series:'🔁', largo:'📏', strength:'💪', descanso:'😴' }[proxima.tipo] || '📅';
            proxEl.innerHTML = `
              <div style="display:flex; align-items:center; gap:12px;">
                <span style="font-size:28px;">${tipoEmoji}</span>
                <div>
                  <div style="font-size:15px; font-weight:500; color:var(--text-primary);">${Utils.escapeHTML(proxima.nombre || proxima.tipo || 'Sesión')}</div>
                  <div style="font-size:12px; color:var(--gold); margin-top:2px;">${cuandoStr} · ${fp.toLocaleDateString('es-ES',{weekday:'long', day:'numeric', month:'short'})}</div>
                  ${proxima.duracion ? `<div style="font-size:11px; color:var(--text-secondary); margin-top:4px;">⏱ ${proxima.duracion} ${proxima.unidad || 'min'}</div>` : ''}
                </div>
              </div>
            `;
            proxEl.style.cursor = 'pointer';
            proxEl.onclick = () => {
              if (!proxima.detalle) {
                const sesionOriginal = sesiones.find(s => s.diaGlobal === proxima.diaGlobal);
                if (sesionOriginal && sesionOriginal.detalle) proxima.detalle = sesionOriginal.detalle;
                else {
                  proxima.detalle = {
                    nombre: proxima.nombre || proxima.tipo || 'Sesión',
                    objetivo: 'Sesión del plan',
                    porque: 'Sigue tu plan de entrenamiento',
                    zona: 'Z2',
                    ritmoObjetivo: '--:--',
                    sensacion: '—',
                    estructura: '',
              ...
              abrirModalDetalleSesion(proxima, true);
            };
          } else if (proxEl) {
            proxEl.innerHTML = `<span style="color:var(--text-secondary); font-size:13px;">Plan activo sin sesiones pendientes. <button onclick="switchTabFromDashboard('plan')" style="background:none; border:none; color:var(--gold); cursor:pointer; font-size:13px; padding:0; text-decoration:underline;">Ver plan →</button></span>`;
            proxEl.style.cursor = 'default';
            proxEl.onclick = null;
          }
          const progresoCard = document.getElementById('dashboardPlanProgresoCard');
          if (progresoCard && sesiones.length > 0) {
            const sesionesValidas = sesiones.filter(s => s.tipo !== 'descanso');
            const realizadasMap = plan.sesionesRealizadas || {};
            const completadas = sesionesValidas.filter(s => !!realizadasMap[s.diaGlobal]).length;
            const totalSesiones = sesionesValidas.length;
            const pct = totalSesiones > 0 ? Math.round((completadas / totalSesiones) * 100) : 0;
            progresoCard.style.display = 'block';
            const pctEl = document.getElementById('dashboardPlanProgresoPct');
            const barEl = document.getElementById('dashboardPlanProgresoBar');
            const txtEl = document.getElementById('dashboardPlanProgresoTexto');
            if (pctEl) pctEl.textContent = `${pct}%`;
            if (barEl) setTimeout(() => { barEl.style.width = `${pct}%`; }, 50);
            if (txtEl) txtEl.textContent = `${completadas} de ${totalSesiones} sesiones completadas`;
          }
        }
      }
    } catch(e) { console.warn('Dashboard: error cargando plan', e); }

    // 4. ZONAS (con comprobación de existencia del contenedor)
    const zonasContent = document.getElementById('dashboardZonasContent');
    if (!zonasContent) {
      console.warn('⚠️ No se encontró #dashboardZonasContent, se omite renderizado de zonas');
    } else {
      try {
        const raw = localStorage.getItem('ri5_last_calculation');
        if (raw && AppState.lastZones && AppState.lastZones.length > 0) {
          const calc = JSON.parse(raw);
          if (calc && calc.zones && calc.ul && calc.ritmoBase) {
            const ul = calc.ul;
            const r = calc.ritmoBase;
            const zonas = calc.zones;
            const colores = ['#8AA0B0', '#9BB5A0', '#C9A78B', '#C99BA5', '#9AA5A5', '#8A8A8A'];
            const nombresCortos = ['Rec.', 'Base', 'Tempo', 'Umbral', 'VO₂', 'Vel.'];
            let html = `<div style="display:flex; flex-direction:column; gap:10px;">`;
            zonas.forEach((z, index) => {
              const id = `Z${index+1}`;
              const fcMin = Math.round(ul * z[2]);
              const fcMax = index < 5 ? Math.round(ul * z[3]) : null;
              const fcStr = fcMax ? `${fcMin}–${fcMax}%` : `>${fcMin}%`;
              const ritmo = fmtRitmo(r * z[4]);
              let descRaw = z[6] || '';
              const parrafos = [...descRaw.matchAll(/<p[^>]*>(.*?)<\/p>/gs)].map(m => m[1].replace(/<[^>]*>/g, '').trim()).filter(p => p.length > 0);
              const propositoRaw = parrafos.find(p => /PROPÓSITO/i.test(p)) || parrafos[0] || '';
              const desc = propositoRaw.replace(/^.*?PROPÓSITO:?\s*/i, '').trim();
              html += `
                <div class="zona-item" data-zona="${id}" onclick="this.classList.toggle('expanded')" style="background: var(--bg-primary); border-radius: 12px; padding: 12px 14px; border-left: 4px solid ${colores[index]}; display: flex; align-items: center; gap: 14px; cursor: pointer; transition: all 0.25s ease; position: relative; overflow: visible;">
                  <div class="zona-circle" style="width: 44px; height: 44px; border-radius: 50%; display: flex; flex-direction: column; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; flex-shrink: 0; color: #fff; background: ${colores[index]};">
                    ${id}<small style="font-size:8px; font-weight:400; opacity:0.8; margin-top:-2px;">${nombresCortos[index]}</small>
                  </div>
                  <div class="zona-info" style="flex:1; min-width:0;">
                    <div class="nombre" style="font-weight:600; font-size:14px; color:var(--text-primary);">${z[1]}</div>
                    <div class="detalle" style="font-size:11px; color:var(--text-secondary); margin-top:2px;">FC ${fcStr}</div>
                    <div class="ritmo" style="font-size:12px; font-weight:500; color:#c0a060; margin-top:2px;">⏱ ${ritmo} min/km</div>
                    <div class="zona-expand" style="max-height: 0; overflow: hidden; transition: max-height 0.4s ease, margin 0.3s ease, opacity 0.3s ease; margin-top: 0; font-size: 12px; color: var(--text-secondary); line-height: 1.6; opacity: 0;">
                      <p style="margin:4px 0; white-space:pre-wrap; word-break:break-word;"><strong style="color:#c0a060;">📌</strong> ${desc}</p>
                    </div>
                  </div>
                </div>
              `;
            ...
            html += `</div>`;
            zonasContent.innerHTML = html;
            if (!document.getElementById('zonaExpansionStyle')) {
              const style = document.createElement('style');
              style.id = 'zonaExpansionStyle';
              style.textContent = `
                .zona-item.expanded {
                ...
                .zona-item.expanded .zona-expand {
            ...
            zonasContent.innerHTML = `
              <div style="color:var(--text-secondary); font-size:13px;">
                Aún no has calculado tus zonas.
                <button onclick="switchPerfilSubtab('perfil-entreno'); switchTabFromDashboard('perfil');" style="background:none; border:none; color:var(--gold); cursor:pointer; font-size:13px; padding:0; text-decoration:underline;">Calcular →</button>
              </div>
            `;
          }
        } else {
        ...
        console.warn('Error mostrando zonas', e);
        zonasContent.innerHTML = `
          <div style="color:var(--text-secondary); font-size:13px;">
            Error al cargar zonas.
            <button onclick="switchPerfilSubtab('perfil-entreno'); switchTabFromDashboard('perfil');" style="background:none; border:none; color:var(--gold); cursor:pointer; font-size:13px; padding:0; text-decoration:underline;">Recalcular →</button>
          </div>
        `;
      }
    }
    ...
    // 5. GRÁFICAS
    cargarGraficaDashboard('semana').catch(e => console.warn('Error en gráfica:', e));
    cargarZonasUsadasDashboard().catch(e => console.warn('Error en zonas usadas:', e));

    // 6. CONSEJO DEL DÍA
    initDashboardConsejo();

    // 7. GUARDAR EL HTML EN CACHÉ (SOLO SI SE COMPLETÓ CON ÉXITO)
    guardarDashboardEnCache();

  } catch (error) {
    console.error('❌ Error crítico en cargarDashboard:', error);
    const container = document.getElementById('tab-inicio');
    if (container) {
      container.innerHTML = `
        <div style="padding: 40px 20px; text-align: center; color: var(--zone-5);">
          <strong>⚠️ Error al cargar el dashboard</strong>
          <p style="font-size: 14px; margin-top: 10px;">
            ${error.message || 'Intenta recargar la página o contacta con soporte.'}
          </p>
        </div>
      `;
    }
    // No guardamos el HTML con error
  }
}

// ----- GRÁFICAS -----
let _dashboardChartInstance = null;
let _dashboardZonasUsadasInstance = null;
let _dashboardChartPeriodo = 'semana';

async function cargarChartJsSiFalta() {
...
function _coloresTemaGrafica() {
  const isLight = document.body.classList.contains('manual-light');
  return {
...
window.switchDashboardChart = function(periodo) {
  _dashboardChartPeriodo = periodo;
  const btnS = document.getElementById('chartBtnSemana');
  const btnM = document.getElementById('chartBtnMes');
  if (btnS && btnM) {
    if (periodo === 'semana') {
      btnS.style.background = 'var(--gold)'; btnS.style.color = '#000'; btnS.style.borderColor = 'var(--gold)';
      btnM.style.background = 'transparent'; btnM.style.color = 'var(--text-secondary)'; btnM.style.borderColor = 'var(--border-color)';
    } else {
      btnM.style.background = 'var(--gold)'; btnM.style.color = '#000'; btnM.style.borderColor = 'var(--gold)';
      btnS.style.background = 'transparent'; btnS.style.color = 'var(--text-secondary)'; btnS.style.borderColor = 'var(--border-color)';
    }
  }
  cargarGraficaDashboard(periodo);
};

async function cargarGraficaDashboard(periodo) {
  if (!AppState.currentUserId) return;
  const canvas = document.getElementById('dashboardKmChart');
  if (!canvas) return;
  ...
  try {
    try { await cargarChartJsSiFalta(); } catch (e) {
      console.warn('Chart.js no disponible', e);
      const content = document.getElementById('dashboardChartContent');
      if (content) content.innerHTML = '<div style="color: var(--text-secondary);">📊 Gráfica no disponible</div>';
      return;
    }
    let snap;
    try {
      snap = await firebaseServices.db.collection('globalFeed')
        .where('userId', '==', AppState.currentUserId)
        .where('timestamp', '>=', firebaseServices.Timestamp.fromDate(desde))
        .orderBy('timestamp', 'asc')
        .get();
    } catch (innerErr) {
      console.warn('Fallback gráfica km (sin orderBy, posible falta de índice):', innerErr.message);
      const snapAll = await firebaseServices.db.collection('globalFeed').where('userId', '==', AppState.currentUserId).get();
      const docsFiltrados = snapAll.docs.filter(doc => {
        const d = doc.data();
        const fecha = d.timestamp?.toDate ? d.timestamp.toDate() : new Date(d.timestamp);
        return fecha >= desde;
      });
      snap = { empty: docsFiltrados.length === 0, forEach: (cb) => docsFiltrados.forEach(cb) };
    }
    const contentEl = document.getElementById('dashboardChartContent');
    if (snap.empty) {
      canvas.style.display = 'none';
      if (contentEl) contentEl.innerHTML = '<div style="color:var(--text-secondary);font-size:13px;text-align:center;padding:20px 0;">Sin sesiones en este período</div><canvas id="dashboardKmChart" height="140" style="width:100%;display:none;"></canvas>';
      return;
    }
    if (contentEl && !document.getElementById('dashboardKmChart')) {
      contentEl.innerHTML = '<canvas id="dashboardKmChart" height="140" style="width:100%;"></canvas>';
    }
    const canvasFinal = document.getElementById('dashboardKmChart');
    ...
    snap.forEach(doc => {
      ...
      kmPorDia[key] = (kmPorDia[key] || 0) + km;
    });
    const labels = [];
    const data = [];
    for (let i = dias - 1; i >= 0; i--) {
      ...
      labels.push(key);
      data.push(parseFloat((kmPorDia[key] || 0).toFixed(2)));
    }
    const tema = _coloresTemaGrafica();
    if (_dashboardChartInstance) { _dashboardChartInstance.destroy(); _dashboardChartInstance = null; }
    const ctx = canvasFinal.getContext('2d');
    _dashboardChartInstance = new Chart(ctx, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'km', data, backgroundColor: 'rgba(192,160,96,0.85)', borderColor: '#c0a060', borderWidth: 1, borderRadius: 6, borderSkipped: false }] },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${ctx.parsed.y.toFixed(2)} km` } } },
        scales: {
          x: { grid: { color: tema.grid }, ticks: { color: tema.text, font: { size: 10 }, maxRotation: 0, maxTicksLimit: periodo === 'semana' ? 7 : 10 } },
          y: { grid: { color: tema.grid }, ticks: { color: tema.text, font: { size: 10 }, callback: v => v + ' km' }, beginAtZero: true }
        }
      }
    });
  } catch(e) {
    console.warn('Error cargando gráfica dashboard:', e);
    if (e && e.message && e.message.includes('index')) console.warn('⚠️ Firestore pide crear un índice compuesto para esta consulta.');
    const contentEl = document.getElementById('dashboardChartContent');
    if (contentEl) contentEl.innerHTML = '<div style="color:var(--text-secondary);font-size:13px;text-align:center;padding:20px 0;">No se pudo cargar el progreso</div>';
  }
}

async function cargarZonasUsadasDashboard() {
  if (!AppState.currentUserId) return;
  const canvas = document.getElementById('dashboardZonasUsadasChart');
  const card = document.getElementById('dashboardZonasUsadasCard');
  ...
  try {
    ...
    try {
      ...
      console.warn('Fallback zonas usadas (posible falta de índice):', innerErr.message);
      const snapAll = await firebaseServices.db.collection('globalFeed').where('userId', '==', AppState.currentUserId).get();
      const docsFiltrados = snapAll.docs.filter(doc => {
        const d = doc.data();
        const fecha = d.timestamp?.toDate ? d.timestamp.toDate() : new Date(d.timestamp);
    ...
    const conteoZonas = { Z1:0, Z2:0, Z3:0, Z4:0, Z5:0, Z6:0 };
    let huboZonas = false;
    snap.forEach(doc => {
      const d = doc.data();
      const zona = d.zone;
      if (zona && conteoZonas.hasOwnProperty(zona)) { conteoZonas[zona]++; huboZonas = true; }
    });
    if (!huboZonas) { card.style.display = 'none'; return; }
    card.style.display = 'block';
    const colores = ['#8AA0B0', '#9BB5A0', '#C9A78B', '#C99BA5', '#9AA5A5', '#8A8A8A'];
    const tema = _coloresTemaGrafica();
    if (_dashboardZonasUsadasInstance) { _dashboardZonasUsadasInstance.destroy(); _dashboardZonasUsadasInstance = null; }
    await cargarChartJsSiFalta();
    const ctx = canvas.getContext('2d');
    _dashboardZonasUsadasInstance = new Chart(ctx, {
      type: 'doughnut',
      data: { labels: Object.keys(conteoZonas), datasets: [{ data: Object.values(conteoZonas), backgroundColor: colores, borderWidth: 0 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: tema.text, font: { size: 11 }, padding: 10 } } } }
    });
  } catch(e) {
    console.warn('Error cargando zonas más usadas:', e);
    if (e && e.message && e.message.includes('index')) console.warn('⚠️ Firestore pide crear un índice compuesto para esta consulta.');
...
// ----- CONSEJO DEL DÍA -----
const _consejos = [
  'Corre el 80% de tus km en Z2. El progreso llega despacio, pero llega.',
  'La recuperación ES el entrenamiento. Sin descanso no hay adaptación.',
  'Tu FC en reposo baja cuando estás mejorando. Es la mejor métrica de forma.',
  'La tirada larga semanal es la sesión más importante. Hazla despacio.',
  'Un corredor con datos mejora 3 veces más rápido que uno sin ellos.',
  'Entrena por zonas, no solo por ritmo. El calor y el desnivel lo cambian todo.',
  'Las series en Z5 desarrollan el corazón. Dos veces por semana es suficiente.',
  'Descansa mínimo 1 día por semana. El cuerpo se adapta en el descanso.',
  'La hidratación afecta la FC. Bebe antes, durante y después.',
  'El umbral de lactato es la clave. Subirlo es el objetivo principal del entrenamiento.',
];
let _consejoIdx = Math.floor(Math.random() * _consejos.length);

function initDashboardConsejo() {
  const el = document.getElementById('dashboardConsejoText');
  if (el) el.textContent = '> ' + _consejos[_consejoIdx] + '_';
}
function changeDashboardConsejo() {
  _consejoIdx = (_consejoIdx + 1) % _consejos.length;
  initDashboardConsejo();
}

function switchTabFromDashboard(tab) {
  if (tab === 'entreno') {
    if (window.UI && window.UI.switchTab) window.UI.switchTab('perfil');
    else if (window.switchTab) window.switchTab('perfil');
    document.querySelectorAll('.bottom-nav-item').forEach(b => b.classList.remove('active'));
    const navBtn = document.querySelector('.bottom-nav-item[data-tab="perfil"]');
    if (navBtn) navBtn.classList.add('active');
    setTimeout(() => switchPerfilSubtab('perfil-entreno'), 100);
    return;
  }
  if (window.UI && window.UI.switchTab) window.UI.switchTab(tab);
  else if (window.switchTab) window.switchTab(tab);
  document.querySelectorAll('.bottom-nav-item').forEach(b => b.classList.remove('active'));
  const navBtn = document.querySelector(`.bottom-nav-item[data-tab="${tab}"]`);
  if (navBtn) navBtn.classList.add('active');
}
...
window.cargarMasMensajesAdmin = () => Admin.cargarMasMensajesAdmin();
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setTimeout(() => {
    document.querySelectorAll('.password-toggle').forEach(button => {
      if (!button.getAttribute('onclick')) {
        ...
        if (input && input.id) button.setAttribute('onclick', `togglePassword('${input.id}', this)`);
      }
    });
  }, 500);
  if(!localStorage.getItem('ri5_info_visto')) {
    setTimeout(() => { const banner = document.getElementById('infoBanner'); if (banner) banner.style.display = 'block'; }, 1000);
  }
  ...
  if (ageInput) {
    ...
    ageInput.addEventListener("input", () => { if(AppState && AppState.camposTocados.age) UI.validarCampo('age'); UI.validarTodo(); });
  }
  if (timeInput) {
    timeInput.addEventListener("blur", () => UI.marcarCampoTocado('time'));
    timeInput.addEventListener("input", () => { if(AppState && AppState.camposTocados.time) UI.validarCampo('time'); UI.validarTodo(); });
  }
  UI.validarTodo();
  ...
  if (savedTheme === 'light' || savedTheme === 'dark') document.body.classList.add(`manual-${savedTheme}`);
  ...
  window.addEventListener('online', () => { Storage.procesarCalculosPendientes(); });
  document.addEventListener('click', function(e) {
    let target = e.target;
    let premiumElement = null;
    if (target.innerText === 'PREMIUM') premiumElement = target;
    else if (target.parentElement && target.parentElement.innerText === 'PREMIUM') premiumElement = target.parentElement;
    else if (target.closest && target.closest('[class*="premium"]')) premiumElement = target.closest('[class*="premium"]');
    if (premiumElement) {
      ...
      if (perfilTab && perfilTab.classList.contains('active')) {
        ...
        Utils.confirm('Contactar con Administrador', '¿Quieres contactar con el administrador para obtener Premium?\n\nAceptar te llevará a Instagram (@coachjoaquinpeinado)')
          .then((resultado) => { if (resultado) window.open('https://www.instagram.com/coachjoaquinpeinado', '_blank'); }).catch(() => {});
      }
    }
  });
  const savedUid = localStorage.getItem('ri5_current_user');
  if (!savedUid) { document.getElementById("loginPage").style.display = "flex"; document.getElementById("mainContent").style.display = "none"; }
  else { Utils.showLoading(); setTimeout(() => { const overlay = document.getElementById('loadingOverlay'); if (overlay && overlay.classList.contains('active')) console.warn('⚠️ El overlay de carga lleva más de 15s activo.'); }, 15000); }
  if (window.Training && Training._loadFromLocalStorage) {
    ...
    if (savedCalc && AppState && !AppState.zonasCalculadas) {
      ...
      if (document.getElementById('tab-entreno') && document.getElementById('tab-entreno').classList.contains('active')) Training.mostrarResultados(savedCalc);
      else console.log('📦 Cálculo restaurado en segundo plano desde localStorage');
    }
  }
...
window.contactarAdmin = () => { window.open('https://www.instagram.com/coachjoaquinpeinado', '_blank'); };
window.cerrarInfoBanner = () => { const noMostrar = document.getElementById('infoBannerDontShow')?.checked; if (noMostrar) localStorage.setItem('ri5_info_visto', 'true'); document.getElementById('infoBanner').style.display = 'none'; document.getElementById('infoBannerOverlay').style.display = 'none'; };
window.cerrarWelcomeModal = () => { document.getElementById('welcomeOverlay')?.classList.remove('active'); document.getElementById('welcomeModal')?.classList.remove('active'); };
...
if (typeof PlanGenerator !== 'undefined') { window.cambiarTrimestre = async (delta) => { await PlanGenerator.cambiarTrimestre(delta); }; }
else { window.cambiarTrimestre = async (delta) => { console.warn('PlanGenerator no disponible aún'); Utils.showToast('Cargando planificador...', 'info'); }; }

// ==================== EVENTOS PARA RE-INICIALIZAR DASHBOARD AL CAMBIAR DE PESTAÑA ====================
document.addEventListener('DOMContentLoaded', function() {
  const tabInicio = document.getElementById('tab-inicio');
  if (tabInicio) {
    new MutationObserver(() => {
      if (tabInicio.classList.contains('active') && AppState.currentUserId) {
        cargarDashboard();
      }
    }).observe(tabInicio, { attributes: true, attributeFilter: ['class'] });
  }
});

// Al cambiar de usuario, recargar dashboard
...
window.restaurarDashboardDesdeCache = restaurarDashboardDesdeCache;
window.guardarDashboardEnCache = guardarDashboardEnCache;
window.abrirModalDetalleSesion = abrirModalDetalleSesion;
window.switchDashboardChart = switchDashboardChart;
window.changeDashboardConsejo = changeDashboardConsejo;
window.initDashboardConsejo = initDashboardConsejo;