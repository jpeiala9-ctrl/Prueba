...
const AppState = {
  ...
  // Aplica/quita el resplandor dorado especial (clase CSS "premium") sobre
  // el logo "RI5" (#appLogoText) según si el usuario actual tiene premium
  // activo y no caducado. Mientras el premium está activo, este color
  // dorado con !important pisa el color de nivel que pinta la pantalla de
  // dashboard (Gamification.getColorByLevel) — es intencional, es el
  // detalle que se quiere conservar. Se llama al iniciar sesión
  // (setCurrentUser) y también cuando se detecta en segundo plano que el
  // premium acaba de caducar, para refrescarlo al instante.
...
const Admin = {
  ...
  // --- Selección de usuarios para enviar una sesión (checkboxes en la lista) ---
  usuariosSeleccionadosSesion: new Set(),

  toggleSeleccionUsuarioSesion(uid, checked) {
    if (checked) this.usuariosSeleccionadosSesion.add(uid);
    else this.usuariosSeleccionadosSesion.delete(uid);
    this._actualizarContadorSeleccionSesion();
  },

  _actualizarContadorSeleccionSesion() {
    const n = this.usuariosSeleccionadosSesion.size;
    const btn = document.getElementById('enviarSesionBtn');
    if (btn) {
      btn.textContent = n > 0 ? `ENVIAR SESIÓN A ${n} USUARIO${n === 1 ? '' : 'S'}` : 'ENVIAR SESIÓN (selecciona usuarios abajo)';
      btn.disabled = n === 0;
    }
  },

  async enviarSesionAUsuariosSeleccionados() {
    const uids = Array.from(this.usuariosSeleccionadosSesion);
    if (uids.length === 0) {
      Utils.showToast('Selecciona al menos un usuario', 'info');
      return;
    }
    const nombre = document.getElementById('sesionPlantillaNombre')?.value.trim();
    const km = parseFloat(document.getElementById('sesionPlantillaKm')?.value);
    const zona = document.getElementById('sesionPlantillaZona')?.value;
    const descripcion = document.getElementById('sesionPlantillaDescripcion')?.value.trim();
    const fecha = document.getElementById('sesionPlantillaFecha')?.value;

    if (!nombre || !descripcion || !fecha || isNaN(km) || km <= 0) {
      Utils.showToast('Rellena nombre, km, descripción y fecha', 'error');
      return;
    }
    ...
    try {
      await Storage.enviarPropuestasSesion(uids, { nombre, km, zona, descripcion }, fecha);
      Utils.showToast(`Sesión enviada a ${uids.length} usuario${uids.length === 1 ? '' : 's'}`, 'success');
      this.usuariosSeleccionadosSesion.clear();
      document.querySelectorAll('.usuario-checkbox-sesion').forEach(cb => cb.checked = false);
      document.getElementById('sesionPlantillaNombre').value = '';
      document.getElementById('sesionPlantillaKm').value = '';
      document.getElementById('sesionPlantillaDescripcion').value = '';
      this._actualizarContadorSeleccionSesion();
    } catch (e) {
      console.error('Error enviando propuestas de sesión:', e);
      Utils.showToast('Error al enviar la sesión', 'error');
    } finally {
      if (btn) btn.disabled = this.usuariosSeleccionadosSesion.size === 0;
    }
  },
  ...
  renderUsersList(usersData) {
      ...
      html += `
        <div class="usuario-item ${u.esNuevo ? 'nuevo' : ''}" data-uid="${u.uid}">
          <div class="usuario-header" data-uid="${u.uid}">
            <input type="checkbox" class="usuario-checkbox-sesion" data-uid="${u.uid}" onclick="event.stopPropagation();" title="Seleccionar para enviarle una sesión">
    ...
    container.querySelectorAll('.usuario-checkbox-sesion').forEach(cb => {
      if (this.usuariosSeleccionadosSesion.has(cb.dataset.uid)) cb.checked = true;
      cb.addEventListener('change', () => {
        this.toggleSeleccionUsuarioSesion(cb.dataset.uid, cb.checked);
      });
    });
...
const UI = {
    ...
    try {
      ...
      if(tab === 'perfil') {
        if (window.Profile) {
          ...
          //
          // Las miniaturas de "Mis últimos entrenamientos" ya no necesitan
          // ningún reintento aquí: son un SVG estático leído de
          // localStorage (generado una sola vez al guardar la sesión, en
          // gps-tracker.js), así que se pintan de golpe con el resto del
          // HTML, sin depender de que Leaflet cargue ni de que la pestaña
          // ya sea visible.
      ...
      if(tab === 'admin' && AppState && AppState.isAdmin) {
        ...
        const fechaEl = document.getElementById('sesionPlantillaFecha');
  ...
  renderMessageList(container, mensajes, tipo, onToggleCallback, otherName = 'Soporte') {
    if (!container) return;
  ...
  async cargarMensajesRecibidos() {
    ...
    this.renderMessageList(container, recibidos, 'recibido', null, adminNombre);
    this.actualizarBadgeMensajes();
  },
  ...
  async cargarMensajesEnviados() {
    ...
    this.renderMessageList(container, enviados, 'enviado', null, adminNombre);
  },
...
// ==================== SESIONES ASIGNADAS POR EL ENTRENADOR ====================
// Gestiona, del lado del usuario que las recibe, las propuestas de sesión
// que un admin le envía (ver Storage.enviarPropuestasSesion /
// responderPropuestaSesion). Se muestran de una en una, en un modal con
// Aceptar/Rechazar; al responder la primera se comprueba si queda otra.
const SesionAsignada = {
  cola: [],
  _comprobando: false,

  async comprobarPendientes() {
    if (this._comprobando) return;
    this._comprobando = true;
    try {
      const uid = AppState.currentUserId;
      if (!uid) return;
      this.cola = await Storage.getPropuestasSesionPendientes(uid);
      if (this.cola.length > 0) this._mostrarSiguiente();
    } catch (e) {
      console.warn('Error comprobando sesiones asignadas pendientes:', e);
    } finally {
  ...
  _mostrarSiguiente() {
    const prop = this.cola[0];
    if (!prop) return;
    const overlay = document.getElementById('propuestaSesionOverlay');
    const modal = document.getElementById('propuestaSesionModal');
    if (!overlay || !modal) return;

    const fechaFmt = new Date(prop.fecha + 'T00:00:00').toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' });
    document.getElementById('propuestaSesionEntrenador').textContent = prop.deNombre || 'Tu entrenador';
    document.getElementById('propuestaSesionFecha').textContent = fechaFmt;
    document.getElementById('propuestaSesionNombre').textContent = prop.sesion.nombre;
    document.getElementById('propuestaSesionKm').textContent = `${prop.sesion.km} km`;
    document.getElementById('propuestaSesionZona').textContent = prop.sesion.zona || '—';
    document.getElementById('propuestaSesionDescripcion').textContent = prop.sesion.descripcion;

    overlay.style.display = 'block';
    modal.style.display = 'block';
  },

  async responder(aceptar) {
    const prop = this.cola[0];
    if (!prop) return;
    const overlay = document.getElementById('propuestaSesionOverlay');
    const modal = document.getElementById('propuestaSesionModal');
    ...
    try {
      const res = await Storage.responderPropuestaSesion(prop.id, aceptar);
      if (!aceptar) {
        Utils.showToast('Sesión rechazada', 'info');
      } else if (res.ok && res.aceptada) {
        Utils.showToast('¡Sesión actualizada en tu plan!', 'success');
        if (window.PlanGenerator && typeof PlanGenerator.mostrarCalendario === 'function') {
          PlanGenerator.mostrarCalendario().catch(() => {});
        }
      } else if (res.error === 'sin_plan') {
        Utils.showToast('No tienes un plan activo para aplicar esta sesión', 'error');
      } else if (res.error === 'fuera_de_rango') {
        Utils.showToast('La fecha de esta sesión no está dentro de tu plan actual', 'error');
      } else {
        Utils.showToast('No se pudo aplicar la sesión', 'error');
      }
    } catch (e) {
      console.error('Error respondiendo a la propuesta de sesión:', e);
      Utils.showToast('Error al responder', 'error');
    }

    this.cola.shift();
    if (this.cola.length > 0) {
      this._mostrarSiguiente();
    } else {
      overlay?.classList.remove('active');
      modal?.classList.remove('active');
    }
...
window.SesionAsignada = SesionAsignada;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.addEventListener('click', function(e) {
    // Los administradores nunca necesitan este aviso: ni en su propio
    // perfil, ni (sobre todo) cuando están gestionando el premium de OTRO
    // usuario desde el panel de admin -- ese panel usa clases CSS
    // (premium-field, premium-buttons) que antes coincidían por accidente
    // con el selector genérico de abajo, así que un clic en el propio
    // desplegable o en "GUARDAR" del panel "GESTIONAR PREMIUM" disparaba
    // este aviso de "contacta con el administrador" en vez de dejar pasar
    // la acción real del admin.
    if (AppState.isAdmin) return;
    if (e.target.closest && e.target.closest('#premiumManageModal, #premiumManageOverlay, #tab-admin')) return;
...
window.aceptarPropuestaSesion = () => SesionAsignada.responder(true);
window.rechazarPropuestaSesion = () => SesionAsignada.responder(false);
window.enviarSesionAUsuariosSeleccionados = () => Admin.enviarSesionAUsuariosSeleccionados();
