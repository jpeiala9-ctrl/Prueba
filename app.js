...
// VERSIÓN: 4.43 - Carga instantánea, siempre activa la pestaña Inicio si no hay estado.
// ====================
...
const AppState = {
  ...
  precargarDatos() {
    ...
    if (window.Profile) {
      Profile.cargarPerfil(false).catch(e => console.warn('Error precargando perfil:', e));
    }
    if (window.Friends) {
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      // === NUEVO: si la pestaña es 'inicio', asegurar que el dashboard se cargue ===
      if (tab === 'inicio') {
        const tabContent = document.getElementById('tab-inicio');
        // Si el contenido está vacío o no tiene el hero del dashboard, cargarlo
        if (tabContent && !tabContent.querySelector('#dashboardHero')) {
          if (typeof cargarDashboard === 'function') {
            await cargarDashboard();
          } else {
            console.warn('cargarDashboard no está definida aún, se reintentará en 500ms');
            // Reintentar una vez después de que los scripts estén cargados
            setTimeout(() => {
              if (typeof cargarDashboard === 'function') {
                cargarDashboard();
              }
  ...
  async restaurarEstado() {
    ...
    if (!estadoStr) {
      // No hay estado guardado → activar Inicio por defecto
      console.log('🔄 No hay estado guardado, activando Inicio');
      await this.switchTab('inicio');
      return;
    }
    try {
      ...
      console.warn('Error restaurando estado, activando Inicio', e);
      await this.switchTab('inicio');
    }
  },
