    ...
    for (const user of usuarios) {
      ...
      html += `
        <div class="usuario-soporte-item" data-uid="${user.uid}" style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:background 0.2s;">
          ...
          ${badgeNoLeidos}
          <button type="button" class="borrar-conversacion-soporte" data-uid="${user.uid}" title="Eliminar conversación" style="flex-shrink:0;border:none;background:var(--bg-primary);color:var(--zone-5);width:32px;height:32px;border-radius:10px;font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center;">🗑️</button>
        </div>
      `;
    }
