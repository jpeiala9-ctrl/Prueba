...
const Admin = {
  ...
  _renderizarListaSoporte(usuarios, container) {
    ...
    for (const user of usuarios) {
      ...
      html += `
        <div class="usuario-soporte-item" data-uid="${user.uid}" style="position:relative;display:flex;align-items:center;gap:12px;padding:12px 40px 14px 12px;background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:background 0.2s;">
          ${avatar}
          ...
          ${badgeNoLeidos}
          <button type="button" class="borrar-conversacion-soporte" data-uid="${user.uid}" title="Eliminar conversación" style="position:absolute;right:10px;bottom:6px;border:none;background:transparent;color:var(--zone-5);padding:6px;line-height:1;font-size:17px;cursor:pointer;display:flex;align-items:center;justify-content:center;">🗑️</button>
        </div>
      `;
    }
