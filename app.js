...
const Admin = {
  ...
  renderUsersList(usersData) {
        ...
        <div class="usuario-item ${u.esNuevo ? 'nuevo' : ''}" data-uid="${u.uid}">
          ...
          <div class="usuario-detalle">
            ...
            <div class="usuario-acciones">
              ...
              <button class="entrenador-usuario-btn" data-uid="${u.uid}" data-username="${Utils.escapeHTML(u.username)}" data-entrenador="${u.user.isTrainer === true}" style="${u.user.isTrainer === true ? 'background:var(--gold); color:#000; border-color:var(--gold);' : ''}">${u.user.isTrainer === true ? '🎯 ENTRENADOR' : '🎯 HACER ENTRENADOR'}</button>
    ...
    container.querySelectorAll('.entrenador-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const uid = btn.dataset.uid;
        const username = btn.dataset.username;
        const esEntrenadorActual = btn.dataset.entrenador === 'true';
        Admin.toggleEntrenador(uid, username, esEntrenadorActual);
      });
    });
  ...
  async toggleEntrenador(uid, username, esEntrenadorActual) {
    ...
    const confirmado = await Utils.confirm(
      ...
      nuevoEstado
        ? `¿Convertir a "${username}" en entrenador? Tendrá exactamente los mismos derechos que un corredor normal, más acceso al generador de sesiones para poder enviárselas a otros usuarios.`
        : `¿Quitar el rol de entrenador a "${username}"? Perderá el acceso al generador de sesiones.`
    );
    if (!confirmado) return;
    ...
    try {
      ...
      Utils.showToast(nuevoEstado ? `✅ ${username} ahora es entrenador` : `✅ ${username} ya no es entrenador`, 'success');
      this.cargarUsuarios(true);
    } catch (error) {
      console.error('Error actualizando isTrainer:', error);
    ...
    try {
      ...
      if(tab === 'perfil') {
        ...
        if (AppState.isAdmin) {
          ...
          // El entrenador solo tiene acceso al generador de sesiones dentro
          // del panel (no al control ni al soporte, que son solo de admin).
          setTimeout(() => {
            if (document.getElementById('soporte-admin-panel')) {
              Admin.cambiarSubtab('generar');
            }
          }, 100);
        }
      }
