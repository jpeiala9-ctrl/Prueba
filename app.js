...
const Admin = {
  ...
  // 🔥 FIX (ronda de optimización Firestore): caché en memoria de la
  // PRIMERA página de usuarios (sin búsqueda, sin paginar más allá) --
  // { data: [...20 docs...], lastDoc, hasMore, timestamp }. Antes, cada vez
  // que el admin entraba a Soporte→Admin (o volvía a esa subpestaña), se
  // pedían 20 lecturas nuevas de 'users' aunque no hubiera pasado nada
  // desde la última vez. TTL de 2.5 min; se invalida explícitamente tras
  // crear/eliminar/modificar un usuario o al pulsar "Recargar" (ver
  // invalidarCacheUsuarios more abajo). Los filtros (premium/fecha/orden)
  // se aplican en memoria sobre estos mismos datos, así que no necesitan
  // invalidar la caché ni forman parte de su clave.
  _usuariosCache: null,
  _USUARIOS_CACHE_TTL_MS: 150000,
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (searchTerm) {
        ...
        // 🔥 FIX: si es la primera página (reset, sin startAfter) y sin
        // término de búsqueda, se reutiliza la caché en memoria si sigue
        // viva (TTL 2.5 min) en vez de volver a pedir 20 documentos a
        // Firestore. La paginación posterior (cargarMasUsuarios, reset=false)
        // sigue yendo siempre a Firestore -- solo la primera página cachea.
        const usarCache = reset && !this.usersPagination.lastDoc &&
          this._usuariosCache && (Date.now() - this._usuariosCache.timestamp) < this._USUARIOS_CACHE_TTL_MS;

        if (usarCache) {
          usersData = this._usuariosCache.data;
          hasMore = this._usuariosCache.hasMore;
          this.usersPagination.lastDoc = this._usuariosCache.lastDoc;
          ...
          // Solo se cachea la PRIMERA página (reset=true), que es la que
          // se repite cada vez que se reabre la pestaña de Admin.
          if (reset) {
            this._usuariosCache = {
              data: usersData,
              lastDoc: this.usersPagination.lastDoc,
              hasMore: this.usersPagination.hasMore,
              timestamp: Date.now()
            };
          }
