...
// VERSIÓN: 4.66 - CACHÉ EN VIVO (sustituye a los TTL de la ronda anterior
//                 de optimización Firestore): (1) setCurrentUser() arranca
//                 ahora Gamification.iniciarListener(uid) y
//                 Sponsors.iniciarListener() junto al resto de listeners
//                 de la sesión (ver gamification.js v5.22 y sponsors.js
//                 v1.14); detenerListeners() los para. (2) La lista de
//                 usuarios de Soporte→Admin (Admin._usuariosCache, con TTL
//                 de 2,5min) se sustituye por Admin._usuariosEnVivo,
//                 alimentada por un listener onSnapshot que arranca
//                 Admin.iniciarListenerUsuariosAdmin() la primera vez que
//                 el admin abre esa pestaña en la sesión (no en el
//                 arranque global, para no costarle nada a quien nunca
//                 entra ahí); se para en detenerListeners() junto a los
//                 demás. Admin.invalidarCacheUsuarios() queda como no-op
//                 por compatibilidad con las llamadas ya existentes tras
//                 crear/eliminar/modificar un usuario. Motivo del cambio:
//                 con TTL, cambiar el nivel/insignias de alguien, activar/
//                 desactivar una tienda, o ver la lista de usuarios al día
//                 podía tardar hasta 30s/8min/2,5min en reflejarse; con
//                 listeners es instantáneo Y cuesta menos lecturas (1
//                 lectura inicial + solo cambios reales, en vez de 1
//                 lectura cada vez que expiraba el TTL con la pantalla
//                 abierta). NOTA: se revisó el listener de
//                 unsubscribeMisDatos (users/{uid}) para el mismo motivo
//                 con la lista de amigos -- ya llamaba a
//                 Friends.cargarListaAmigos(true) cuando cambia friendIds
//                 estando esa pestaña abierta, así que no hizo falta
//                 tocar nada ahí.
...
const AppState = {
  ...
  likesNuevosCount: 0,
  // 🔥 FIX (ronda de optimización Firestore, punto [11]): arrays en vivo de
  // solicitudes de amistad pendientes, alimentados por los propios
  // listeners onSnapshot de abajo (unsubscribeFriendRequests para
  // recibidas, unsubscribeSolicitudesEnviadas para enviadas) -- antes esos
  // listeners solo actualizaban un CONTADOR o disparaban un toast, y
  // Friends.obtenerSolicitudesPendientes() volvía a consultar Firestore
  // con un .get() cada vez que se llamaba (varias veces por sesión:
  // cargarSolicitudes, badges, modales...). null significa "aún sin datos
  // en vivo" (listener todavía no ha disparado su primer snapshot) --
  // Friends.obtenerSolicitudesPendientes() solo hace .get() explícito en
  // ese caso.
  solicitudesRecibidasCache: null,
  solicitudesEnviadasCache: null,
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (soporteTab) {
      ...
      if (this.isAdmin) {
        ...
        soporteTab.innerHTML = `<span style="color:${colorAdmin};">Administración</span>`;
      } else {
        soporteTab.innerHTML = '💬 Soporte';
      }
    }
    ...
    // 🔥 "Mis alumnos" (invitar/quitar) es solo para un entrenador que NO
    // es admin: el admin ya puede enviar sesiones a cualquier usuario sin
    ...
    // abra la pestaña "Mis alumnos" en esta sesión del navegador esté al
    // instante también, en vez de notarse una espera la primera vez.
    // cargarPanelAlumnos() sin forceRefresh ya comprueba sola si hace
    // falta pedir algo (usa sessionStorage si ya había).
    ...
    if (uid) {
      ...
      // 🔥 CAMBIO v4.66 (caché en vivo): arranca los listeners globales de
      // gamificación y tiendas patrocinadoras para que getData()/getActivos()
      // sean instantáneos y en tiempo real en vez de depender de un TTL (ver
      // gamification.js v5.22 y sponsors.js v1.14). Se paran en
      // detenerListeners(), más abajo.
      if (window.Gamification && Gamification.iniciarListener) Gamification.iniciarListener(uid);
      if (window.Sponsors && Sponsors.iniciarListener) Sponsors.iniciarListener();
  ...
  async precargarDatos() {
    ...
    // 🔥 FIX (ronda de optimización Firestore): Admin.precargarEstadisticas()
    // y Sponsors.precargarAdminLista() dejan de estar dentro del
    // Promise.all crítico (preloadCriticos) que bloquea el splash -- solo
    // afectan a la propia cuenta de admin cuando entra en Soporte→Admin o
    // en Administración→Tienda, pantallas secundarias que no deben
    // retrasar la aparición del resto de la app (muro/banner). Se lanzan
    // en segundo plano sin await; el resto del código ya las trata como
    // caché (getData/getActivos con su propio TTL), así que si el admin
    // llega a esas pantallas antes de que terminen, simplemente ve el
    // "Cargando..."/"0" un instante, como pasaba antes de v4.63.
    if (this.isAdmin) {
      if (window.Admin && Admin.precargarEstadisticas) {
        Admin.precargarEstadisticas().catch(e => console.warn('Error precargando estadísticas de admin:', e));
      }
      if (window.Sponsors && Sponsors.precargarAdminLista) {
        Sponsors.precargarAdminLista().catch(e => console.warn('Error precargando el panel admin de tiendas:', e));
      }
    }
  ...
  async incrementarCalculo() {
    ...
    if (this.calculosMes >= limite) {
      Utils.showToast('⚠️ Límite de 3 cálculos mensuales alcanzado. Actualiza a premium para más.', 'warning');
      return false;
    }
  ...
  iniciarListeners() {
    ...
    this.unsubscribeMisDatos = firebaseServices.db
      ...
      .onSnapshot(async (doc) => {
        ...
        const nuevoIsPremium = nuevaData.premium || false;
    ...
    this.unsubscribeSolicitudesEnviadas = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        // 🔥 FIX: se mantiene aquí, en cada snapshot, la caché en vivo de
        // solicitudes ENVIADAS pendientes (ver solicitudesEnviadasCache
        // arriba) -- la query no filtra por status porque también hace
        // falta ver las 'rejected' para el toast de abajo, así que se
        // filtra aquí en memoria.
        this.solicitudesEnviadasCache = snapshot.docs
    ...
    this.unsubscribeFriendRequests = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        // 🔥 FIX: caché en vivo de solicitudes RECIBIDAS pendientes (ver
        // solicitudesRecibidasCache arriba) -- la query ya filtra por
        // status=='pending', así que aquí no hace falta filtrar nada más.
        this.solicitudesRecibidasCache = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    this.unsubscribeLikesPropios = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        ...
        this.entriesConLikesNuevos = conNuevos.map(e => e.id);
  ...
  detenerListeners() {
    ...
    // 🔥 FIX: al cerrar sesión, las cachés en vivo de solicitudes vuelven a
    // null -- si no, el próximo login (con otro uid, o incluso el mismo)
    // podría arrancar leyendo datos de la sesión anterior antes de que el
    // nuevo listener mande su primer snapshot.
    this.solicitudesRecibidasCache = null;
    this.solicitudesEnviadasCache = null;
    ...
    // 🔥 CAMBIO v4.66 (caché en vivo): cierra también los listeners
    // globales de gamificación y tiendas patrocinadoras (ver setCurrentUser
    // más arriba, donde se arrancan).
    if (window.Gamification && Gamification.detenerListener) {
      Gamification.detenerListener();
    }
    if (window.Sponsors && Sponsors.detenerListener) {
      Sponsors.detenerListener();
    }
    // 🔥 CAMBIO v4.66 (caché en vivo de la lista de usuarios admin): cierra
    // el listener de Admin.cargarUsuarios si el admin llegó a abrir
    // Soporte→Admin en esta sesión.
    this.detenerListenerUsuariosAdmin();
  },
...
const Admin = {
  ...
  // 🔥 CAMBIO v4.66 (caché en vivo -- sustituye al TTL de 2,5min de la
  // v4.65): caché de la PRIMERA página de usuarios (sin búsqueda, sin
  // paginar más allá) -- { data: [...20 docs...], lastDoc, hasMore},
  // alimentada por un listener onSnapshot que arranca
  // iniciarListenerUsuariosAdmin() la primera vez que el admin entra a
  // Soporte→Admin en la sesión (no en el arranque global, para no
  // costarle nada a los usuarios que nunca abren esa pantalla). Antes
  // (TTL de 2,5 min), cambiar el estado premium o borrar un usuario desde
  // OTRO dispositivo/pestaña de admin podía tardar hasta 2,5 minutos en
  // reflejarse aquí; con el listener es instantáneo. Los filtros
  // (premium/fecha/orden) se aplican en memoria sobre estos mismos datos,
  // así que no necesitan tocar el listener ni forman parte de su clave.
  _usuariosEnVivo: null,
  _unsubscribeAdminUsuarios: null,

  // Arranca (de forma idempotente) el listener en vivo de la primera
  // página de usuarios (orderBy username_lowercase, limit 20).
  iniciarListenerUsuariosAdmin() {
    if (this._unsubscribeAdminUsuarios) return;
    this._unsubscribeAdminUsuarios = firebaseServices.db.collection('users')
      ...
      .onSnapshot(snap => {
        ...
        this._usuariosEnVivo = {
          data,
          lastDoc: snap.docs[snap.docs.length - 1] || null,
          hasMore: snap.docs.length === 20
        };
      }, error => {
        console.warn('Error en el listener en vivo de la lista de usuarios admin:', error);
      });
  },

  // Para el listener en vivo de la lista de usuarios admin. Se llama
  // desde AppState.detenerListeners (logout).
  detenerListenerUsuariosAdmin() {
    if (this._unsubscribeAdminUsuarios) {
      this._unsubscribeAdminUsuarios();
      this._unsubscribeAdminUsuarios = null;
    }
    this._usuariosEnVivo = null;
  },

  // Ya no invalida nada (el listener recibe los cambios solo, en tiempo
  // real). Se deja como no-op para no tener que tocar las llamadas
  // existentes tras crear/eliminar/modificar un usuario o pulsar
  // "Recargar".
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (searchTerm) {
        ...
        // 🔥 CAMBIO v4.66 (caché en vivo -- sustituye al TTL de 2,5min de la
        // v4.65): si es la primera página (reset, sin startAfter) y sin
        // término de búsqueda, se reutiliza la caché en vivo si el listener
        // ya tiene datos, en vez de volver a pedir 20 documentos a
        // Firestore. Si es la primera vez que se entra a Soporte→Admin en
        // esta sesión, se arranca el listener de paso. La paginación
        // posterior (cargarMasUsuarios, reset=false) sigue yendo siempre a
        // Firestore -- solo la primera página vive en el listener.
        const usarCache = reset && !this.usersPagination.lastDoc && this._usuariosEnVivo;

        if (usarCache) {
          usersData = this._usuariosEnVivo.data;
          hasMore = this._usuariosEnVivo.hasMore;
          this.usersPagination.lastDoc = this._usuariosEnVivo.lastDoc;
          ...
          // Solo la primera página (reset=true) arranca/usa el listener en
          // vivo, que es la que se repite cada vez que se reabre la
          // pestaña de Admin.
          if (reset) {
            this.iniciarListenerUsuariosAdmin();
          }
        }
      }
  ...
  renderUsersList(usersData) {
        ...
        <div class="usuario-item ${u.esNuevo ? 'nuevo' : ''}" data-uid="${u.uid}">
          ...
          <div class="usuario-detalle">
            ...
            <div class="usuario-acciones">
              ...
              <button class="entrenador-usuario-btn" data-uid="${u.uid}" data-username="${Utils.escapeHTML(u.username)}" data-entrenador="${!!u.user.isTrainer}" style="${u.user.isTrainer ? 'border-color:var(--gold); color:var(--gold);' : ''}">${u.user.isTrainer ? '🎯 ENTRENADOR' : 'HACER ENTRENADOR'}</button>
    ...
    container.querySelectorAll('.entrenador-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        Admin.toggleEntrenador(uid, username, esEntrenador, btn);
      });
    });
  ...
  async eliminarUsuario(uid, username) {
    ...
    try {
      ...
      this.invalidarCacheUsuarios(); // 🔥 FIX: la lista cacheada ya no refleja al usuario eliminado
      this.cargarUsuarios(true);
  ...
  async guardarPremium() {
    ...
    try {
      ...
      this.invalidarCacheUsuarios(); // 🔥 FIX: el estado premium cacheado ya no es válido
      this.cargarUsuarios(true);
      this.cargarEstadisticas();
    } catch (error) { 
  ...
  async toggleEntrenador(uid, username, esEntrenadorActual, btnEl = null) {
    const nuevoEstado = !esEntrenadorActual;
    const confirmado = await Utils.confirm(
      nuevoEstado ? 'Hacer entrenador' : 'Quitar entrenador',
      nuevoEstado
        ? `¿Dar a ${Utils.escapeHTML(username)} acceso al generador de sesiones para crear y enviar entrenos a otros usuarios?`
        : `¿Quitarle a ${Utils.escapeHTML(username)} el acceso al generador de sesiones?`
    );
    if (!confirmado) return;

    Utils.showLoading();
    try {
      await firebaseServices.db.collection('users').doc(uid).update({ isTrainer: nuevoEstado });
    } catch (error) {
    ...
    Utils.showToast(nuevoEstado ? '✅ Ahora es entrenador' : 'Entrenador desactivado', 'success');

    // 🔥 A petición del usuario: hacer/quitar entrenador a alguien NO debe
    // cambiar nada más en la pantalla del admin -- antes se llamaba a
    // cargarUsuarios(true), que recargaba la lista entera desde Firestore
    // (perdiendo scroll, búsqueda y paginación) solo para actualizar una
    // etiqueta. Ahora se actualiza en el sitio únicamente el botón de esa
    // fila, sin tocar el resto de la lista ni volver a leer Firestore.
    const btn = btnEl || document.querySelector(`.entrenador-usuario-btn[data-uid="${uid}"]`);
    if (btn) {
      btn.dataset.entrenador = nuevoEstado ? 'true' : 'false';
      btn.textContent = nuevoEstado ? '🎯 ENTRENADOR' : 'HACER ENTRENADOR';
  ...
  _renderizarListaSoporte(usuarios, container) {
    ...
    for (const user of usuarios) {
      ...
      const badgeNoLeidos = tieneNoLeidos ? `<span style="background:rgba(0,0,0,0.75);color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;white-space:nowrap;">${user.noLeidos} nuevo${user.noLeidos > 1 ? 's' : ''}</span>` : '';

      html += `
        <div class="usuario-soporte-item" data-uid="${user.uid}" style="display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-rows:auto auto auto;column-gap:8px;row-gap:4px;padding:12px;background:${cardBg};border:1px solid ${cardBorder};border-radius:12px;margin-bottom:8px;cursor:pointer;transition:background 0.2s, border-color 0.2s;">
          <div style="grid-column:1;grid-row:1 / 4;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;">
            ${avatar}
            <span style="font-weight:700;font-size:14px;color:${colorNombre};text-align:center;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${nombre}</span>
          </div>
          <span style="grid-column:3;grid-row:1;font-size:11px;color:${colorSecundario};align-self:start;justify-self:end;white-space:nowrap;">${fecha}</span>
          <div style="grid-column:3;grid-row:2;align-self:center;justify-self:end;">${badgeNoLeidos}</div>
          <div style="grid-column:2;grid-row:3;align-self:end;justify-self:center;font-size:13px;color:${colorSecundario};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center;max-width:100%;">${textoPreview}</div>
  ...
  async abrirSelectorDestinatarios() {
    ...
    overlay.style.cssText = `position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); backdrop-filter:blur(4px); z-index:60000; display:flex; align-items:center; justify-content:center; opacity:0; transition:opacity 0.2s ease;`;
    overlay.addEventListener('click', (e) => { if (e.target === overlay) this._cerrarSelectorDestinatarios(); });

    const modal = document.createElement('div');
    modal.id = modalId + 'Modal';
    modal.style.cssText = `background:var(--bg-secondary); border-radius:20px; width:92%; max-width:700px; max-height:88vh; display:flex; flex-direction:column; overflow:hidden; box-shadow:0 20px 40px rgba(0,0,0,0.5); border:1px solid var(--border-color); font-family:'Courier New',monospace; opacity:0; transition:opacity 0.2s ease;`;
    ...
    modal.innerHTML = `
      <div style="padding:16px 44px; background:var(--bg-primary); border-bottom:1px solid var(--border-color); text-align:center;">
        <span style="font-size:16px; font-weight:bold; letter-spacing:1px; color:var(--text-primary);">👥 ELIGE USUARIOS</span>
        <div style="margin-top:6px;">
          <span id="broadcastDestContador" style="display:inline-block; font-size:11px; font-weight:bold; letter-spacing:0.5px; color:var(--gold); background:rgba(192,160,96,0.12); border:1px solid var(--gold); border-radius:20px; padding:3px 14px;">0 elegidos</span>
        </div>
      </div>
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        <div style="margin-bottom:16px;">
          <button onclick="Admin._abrirGestionGruposDestino()" style="display:block; width:100%; background:transparent; border:none; padding:4px 0 12px; margin:0; font-family:'Courier New',monospace; font-size:15px; font-weight:bold; letter-spacing:1px; color:var(--text-primary); text-align:center; cursor:pointer;">GESTIONAR GRUPOS</button>
        ...
        <input id="broadcastDestBuscar" placeholder="> BUSCAR USUARIO_" style="width:100%; margin-bottom:12px; text-align:center;">
...
const UI = {
  ...
  _renderizarTarjetaSoporteUsuario(datos, container) {
    ...
    const badgeNoLeidos = tieneNoLeidos ? `<span style="background:rgba(0,0,0,0.75);color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;white-space:nowrap;">${datos.noLeidos} nuevo${datos.noLeidos > 1 ? 's' : ''}</span>` : '';

    container.innerHTML = `
      <div id="tarjetaSoporteUsuario" style="display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-rows:auto auto auto;column-gap:8px;row-gap:4px;padding:12px;background:${cardBg};border:1px solid ${cardBorder};border-radius:12px;cursor:pointer;transition:background 0.2s, border-color 0.2s;">
        <div style="grid-column:1;grid-row:1 / 4;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;">
          ${avatar}
          <span style="font-weight:700;font-size:14px;color:${colorNombre};text-align:center;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${nombre}</span>
        </div>
        <span style="grid-column:3;grid-row:1;font-size:11px;color:${colorSecundario};align-self:start;justify-self:end;white-space:nowrap;">${datos.hayConversacion ? fecha : ''}</span>
        <div style="grid-column:3;grid-row:2;align-self:center;justify-self:end;">${badgeNoLeidos}</div>
        <div style="grid-column:2;grid-row:3;align-self:end;justify-self:center;font-size:13px;color:${colorSecundario};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center;max-width:100%;font-style:${datos.hayConversacion ? 'normal' : 'italic'};">${textoPreview}</div>
      </div>
    `;
  ...
  _mostrarModalChatSoporteUsuario(mensajes, adminInfo) {
    ...
    header.style.cssText = `
      ...
      background:var(--bg-primary); flex-shrink:0;
    `;
    header.innerHTML = `<span style="font-size:16px; font-weight:bold; color:var(--accent-yellow); text-align:center;">💬 Soporte con ${Utils.escapeHTML(adminInfo.username)}</span>`;
    modal.appendChild(header);
