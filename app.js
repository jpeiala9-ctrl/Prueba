...
// VERSIÓN: 4.42 - CORREGIDO: semana empieza el lunes y no se pierden sesiones por limit(20)
// ====================
...
const UI = {
  ...
  async restaurarEstado() {
      ...
      try {
        ...
        if (visible && planId) {
          ...
          if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
            ...
            // Asegura que la pestaña "Plan" quede realmente visible, no solo
            // los divs internos del calendario (antes se quedaba la
            // pestaña que estuviera activa por defecto, normalmente Inicio).
            if (typeof window.switchTab === 'function') {
              await window.switchTab('plan');
            } else {
              await this.switchTab('plan');
            }
            const calendario = document.getElementById("calendarioEntreno");
            const cuestionario = document.getElementById("cuestionarioEntreno");
    ...
    try {
      ...
      if (estado.activeTab) {
        // Mapa actualizado a las 4 pestañas reales de la barra inferior.
        // 'entreno', 'historial', 'soporte', 'amigos', 'muro', 'admin' ya
        // no son pestañas de nivel superior (viven como subpestañas dentro
        // de Plan/Comunidad/Perfil), así que si aparece un valor antiguo en
        // sessionStorage (de antes de este fix) se cae a 'inicio' en vez de
        // al 'entreno' muerto.
        const tabMap = { 'inicio': 'inicio', 'plan': 'plan', 'comunidad': 'comunidad', 'perfil': 'perfil' };
        const tab = tabMap[estado.activeTab] || 'inicio';
        // Se usa window.switchTab (si existe) en vez de this.switchTab
        // directamente: window.switchTab es el que también actualiza el
        // resaltado de la barra inferior (actualizarBarraActiva). Llamando
        // solo a this.switchTab, el contenido cambiaba pero la barra de
        // abajo se quedaba marcando "Inicio" aunque ya no lo fuera.
        if (typeof window.switchTab === 'function') {
