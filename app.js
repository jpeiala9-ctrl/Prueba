...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (soporteTab) {
      soporteTab.innerHTML = this.isAdmin ? '🛠️ Administración' : (this.isTrainer ? '🎯 Soporte' : '💬 Soporte');
    }
...
const Admin = {
  ...
  renderUsersList(usersData) {
    ...
    container.querySelectorAll('.entrenador-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        Admin.toggleEntrenador(uid, username, esEntrenador);
      });
    });
  ...
  async toggleEntrenador(uid, username, esEntrenadorActual) {
    ...
    try {
      ...
      console.error('Error refrescando la lista tras cambiar el rol de entrenador:', error);
    } finally {
      Utils.hideLoading();
    }
  },
