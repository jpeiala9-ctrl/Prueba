// ==================== app.js - VERSIÓN COMPLETA CON CACHÉ DE DATOS PARA RECARGA INSTANTÁNEA ====================
// VERSIÓN: 4.43 - Caché de datos del dashboard en localStorage
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    // Guardar en caché local para recarga instantánea
    if (uid && userData) {
  ...
  iniciarListeners() {
    ...
    this.unsubscribeMensajesSoporte = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        if (soporteTab && soporteTab.classList.contains('active')) {
          if (window.UI) {
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (this.filtrosActuales.sort === 'created') {
        ...
        usersDataProcessed.sort((a, b) => (b.user.premium ? 1 : 0) - (a.user.premium ? 1 : 0));
      } else {
  ...
  async enviarMensajeATodos() {
    ...
    try {
      ...
      for (const userDoc of users) {
        try {
          await Storage.enviarMensajeSoporte(AppState.currentUserId, userDoc.id, texto);
          enviados++;
        } catch (e) {
      ...
      if (estado.planVisible && estado.planId && AppState && AppState.currentUserId) {
        ...
        if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
          if (AppState) {
            ...
            AppState.planActualId = estado.planId;
            AppState.sesionesRealizadas = planExiste.sesionesRealizadas || {};
            AppState.feedbackSesiones = planExiste.feedback || {};
            AppState.planGeneradoActual = planExiste.params;
          }
          this.cargarPlanDesdeHistorial(estado.planId).catch(e => console.warn('Error cargando plan:', e));
        }
      }
    } catch (e) { console.warn('Error restaurando estado', e); }

    // === Inyectar caché del dashboard si la pestaña de inicio está vacía ===
    const activeTab = document.querySelector('.tab-content.active')?.id;
    if (activeTab === 'tab-inicio') {
      const tabInicio = document.getElementById('tab-inicio');
      if (tabInicio && !tabInicio.innerHTML.trim()) {
        if (typeof renderDashboardFromCache === 'function') {
          renderDashboardFromCache();
        } else {
          // Fallback: poner un mensaje de carga
          tabInicio.innerHTML = `
...
// ==================== DASHBOARD CON CACHÉ DE DATOS ====================

// ----- Renderizar desde caché (síncrono) -----
function renderDashboardFromCache() {
  const tabInicio = document.getElementById('tab-inicio');
  if (!tabInicio) return;

  const raw = localStorage.getItem('ri5_dashboard_data');
  if (!raw) {
    tabInicio.innerHTML = `
      <div style="padding: 40px 20px; text-align: center; color: var(--text-secondary);">
        <div style="font-size: 24px; margin-bottom: 10px;">⏳</div>
  ...
  try {
    ...
    if (edad > 60000) {
      console.log('Datos de caché antiguos, mostrando mientras se actualiza');
    }

    // --- Construir HTML a partir de data ---
    let html = '';
    ...
    html += `
      <div id="dashboardHero" style="position:relative; overflow:hidden; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:20px; padding:20px 20px 16px; margin-bottom:16px; display:flex; align-items:center; gap:16px;">
        ...
        <div style="flex:1; min-width:0;">
          <div id="dashboardGreeting" style="font-size:11px; color:var(--text-secondary); letter-spacing:1px; text-transform:uppercase; margin-bottom:2px;">${saludo}</div>
          <div id="dashboardUsername" style="font-size:20px; font-weight:600; color:var(--text-primary); letter-spacing:0.5px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${data.username || 'Usuario'}</div>
          <div id="dashboardCity" style="font-size:12px; color:var(--text-secondary); margin-top:2px;">${data.city || ''}</div>
          <div id="dashboardShoe" style="display:inline-flex; align-items:center; gap:6px; margin-top:8px; padding:4px 10px; background:rgba(192,160,96,0.1); border:1px solid rgba(192,160,96,0.3); border-radius:20px; font-size:11px; color:var(--gold);">
            <span>👟</span><span id="dashboardShoeName">${data.zapatilla || '—'}</span>
            <span style="color:var(--text-secondary);" id="dashboardShoeKm">${data.kmZapatilla || '0'} km</span>
          </div>
        </div>
        <div id="dashboardLevelBadge" style="position:absolute; top:14px; right:14px; background:var(--bg-primary); border:1px solid var(--border-color); border-radius:10px; padding:4px 10px; text-align:center;">
          ...
          <div id="dashboardLevel" style="font-size:22px; font-weight:300; color:var(--gold); line-height:1;">${data.nivel || 1}</div>
        </div>
      </div>
    `;
    ...
    html += `
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:16px;">
        <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; text-align:center;">
          ...
          <div id="dashboardKmSemana" style="font-size:32px; font-weight:300; color:var(--gold); line-height:1;">${data.kmSemana?.toFixed(1) || '0'}</div>
        ...
        <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; text-align:center;">
          ...
          <div id="dashboardSesionesSemana" style="font-size:32px; font-weight:300; color:var(--gold); line-height:1;">${data.sesionesSemana || 0}</div>
    ...
    if (data.ultimaSesion) {
      ...
      html += `
        <div id="dashboardUltimaSesion" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
          ...
          <div id="dashboardUltimaSesionContent" style="color:var(--text-secondary); font-size:13px; cursor:pointer;">
            <div style="display:flex; align-items:flex-start; gap:12px;">
              <span style="font-size:28px; line-height:1;">${us.tipoEmoji || '🏃'}</span>
              <div style="flex:1;">
                <div style="font-size:15px; font-weight:500; color:var(--text-primary); margin-bottom:4px;">${us.nombre || 'Sesión'}</div>
                <div style="font-size:12px; color:var(--text-secondary);">${us.fecha || ''}</div>
                <div style="display:flex; gap:16px; margin-top:10px; flex-wrap:wrap;">
                  ${us.distancia ? `<div style="text-align:center;"><div style="font-size:18px; font-weight:600; color:var(--gold);">${us.distancia.toFixed(2)} km</div><div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.5px;">distancia</div></div>` : ''}
                  ${us.duracion ? `<div style="text-align:center;"><div style="font-size:18px; font-weight:600; color:var(--gold);">${us.duracion} min</div><div style="font-size:10px; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.5px;">tiempo</div></div>` : ''}
                </div>
              </div>
            </div>
          </div>
    ...
    if (data.proximaSesion) {
      ...
      html += `
        <div id="dashboardProximaSesion" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
          ...
          <div id="dashboardProximaSesionContent" style="color:var(--text-secondary); font-size:13px; cursor:pointer;">
            <div style="display:flex; align-items:center; gap:12px;">
              <span style="font-size:28px;">${ps.tipoEmoji || '📅'}</span>
              <div>
                <div style="font-size:15px; font-weight:500; color:var(--text-primary);">${ps.nombre || 'Sesión'}</div>
                <div style="font-size:12px; color:var(--gold); margin-top:2px;">${ps.fecha || ''}</div>
                ${ps.duracion ? `<div style="font-size:11px; color:var(--text-secondary); margin-top:4px;">⏱ ${ps.duracion} min</div>` : ''}
              </div>
            </div>
          </div>
        </div>
    ...
    if (data.zonas && data.zonas.length > 0) {
      html += `<div id="dashboardZonasCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
          <span style="font-size:11px; letter-spacing:1px; color:var(--text-secondary); text-transform:uppercase;">❤️ Mis zonas</span>
          <span style="font-size:18px;">📊</span>
        </div>
        <div id="dashboardZonasContent">`;
      data.zonas.forEach((z, index) => {
        html += `
          <div class="zona-item" data-zona="${z.id}" onclick="this.classList.toggle('expanded')" style="background: var(--bg-primary); border-radius: 12px; padding: 12px 14px; border-left: 4px solid ${z.color}; display: flex; align-items: center; gap: 14px; cursor: pointer; transition: all 0.25s ease; position: relative; overflow: visible;">
            <div class="zona-circle" style="width: 44px; height: 44px; border-radius: 50%; display: flex; flex-direction: column; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; flex-shrink: 0; color: #fff; background: ${z.color};">${z.id}<small style="font-size:8px; font-weight:400; opacity:0.8; margin-top:-2px;">${z.nombreCorto || ''}</small></div>
            <div class="zona-info" style="flex:1; min-width:0;">
              <div class="nombre" style="font-weight:600; font-size:14px; color:var(--text-primary);">${z.nombre}</div>
              <div class="detalle" style="font-size:11px; color:var(--text-secondary); margin-top:2px;">FC ${z.fcStr}</div>
              <div class="ritmo" style="font-size:12px; font-weight:500; color:#c0a060; margin-top:2px;">⏱ ${z.ritmo}</div>
              <div class="zona-expand" style="max-height: 0; overflow: hidden; transition: max-height 0.4s ease, margin 0.3s ease, opacity 0.3s ease; margin-top: 0; font-size: 12px; color: var(--text-secondary); line-height: 1.6; opacity: 0;">
                <p style="margin:4px 0; white-space:pre-wrap; word-break:break-word;"><strong style="color:#c0a060;">📌</strong> ${z.desc}</p>
              </div>
            </div>
          </div>
        `;
      ...
      html += `
        <div id="dashboardZonasCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px;">
          ...
          <div id="dashboardZonasContent" style="color:var(--text-secondary); font-size:13px;">
    ...
    // Progreso del plan
    if (data.progresoPlan) {
      html += `
        <div id="dashboardPlanProgresoCard" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:16px; padding:16px; margin-bottom:16px; display:block;">
          <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
            ...
            <span id="dashboardPlanProgresoPct" style="font-size:13px; font-weight:700; color:var(--gold);">${data.progresoPlan.porcentaje}%</span>
          </div>
          <div style="width:100%; height:8px; background:var(--bg-primary); border-radius:10px; overflow:hidden;">
            <div id="dashboardPlanProgresoBar" style="height:100%; width:${data.progresoPlan.porcentaje}%; background:var(--gold); border-radius:10px; transition:width 0.5s ease;"></div>
          </div>
          <div id="dashboardPlanProgresoTexto" style="font-size:12px; color:var(--text-secondary); margin-top:8px;">${data.progresoPlan.completadas} de ${data.progresoPlan.total} sesiones completadas</div>
        </div>
      `;
    }
    ...
    tabInicio.innerHTML = html;

    // Inicializar consejo
    initDashboardConsejo();

    // Enlazar eventos de zonas
    document.querySelectorAll('.zona-item').forEach(el => {
      el.onclick = function() { this.classList.toggle('expanded'); };
    });

    console.log('📦 Dashboard renderizado desde caché de datos');
  } catch (e) {
    console.warn('Error renderizando desde caché:', e);
    tabInicio.innerHTML = `
      <div style="padding: 20px; text-align: center; color: var(--zone-5);">
        <strong>⚠️ Error al cargar datos en caché</strong>
        <p style="font-size: 14px; margin-top: 10px;">Se actualizará automáticamente</p>
      </div>
    `;
  }
}
...
function guardarDatosDashboardEnCache() {
  try {
    const data = {
      username: document.getElementById('dashboardUsername')?.textContent || AppState.currentUserData?.username || 'Usuario',
      city: document.getElementById('dashboardCity')?.textContent?.replace('📍', '').trim() || '',
      nivel: document.getElementById('dashboardLevel')?.textContent || 1,
      zapatilla: document.getElementById('dashboardShoeName')?.textContent || '—',
      kmZapatilla: parseFloat(document.getElementById('dashboardShoeKm')?.textContent?.replace('km', '').trim()) || 0,
      kmSemana: parseFloat(document.getElementById('dashboardKmSemana')?.textContent) || 0,
      sesionesSemana: parseInt(document.getElementById('dashboardSesionesSemana')?.textContent) || 0,
      ultimaSesion: null,
      proximaSesion: null,
      zonas: [],
      progresoPlan: null,
    ...
    // Última sesión
    const ultContent = document.getElementById('dashboardUltimaSesionContent');
    if (ultContent) {
      const nombreEl = ultContent.querySelector('.nombre') || ultContent.querySelector('[style*="font-size:15px"]');
      const fechaEl = ultContent.querySelector('[style*="font-size:12px"]');
      const distEl = ultContent.querySelector('[style*="font-size:18px"]');
      const durEl = ultContent.querySelectorAll('[style*="font-size:18px"]')[1];
      const tipoEmojiEl = ultContent.querySelector('[style*="font-size:28px"]');
      const tipoEmoji = tipoEmojiEl?.textContent || '🏃';
      data.ultimaSesion = {
        tipoEmoji,
        nombre: nombreEl?.textContent || '',
        fecha: fechaEl?.textContent || '',
        distancia: parseFloat(distEl?.textContent) || 0,
        duracion: parseInt(durEl?.textContent) || 0
      };
    }

    // Próxima sesión
    const proxContent = document.getElementById('dashboardProximaSesionContent');
    if (proxContent) {
      const nombreEl = proxContent.querySelector('[style*="font-size:15px"]');
      const fechaEl = proxContent.querySelector('[style*="font-size:12px"]');
      const durEl = proxContent.querySelector('[style*="font-size:11px"]');
      const tipoEmojiEl = proxContent.querySelector('[style*="font-size:28px"]');
      const tipoEmoji = tipoEmojiEl?.textContent || '📅';
      data.proximaSesion = {
        tipoEmoji,
        nombre: nombreEl?.textContent || '',
    ...
    // Zonas
    const zonasItems = document.querySelectorAll('#dashboardZonasContent .zona-item');
    zonasItems.forEach(el => {
      const id = el.dataset.zona || 'Z?';
      const nombre = el.querySelector('.nombre')?.textContent || '';
      const fc = el.querySelector('.detalle')?.textContent?.replace('FC ', '') || '';
      const ritmo = el.querySelector('.ritmo')?.textContent?.replace('⏱ ', '') || '';
      const desc = el.querySelector('.zona-expand p')?.textContent?.replace('📌', '').trim() || '';
      const color = el.querySelector('.zona-circle')?.style?.background || '#888';
      const nombreCorto = el.querySelector('.zona-circle small')?.textContent || '';
      data.zonas.push({ id, nombre, fcStr: fc, ritmo, desc, color, nombreCorto });
    });

    // Progreso del plan
    const pct = document.getElementById('dashboardPlanProgresoPct')?.textContent?.replace('%', '');
    const texto = document.getElementById('dashboardPlanProgresoTexto')?.textContent;
    if (pct && texto) {
      const match = texto.match(/(\d+) de (\d+)/);
      if (match) {
        data.progresoPlan = {
          completadas: parseInt(match[1]),
          total: parseInt(match[2]),
          porcentaje: parseInt(pct)
        };
      }
    }

    localStorage.setItem('ri5_dashboard_data', JSON.stringify(data));
    console.log('✅ Datos del dashboard guardados en caché');
  } catch (e) {
    console.warn('Error guardando caché de dashboard:', e);
  }
}
...
function abrirModalDetalleSesion(sesionData, esPlan = false) {
  ...
  // Botón GPS solo si es sesión del plan y no descanso
...
// ----- CARGA PRINCIPAL DEL DASHBOARD (con try/catch y caché) -----
async function cargarDashboard() {
  try {
    ...
    // 1. DATOS DE USUARIO (perfil + foto + nivel + zapatilla)
    try {
      ...
      if (cityEl && profile.city) {
        cityEl.innerHTML = `📍 ${Utils.escapeHTML(profile.city)}`;
      }
    ...
    // 2. ENTRENAMIENTOS: última sesión + km semana + sesiones semana
    try {
      const snap = await firebaseServices.db
    ...
    try {
      ...
      if (!planId) {
        const planSnap = await firebaseServices.db
          .collection('users')
          .doc(AppState.currentUserId)
          .collection('planes')
          ...
          if (proxEl && proxima) {
            ...
            proxEl.onclick = () => {
              if (!proxima.detalle) {
                ...
                if (sesionOriginal && sesionOriginal.detalle) {
                  proxima.detalle = sesionOriginal.detalle;
                } else {
                  proxima.detalle = {
    ...
    // 4. ZONAS (con validación de AppState.lastZones)
    try {
      ...
      if (raw && AppState.lastZones && AppState.lastZones.length > 0) {
        ...
        if (calc && calc.zones && calc.ul && calc.ritmoBase) {
          ...
          zonas.forEach((z, index) => {
            ...
            let descRaw = z[6] || '';
            const parrafos = [...descRaw.matchAll(/<p[^>]*>(.*?)<\/p>/gs)]
            ...
            html += `
              <div class="zona-item" data-zona="${id}" onclick="this.classList.toggle('expanded')" style="
                background: var(--bg-primary);
                border-radius: 12px;
                padding: 12px 14px;
                border-left: 4px solid ${colores[index]};
                display: flex;
                align-items: center;
                gap: 14px;
                cursor: pointer;
    ...
    // 5. GRÁFICAS DE PROGRESO Y ZONAS MÁS USADAS
    ...
    // 7. GUARDAR LOS DATOS EN CACHÉ PARA PRÓXIMA RECARGA
    guardarDatosDashboardEnCache();
...
async function cargarGraficaDashboard(periodo) {
  ...
  try {
    ...
    let snap;
    try {
      snap = await firebaseServices.db
        .collection('globalFeed')
        .where('userId', '==', AppState.currentUserId)
      ...
      const snapAll = await firebaseServices.db
        .collection('globalFeed')
        .where('userId', '==', AppState.currentUserId)
        .get();
    ...
    if (_dashboardChartInstance) {
      _dashboardChartInstance.destroy();
      _dashboardChartInstance = null;
    }
    ...
    _dashboardChartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'km',
          data,
          backgroundColor: 'rgba(192,160,96,0.85)',
          borderColor: '#c0a060',
          borderWidth: 1,
          borderRadius: 6,
          borderSkipped: false
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { color: tema.text, font: { size: 10 }, maxRotation: 0, maxTicksLimit: periodo === 'semana' ? 7 : 10 }
          },
          y: {
            grid: { color: tema.grid },
            ticks: { color: tema.text, font: { size: 10 }, callback: v => v + ' km' },
            beginAtZero: true
          }
        }
      }
    ...
    if (e && e.message && e.message.includes('index')) {
      console.warn('⚠️ Firestore pide crear un índice compuesto para esta consulta. Revisa el enlace en el error de arriba y créalo en Firebase Console.');
    }
    const contentEl = document.getElementById('dashboardChartContent');
...
async function cargarZonasUsadasDashboard() {
  ...
  try {
    ...
    if (_dashboardZonasUsadasInstance) {
      _dashboardZonasUsadasInstance.destroy();
      _dashboardZonasUsadasInstance = null;
    }
    ...
    _dashboardZonasUsadasInstance = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: Object.keys(conteoZonas),
        datasets: [{
          data: Object.values(conteoZonas),
      ...
      options: {
        ...
        maintainAspectRatio: false,
        plugins: {
          legend: { position: 'bottom', labels: { color: tema.text, font: { size: 11 }, padding: 10 } }
        }
      }
    });
...
// ----- ACTIVAR DASHBOARD AL CAMBIAR DE PESTAÑA -----
...
window.renderDashboardFromCache = renderDashboardFromCache;
    ...
    if (premiumElement) {
      ...
      if (perfilTab && perfilTab.classList.contains('active')) {
        ...
        Utils.confirm(
          ...
          if (resultado) {
            window.open('https://www.instagram.com/coachjoaquinpeinado', '_blank');
          }
        }).catch(() => {});
      }
    }
