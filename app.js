// ==================== app.js - VERSIÓN COMPLETA CON SCROLL CENTRADO ====================
// VERSIÓN: 4.26 - Scroll centrado para el plan
// ====================
...
const UI = {
  ...
  // ===== NUEVA FUNCIÓN DE SCROLL CENTRADO =====
  scrollToPlanCenter() {
    setTimeout(() => {
      ...
      if (element) {
        ...
        // Calcular el offset para que el centro del elemento quede en el centro de la ventana
        const targetScrollY = rect.top + window.pageYOffset - (windowHeight / 2) + (elementHeight / 2);
        window.scrollTo({ top: targetScrollY, behavior: 'smooth' });
      } else {
        // Fallback: buscar el título
        const title = document.querySelector('#calendarioEntreno h3');
        if (title) {
          const rect = title.getBoundingClientRect();
          const elementHeight = rect.height || 200;
          const targetScrollY = rect.top + window.pageYOffset - (window.innerHeight / 2) + (elementHeight / 2);
          window.scrollTo({ top: targetScrollY, behavior: 'smooth' });
        }
      }
    }, 200);
