// ==================== app.js - VERSIÓN COMPLETA CON SCROLL A 1/3 ====================
// VERSIÓN: 4.25 - Scroll mejorado para que el encabezado quede a 1/3 de la pantalla
// ====================
...
const UI = {
  ...
  // ===== NUEVA FUNCIÓN DE SCROLL A 1/3 =====
  scrollToPlanHeader() {
    setTimeout(() => {
      const element = document.getElementById('calendarioEntreno');
      if (element) {
        const rect = element.getBoundingClientRect();
        const targetY = rect.top + window.pageYOffset - window.innerHeight * 0.33;
        window.scrollTo({ top: targetY, behavior: 'smooth' });
      } else {
        // Fallback si no existe el elemento
        const title = document.querySelector('#calendarioEntreno h3');
        if (title) {
      ...
      try {
        ...
        if (visible && planId) {
          ...
          if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
            ...
            // Scroll mejorado a 1/3
            setTimeout(() => {
              this.scrollToPlanHeader();
            }, 300);
            return;
          }
        }
