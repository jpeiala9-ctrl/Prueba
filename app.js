...
const AppState = {
  ...
  iniciarListeners() {
    ...
    this.unsubscribeMisDatos = firebaseServices.db
      ...
      .onSnapshot(async (doc) => {
        ...
        // === PREMIUM / ADMIN EN TIEMPO REAL ===
        // Antes, si el admin pasaba a un usuario de gratis a premium (o al
        // revés) desde el panel de administración, ese cambio solo se
        // reflejaba en la interfaz cuando el usuario afectado cerraba
        // sesión y volvía a entrar (o reiniciaba la app): AppState.isPremium
        // solo se fijaba una vez, en setCurrentUser(), al iniciar sesión.
        // Como este mismo listener ya escucha su propio documento de
        // usuario en tiempo real (es el mismo que sincroniza friendIds),
        // basta con comparar aquí el campo 'premium' recién llegado contra
        // el que ya teníamos y, si cambió, refrescar todo lo que depende
        // de él -- sin que el usuario tenga que hacer nada.
        const premiumAnterior = this.isPremium;
        const nuevoPremium = nuevaData.premium || false;
        if (nuevoPremium !== premiumAnterior) {
          this.isPremium = nuevoPremium;
          this.premiumExpiryDate = nuevaData.expires ? new Date(nuevaData.expires) : null;
          this.actualizarInterfazPremium();
          this.verificarExpiracionPremium();
          this.actualizarBadgePremium();
          Utils.showToast(
            nuevoPremium ? '🌟 ¡Tu cuenta ahora es Premium!' : 'Tu cuenta ha pasado a la versión gratuita',
            nuevoPremium ? 'success' : 'info'
          );
        } else if (nuevaData.expires !== this.currentUserData?.expires) {
          // La fecha de caducidad puede renovarse sin que 'premium' deje
          // de ser true en ningún momento -- se actualiza igual, sin toast
          // (no es un cambio de estado, solo de fecha).
          this.premiumExpiryDate = nuevaData.expires ? new Date(nuevaData.expires) : null;
          this.verificarExpiracionPremium();
        }
        if (typeof nuevaData.isAdmin === 'boolean' && nuevaData.isAdmin !== this.isAdmin) {
          this.isAdmin = nuevaData.isAdmin;
        }
    ...
    // === (SOLO ADMIN) SESIONES QUE YO HE ENVIADO: AVISO EN TIEMPO REAL ===
    // Simétrico al listener de "solicitudes enviadas" de amigos: sin él,
    // el admin no se enteraba de que alguien aceptó/rechazó una sesión
    // hasta recargar la app.
    if (window.SessionInvites && this.isAdmin) SessionInvites.iniciarListenerEnviadas();
  },
  ...
  detenerListeners() {
    ...
    if (window.SessionInvites) SessionInvites.detenerListenerEnviadas();
