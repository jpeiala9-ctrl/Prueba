...
// VERSIÓN: 4.44 - Restaurada la lógica original de agrupación de mensajes
//                y ajustado el estilo del modal de conversación para eliminar saltos.
// ====================
...
const Admin = {
  ...
  // ============================================================
  // NUEVO SISTEMA DE HILOS DE CONVERSACIÓN DE SOPORTE (VERSIÓN FUNCIONAL Y SIN SALTOS)
  // ============================================================
  ...
  abrirConversacionSoporte(uid) {
    ...
    header.innerHTML = `
      ...
      <button onclick="Admin.cerrarConversacionSoporte()" style="background:transparent; border:none; font-size:24px; cursor:pointer; color:var(--text-secondary); padding:0 8px; line-height:1;">✕</button>
    `;
    ...
    // Pie fijo: input + botón con estilo action-button
  ...
  async _cargarMensajesConversacion(uid, container) {
    try {
      ...
      const adminUid2 = AppState.currentUserId;
      for (const msg of todos) {
        const esAdmin = msg.fromUid === adminUid2;
      ...
      const noLeidos = todos.filter(msg => !msg.leido && msg.toUid === adminUid2);
