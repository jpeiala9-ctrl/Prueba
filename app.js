...
const Admin = {
        ...
        <div class="usuario-item ${u.esNuevo ? 'nuevo' : ''}" data-uid="${u.uid}">
          ...
          <div class="usuario-detalle">
            ...
            <div class="usuario-acciones">
              ...
              <button class="entrenador-usuario-btn" data-uid="${u.uid}" data-username="${Utils.escapeHTML(u.username)}" data-entrenador="${!!u.user.isTrainer}" data-max-alumnos="${u.user.maxAlumnos ?? ''}" style="${u.user.isTrainer ? 'border-color:var(--gold); color:var(--gold);' : ''}">${u.user.isTrainer ? `🎯 ENTRENADOR${u.user.maxAlumnos != null ? ' (' + u.user.maxAlumnos + ')' : ''}` : 'HACER ENTRENADOR'}</button>
  ...
  async guardarEntrenador() {
    ...
    Utils.showLoading();
    try {
      await firebaseServices.db.collection('users').doc(uid).update({ isTrainer: nuevoEstado, maxAlumnos });
    } catch (error) {
    ...
    if (btn) {
      ...
      btn.textContent = nuevoEstado ? `🎯 ENTRENADOR${maxAlumnos != null ? ' (' + maxAlumnos + ')' : ''}` : 'HACER ENTRENADOR';
