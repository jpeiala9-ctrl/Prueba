...
// VERSIÓN: 4.42 - Añadida agrupación por usuario en el panel de administración (hilos de conversación)
//                para facilitar la gestión de mensajes de soporte.
// ====================
...
const Admin = {
  ...
  // ============================================================
  // NUEVO SISTEMA DE HILOS DE CONVERSACIÓN DE SOPORTE
  // ============================================================
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    if (reset) {
      container.innerHTML = '<div style="text-align:center; padding:40px; color: var(--text-secondary);">⏳ Cargando conversaciones...</div>';
    }
    ...
    try {
      ...
      for (const msg of mensajes) {
        // Si el mensaje es de un usuario (fromUid no es admin), la clave es fromUid
        // Si el mensaje es del admin, la clave es toUid
        const uid = msg.fromUid === adminUid ? msg.toUid : msg.fromUid;
        if (!uid || uid === adminUid) continue; // saltar mensajes a sí mismo o sin uid
        ...
        // Ordenar mensajes por timestamp (más reciente primero para el último)
      ...
      // Ordenar usuarios por fecha del último mensaje (más reciente primero)
      ...
      for (const user of usuarios) {
        ...
        html += `
          <div class="usuario-soporte-item" data-uid="${user.uid}" style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:background 0.2s;">
            ${avatar}
            <div style="flex:1;min-width:0;">
              <div style="display:flex;justify-content:space-between;">
                <span style="font-weight:600;color:var(--accent-yellow);">${Utils.escapeHTML(Utils.capitalizeUsername(user.username))}</span>
                <span style="font-size:11px;color:var(--text-secondary);">${fecha}</span>
              </div>
              <div style="font-size:13px;color:var(--text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${textoPreview}</div>
            </div>
            ${badgeNoLeidos}
            <span style="font-size:11px;color:var(--accent-blue);background:var(--bg-primary);padding:2px 8px;border-radius:12px;">VER</span>
          </div>
        `;
      }
      ...
      // Evento click para abrir conversación con ese usuario
  ...
  abrirConversacionSoporte(uid) {
    ...
    header.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding:16px 20px; border-bottom:1px solid var(--border-color); background:var(--bg-primary); flex-shrink:0;';
    header.innerHTML = `
      <span style="font-size:16px; font-weight:bold; color:var(--accent-yellow);">💬 Soporte con <span id="conversacionNombre"></span></span>
      <button onclick="Admin.cerrarConversacionSoporte()" style="background:transparent; border:none; font-size:24px; cursor:pointer; color:var(--text-secondary); padding:0 8px;">✕</button>
    `;
    ...
    messagesContainer.style.cssText = 'flex:1; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:8px; background:var(--bg-secondary);';
    modal.appendChild(messagesContainer);

    // Pie: campo de texto y botón enviar
    const footer = document.createElement('div');
    footer.style.cssText = 'display:flex; gap:8px; padding:12px 16px; border-top:1px solid var(--border-color); background:var(--bg-primary); flex-shrink:0;';
    footer.innerHTML = `
      <input type="text" id="conversacionInput" placeholder="Escribe tu respuesta..." style="flex:1; margin:0; padding:8px 12px; border-radius:10px; background:var(--bg-secondary); border:1px solid var(--border-color); color:var(--text-primary);">
      <button id="conversacionEnviarBtn" style="background:var(--accent-blue); border:none; color:var(--bg-primary); padding:8px 16px; border-radius:10px; cursor:pointer; font-weight:bold;">ENVIAR</button>
    `;
    ...
    Storage.getUser(uid).then(userData => {
      ...
      if (nombreEl) nombreEl.textContent = userData?.username ? Utils.capitalizeUsername(userData.username) : 'Usuario';
    }).catch(() => {
      const nombreEl = document.getElementById('conversacionNombre');
    ...
    this._cargarMensajesConversacion(uid, messagesContainer);
    ...
    // Guardar referencia para poder cerrar y actualizar badge
    this._conversacionAbierta = { uid, messagesContainer };
  },

  async _cargarMensajesConversacion(uid, container) {
    try {
      const adminUid = AppState.currentUserId;
      
      // Consulta 1: mensajes enviados POR el usuario (fromUid = uid) y recibidos POR el admin (toUid = adminUid)
      const snapshot1 = await firebaseServices.db.collection('soporteMensajes')
        .where('fromUid', '==', uid)
        .where('toUid', '==', adminUid)
        .get();
      
      // Consulta 2: mensajes enviados POR el admin (fromUid = adminUid) y recibidos POR el usuario (toUid = uid)
      const snapshot2 = await firebaseServices.db.collection('soporteMensajes')
        .where('fromUid', '==', adminUid)
        .where('toUid', '==', uid)
        .get();

      // También incluir mensajes de bienvenida que pueden tener broadcastId
      // pero que son del admin al usuario (ya están en snapshot2)
      ...
      container.innerHTML = '';
      if (todos.length === 0) {
        container.innerHTML = '<p style="text-align:center; color:var(--text-secondary); padding:20px;">No hay mensajes en esta conversación</p>';
        return;
      }
      ...
      for (const msg of todos) {
        ...
        div.style.cssText = `
          ...
          ${esAdmin ? 'align-self:flex-end; background:var(--accent-blue); color:var(--bg-primary);' : 'align-self:flex-start; background:var(--bg-primary); color:var(--text-primary); border:1px solid var(--border-color);'}
          word-wrap:break-word;
        `;
        div.textContent = msg.texto;
        
        const time = document.createElement('div');
        time.style.cssText = `font-size:10px; color:${esAdmin ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)'}; margin-top:4px; text-align:right;`;
      ...
      // Marcar mensajes como leídos (los que el admin ha recibido)
      ...
      // Actualizar badge de soporte admin
      if (noLeidos.length > 0) {
        // Recontar no leídos globales
      ...
      container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar mensajes</p>';
    }
  },
  ...
  async _enviarMensajeSoporte(uid, texto, container) {
    ...
    if (ok) {
      ...
      const div = document.createElement('div');
      div.style.cssText = 'max-width:75%; padding:8px 12px; border-radius:12px; align-self:flex-end; background:var(--accent-blue); color:var(--bg-primary); word-wrap:break-word;';
