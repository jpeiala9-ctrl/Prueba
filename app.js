        ...
        if (alumnosGanados.length || alumnosPerdidos.length) {
          for (const uidNuevoAlumno of alumnosGanados) {
            try {
              ...
              Utils.showToast(`🎯 ${nombre} ahora es tu alumno`, 'success');
            } catch (e) { /* nombre opcional, no bloquea el resto */ }
          }
          const entrenadorTabActiva = document.getElementById('subtab-perfil-entrenador')?.classList.contains('active');
          if (entrenadorTabActiva && this.isTrainer && !this.isAdmin && window.Friends && Friends.cargarPanelAlumnos) {
