...
// Versión: 4.09 - Bump de caché (v401 -> v402): sustitución de las cachés
//                con TTL de la ronda anterior (v401) por CACHÉS EN VIVO
//                alimentadas por listeners onSnapshot, para eliminar la
//                latencia de frescura que traía el TTL (hasta 30s/8min/
//                2,5min según el caso) sin gastar más lecturas -- al
//                contrario, un listener cuesta 1 lectura inicial + solo
//                cambios reales, mientras que un TTL cuesta 1 lectura cada
//                vez que expira con la pantalla activa. Archivos tocados:
//                gamification.js v5.22 (getData(uid) en vivo, sin TTL);
//                sponsors.js v1.14 (getActivos() en vivo, sin TTL);
//                app.js v4.66 (arranca/para ambos listeners globales en
//                setCurrentUser/detenerListeners; lista de usuarios de
//                Soporte→Admin también pasa de TTL 2,5min a listener en
//                vivo, arrancado la primera vez que el admin abre esa
//                pestaña); wall.js v4.19 (likes sin parpadeo -- patch en
//                el sitio del contador y la clase 'liked' en vez de
//                repintar la tarjeta entera -- y precarga de avatares con
//                new Image() antes de insertar el HTML, para que las fotos
//                no aparezcan con retraso). La lista de amigos en tiempo
//                real (unsubscribeMisDatos en app.js) ya estaba bien: no
//                hizo falta tocarla. Fuera de esta ronda, deliberadamente
//                sin tocar: Admin._statsCache, Profile._misEntrenamientosCache,
//                Friends._nivelesCache, Storage._adminUidCache y el
//                sessionStorage de lista de amigos -- ver el motivo de
//                cada uno en sus propios archivos.
// Versión: 4.08 - Bump de caché (v400 -> v401): nueva ronda de revisión de
//                lecturas duplicadas a Firestore, 8 archivos + index.html.
//                Resumen (ver el changelog de cada archivo para el
//                detalle): gamification.js v5.21 (caché en memoria TTL 30s
//                de getData(uid)); index.html (Dashboard reutiliza
//                gam.currentShoe, y actualizarEstaSemanaDashboard /
//                cargarGraficaDashboard enrutadas por la caché compartida
//                de historial); profile.js (reutiliza currentShoe, caché
//                TTL 60s de "mis últimos entrenamientos"); app.js (caché
//                TTL 2.5min de la lista de usuarios admin; precarga de
//                estadísticas/tienda admin ya no bloquea el splash; nuevas
//                cachés en vivo de solicitudes de amistad); storage.js
//                (getUltimoCalculo evita releer users/{uid} cuando ya se
//                tiene el id; eliminado un update duplicado al marcar
//                mensaje de soporte leído); sponsors.js (caché TTL 8min de
//                getActivos); friends.js (precarga de perfiles de amigos
//                limitada a los primeros 20; obtenerSolicitudesPendientes
//                reutiliza las cachés en vivo en vez de volver a consultar
//                Firestore); auth.js (verificarAhora ya no hace un update
//                ni un get redundantes); calendar.js (invalida la nueva
//                caché de "mis entrenamientos" al marcar/desmarcar sesión).
...
console.log('[SW] sw.js cargado correctamente (v396)');