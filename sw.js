...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'RI5', {
      ...
      badge: './icon-192.png',
      // 'target' identifica QUÉ hay que abrir en la app: 'soporte'
      // (mensaje de soporte), 'amistad' (solicitud de amistad) o 'like'
      // (a una publicación del Muro -- 'targetId' lleva el id de esa
      // publicación en ese caso). Quien envíe el push debe incluir estos
      // campos; si no los manda, target queda a null y la notificación
      // simplemente abre la app sin destino concreto (comportamiento de
      // antes). Antes solo se guardaba la url -- clients.openWindow(url)
      // siempre abría una pestaña nueva desde cero y la app no sabía qué
      // mostrar, así que aunque se abriera rápido el usuario tenía que
      // navegar a mano hasta el mensaje/solicitud/publicación.
      data: { url: data.url || './', target: data.target || null, targetId: data.targetId || null }
    })
  );
});
...
self.addEventListener('notificationclick', event => {
  ...
  const urlBase = data.url || './';

  event.waitUntil((async () => {
    // Reutilizar una ventana YA abierta es mucho más rápido que abrir una
    // nueva: la app ya está "caliente" (Firebase inicializado, sesión
    // cargada), así que puede abrir el modal de destino al instante por
    // postMessage, sin ninguna recarga de página. clients.openWindow()
    // siempre creaba una pestaña nueva aunque ya hubiera una abierta.
    const ventanas = await clients.matchAll({ type: 'window', includeUncontrolled: true });
    for (const cliente of ventanas) {
      await cliente.focus();
      cliente.postMessage({ type: 'notificationClick', target, targetId: data.targetId || null });
      return;
    }

    // Sin ninguna ventana abierta: hay que arrancar la app desde cero.
    // El destino se codifica en la propia URL (notifTarget=...) para que,
    // en cuanto la app sepa quién es el usuario, abra ese destino
    // inmediatamente -- ver el listener de AppState.setCurrentUser en
    // index.html -- en vez de dejar al usuario en la pantalla de inicio
    // teniendo que navegar a mano hasta el mensaje.
    let urlDestino = urlBase;
    if (target) {
      urlDestino += (urlBase.includes('?') ? '&' : '?') + 'notifTarget=' + encodeURIComponent(target);
      if (data.targetId) urlDestino += '&notifTargetId=' + encodeURIComponent(data.targetId);
    }
