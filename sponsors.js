...
const Sponsors = {
  ...
  async cargarAdminLista() {
    ...
    container.innerHTML = lista.map(sp => {
      ...
      `;
      return `
        <div style="background:${activo ? 'rgba(107,212,107,0.10)' : 'var(--bg-secondary)'}; border:1px solid ${activo ? 'var(--realizado-color)' : 'var(--border-color)'}; border-radius:14px; padding:12px; margin-bottom:10px; transition:background 0.2s ease, border-color 0.2s ease;">
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; margin-top:10px; padding-top:10px; border-top:1px solid var(--border-color); flex-wrap:wrap;">
            <div style="display:flex; align-items:center; gap:8px; min-width:0; flex-wrap:wrap;">
              <div style="display:inline-flex; align-items:center; gap:4px; flex-shrink:0; padding:4px 10px; border-radius:20px; background:var(--bg-primary); border:1px solid var(--border-color); font-size:11px; color:var(--text-secondary); white-space:nowrap;">
            ...
            <div style="display:flex; gap:6px; flex-shrink:0;">
              <button onclick="Sponsors.abrirFormulario('${sp.id}')" style="border:1px solid var(--border-color); background:transparent; color:var(--text-secondary); border-radius:10px; padding:6px 12px; font-size:11px; font-weight:700; letter-spacing:0.3px; cursor:pointer; white-space:nowrap;">EDITAR</button>
...
console.log('✅ sponsors.js v1.4 - Panel admin: clics como insignia, botón editar sin emoji de lápiz');