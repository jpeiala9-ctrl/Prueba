...
//
// Versión: 1.14
// - CACHÉ EN VIVO (sustituye al TTL de 8min de getActivos de la v1.13):
//   ver iniciarListener()/detenerListener() y el comentario junto a
//   _activosEnVivo, más abajo. Activar/desactivar una tienda desde el
//   panel de admin se ve al instante en el banner de cualquier
//   dispositivo, en vez de tardar hasta 8 minutos.
// Versión: 1.13
...
const Sponsors = {
  ...
  // 🔥 CAMBIO (v1.14, caché en vivo -- sustituye al TTL de 8min de la
  // v1.13): getActivos() ya no expira por tiempo. La alimenta un listener
  // onSnapshot sobre sponsors WHERE activo==true (iniciarListener, arrancado
  // desde AppState.setCurrentUser y parado desde AppState.detenerListeners,
  // ver app.js v4.66). Motivo: con TTL, activar o desactivar una tienda
  // patrocinadora podía tardar hasta 8 minutos en reflejarse en el banner
  // de otros dispositivos/pestañas abiertas. Con el listener el cambio se
  // ve al instante, y además cuesta menos lecturas (1 lectura inicial +
  // solo cambios reales, en vez de 1 lectura cada vez que expiraba el TTL
  // con el Dashboard abierto). guardar()/eliminar() ya no necesitan
  // invalidar nada a mano -- el propio listener recibirá el cambio -- pero
  // se deja invalidarCacheActivos() como no-op por compatibilidad con las
  // llamadas ya existentes.
  _activosEnVivo: null,
  _unsubscribeSponsors: null,

  // Arranca (de forma idempotente) el listener en vivo de tiendas activas.
  // Se llama desde AppState.setCurrentUser justo después de
  // iniciarListeners().
  iniciarListener() {
    if (this._unsubscribeSponsors) return;
    this._unsubscribeSponsors = firebaseServices.db.collection(this.COLECCION)
      .where('activo', '==', true)
      .onSnapshot(snap => {
        ...
        this._activosEnVivo = lista;
      }, error => {
        console.warn('Error en el listener en vivo de tiendas patrocinadoras:', error);
      });
  },

  // Para el listener en vivo. Se llama desde AppState.detenerListeners
  // (logout).
  detenerListener() {
    if (this._unsubscribeSponsors) {
      this._unsubscribeSponsors();
      this._unsubscribeSponsors = null;
    }
    this._activosEnVivo = null;
  },

  // Ya no invalida nada (el listener se mantiene siempre al día solo). Se
  // deja como no-op para no tener que tocar las llamadas existentes en
  // guardar()/eliminar().
  ...
  async getActivos() {
    // 🔥 CAMBIO v1.14: hit de caché en vivo -- instantáneo y sin lectura si
    // iniciarListener() ya recibió su primer snapshot.
    if (this._activosEnVivo) return this._activosEnVivo;
    // Fallback: el listener todavía no ha disparado su primer snapshot (o
    // no se llegó a arrancar). Lectura puntual, igual que antes.
    try {
  ...
  async guardar() {
    ...
    try {
      ...
      this.invalidarCacheActivos(); // 🔥 FIX: la caché de tiendas activas ya no refleja este cambio
...
console.log('✅ sponsors.js v1.14 - getActivos() con caché en vivo (listener onSnapshot), sin TTL');