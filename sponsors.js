...
const Sponsors = {
  ...
  _tarjetaAdminHTML(sp, colorNivelAdmin) {
    ...
    // (toca para llamar a toggleActivo). Ya no lleva insignia ✓ propia --
    // a petición del usuario, quitada porque la tarjeta entera (fondo +
    // borde) ya deja claro que está activa.
    const avatar = `
    ...
    `;
    return `
      <div style="background:${activo ? colorNivelAdmin + '33' : 'var(--bg-secondary)'}; border:${activo ? '2px' : '1px'} solid ${activo ? colorNivelAdmin : 'var(--border-color)'}; border-radius:14px; padding:12px; margin-bottom:10px; transition:background 0.2s ease, border-color 0.2s ease;">
...
console.log('✅ sponsors.js v1.9 - Panel admin: precarga en segundo plano + caché en memoria (solo recarga de Firestore si hay un cambio real)');