...
const Sponsors = {
  ...
  _tarjetaHTML(sp, compacta = false) {
    ...
    return `
      ...
      <div style="display:flex; align-items:center; justify-content:${descuento ? 'space-between' : 'flex-end'}; gap:10px; margin-top:14px;">
        ${pillDescuento}
        <button onclick="Sponsors.abrir('${sp.id}')" style="flex-shrink:0; padding:${compacta ? '9px 16px' : '11px 20px'}; border-radius:10px; background:var(--gold); color:#0a0a0a; font-weight:700; font-size:${compacta ? '11px' : '12px'}; letter-spacing:0.3px; border:none; cursor:pointer; white-space:nowrap;">IR A LA TIENDA</button>
      </div>
    `;
  },
      ...
      return `
        <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:14px; padding:12px; margin-bottom:10px;">
          <div style="display:flex; align-items:center; gap:12px;">
            ...
            <button onclick="Sponsors.toggleActivo('${sp.id}')" style="flex-shrink:0; border:1px solid ${activo ? 'var(--gold)' : 'var(--border-color)'}; background:${activo ? 'rgba(192,160,96,0.12)' : 'transparent'}; color:${activo ? 'var(--gold)' : 'var(--text-secondary)'}; border-radius:20px; padding:5px 10px; font-size:11px; cursor:pointer; white-space:nowrap;">${activo ? '✅ ACTIVA' : '⛔ INACTIVA'}</button>
          </div>
