...
const Sponsors = {
  ...
  // 🔥 FIX (ronda de optimización Firestore): caché en memoria de getActivos
  // (TTL 8 min). Se llama desde precargarBannerInicio (al arrancar),
  // renderBannerInicio (cada repintado del Dashboard), mostrarAlertaZapatillas
  // (al cruzar el umbral de km) y desde el propio panel admin -- sin caché,
  // cada una de esas llamadas suponía una consulta nueva a Firestore aunque
  // la lista de tiendas activas casi nunca cambia de un minuto a otro.
  // guardar() y eliminar() la invalidan explícitamente.
  _cacheActivos: null,
  _CACHE_ACTIVOS_TTL_MS: 480000,
  ...
  async getActivos() {
    if (this._cacheActivos && (Date.now() - this._cacheActivos.timestamp) < this._CACHE_ACTIVOS_TTL_MS) {
      return this._cacheActivos.data;
    }
    try {
      ...
      this._cacheActivos = { data: lista, timestamp: Date.now() };
      return lista;
    } catch (error) {
