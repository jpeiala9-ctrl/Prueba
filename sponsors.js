...
const Sponsors = {
  ...
  // Se llama desde el botón "Ver oferta →", tanto en el banner del
  ...
  _tarjetaHTML(sp, compacta = false) {
    ...
    return `
      <div style="display:flex; align-items:center; gap:${compacta ? '12px' : '14px'};">
        ...
        <div style="flex:1; min-width:0;">
          ...
          ${descuento ? `<div style="display:inline-flex; margin-top:6px; padding:3px 10px; background:rgba(192,160,96,0.12); border:1px solid rgba(192,160,96,0.35); border-radius:20px; font-size:11px; color:var(--gold); white-space:nowrap;">🏷️ ${descuento}</div>` : ''}
        </div>
      </div>
      <button class="action-button" onclick="Sponsors.abrir('${sp.id}')" style="margin-top:12px; height:44px;">VER OFERTA →</button>
    `;
  },
  ...
  async cargarAdminLista() {
    ...
    container.innerHTML = lista.map(sp => {
      ...
      return `
        <div style="display:flex; align-items:center; gap:12px; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:14px; padding:12px; margin-bottom:10px;">
          ...
          <div style="flex:1; min-width:0;">
            <div style="font-weight:600; font-size:14px; color:var(--text-primary); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${nombre}</div>
            <div style="font-size:11px; color:var(--text-secondary); margin-top:2px;">
              👆 ${sp.clics || 0} clics ${sinEnlace ? ' · <span style="color:#c99ba5;">sin enlace todavía</span>' : ''}
            </div>
          </div>
          <button onclick="Sponsors.toggleActivo('${sp.id}')" style="border:1px solid ${activo ? 'var(--gold)' : 'var(--border-color)'}; background:${activo ? 'rgba(192,160,96,0.12)' : 'transparent'}; color:${activo ? 'var(--gold)' : 'var(--text-secondary)'}; border-radius:20px; padding:5px 10px; font-size:11px; cursor:pointer; white-space:nowrap;">${activo ? '✅ ACTIVA' : '⛔ INACTIVA'}</button>
          <button onclick="Sponsors.abrirFormulario('${sp.id}')" style="border:1px solid var(--border-color); background:transparent; color:var(--text-secondary); border-radius:10px; padding:5px 8px; cursor:pointer;">✏️</button>
          <button onclick="Sponsors.eliminar('${sp.id}')" style="border:1px solid var(--border-color); background:transparent; color:var(--text-secondary); border-radius:10px; padding:5px 8px; cursor:pointer;">🗑️</button>
        </div>
      `;
    }).join('');
  },
