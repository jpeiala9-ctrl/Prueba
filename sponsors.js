      ...
      return `
        <div style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:14px; padding:12px; margin-bottom:10px;">
          <div style="display:flex; align-items:center; gap:12px; max-width:100%;">
            ...
            <button onclick="Sponsors.toggleActivo('${sp.id}')" style="flex-shrink:0; box-sizing:border-box; display:inline-flex; align-items:center; gap:4px; border:1px solid ${activo ? 'var(--gold)' : 'var(--border-color)'}; background:${activo ? 'rgba(192,160,96,0.12)' : 'transparent'}; color:${activo ? 'var(--gold)' : 'var(--text-secondary)'}; border-radius:20px; padding:4px 9px; font-size:10px; font-weight:600; line-height:1.2; cursor:pointer; white-space:nowrap; -webkit-text-size-adjust:100%; text-size-adjust:100%;"><span style="width:6px; height:6px; border-radius:50%; flex-shrink:0; background:${activo ? 'var(--gold)' : 'var(--text-secondary)'};"></span>${activo ? 'ACTIVA' : 'INACTIVA'}</button>
          </div>
...
console.log('✅ sponsors.js v1.2 - Botón ACTIVA/INACTIVA más compacto + tinte dorado en el banner del Dashboard');