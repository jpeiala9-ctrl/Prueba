...
const Wall = {
  ...
  async cargarMuro() {
    ...
    try {
      ...
      try {
        const uids = [...new Set(nuevasEntries.map(e => e.userId).filter(Boolean))];
        const niveles = (typeof Friends !== 'undefined' && Friends.getNivelesDirectos)
          ? await Friends.getNivelesDirectos(uids)
          : {};
        nuevasEntries.forEach(e => { e._nivel = niveles[e.userId] || 1; });
      } catch (e) {
        console.warn('Error obteniendo niveles del muro:', e);
      }

      // El refresco automático (iniciarActualizacionPeriodica, cada 30s)
      // llama a cargarMuro() aunque no haya ninguna publicación/like
      // nuevo. Antes eso reconstruía SIEMPRE la lista entera -- destruía y
      // volvía a crear todos los mini-mapas Leaflet (con sus peticiones de
      // teselas) aunque nada hubiera cambiado -- lo que se notaba como un
      // parpadeo periódico del Muro. Ahora, si el contenido es idéntico al
      // ya mostrado, se actualiza el dato en memoria pero se salta el
      // render() y todo el trabajo que conlleva.
      this.currentEntries = nuevasEntries;
      if (!this._entriesIguales(this._ultimasEntriesRenderizadas, nuevasEntries)) {
        this._ultimasEntriesRenderizadas = nuevasEntries;
  ...
  // Compara dos tandas de publicaciones del Muro para saber si el render
  // saldría IGUAL a lo que ya hay en pantalla -- solo mira lo que
  // realmente afecta al HTML/mini-mapas: qué publicaciones hay y en qué
  // orden, sus likes, y si ya tienen mini-mapa cacheado (mapSnapshot).
  // Deliberadamente NO compara todo el objeto (fechas, TSS, etc. no
  // cambian nunca tras publicarse), así que una diferencia ahí no puede
  // dar un falso "distinto".
  _entriesIguales(a, b) {
    if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length) return false;
    for (let i = 0; i < a.length; i++) {
      if (a[i].id !== b[i].id) return false;
      if ((Number(a[i].likeCount) || 0) !== (Number(b[i].likeCount) || 0)) return false;
      if (!!a[i].mapSnapshot !== !!b[i].mapSnapshot) return false;
    }
    return true;
  },
  ...
  render(entries) {
    ...
    // Eventos de likes y clics. No hace falta removeEventListener antes de
    // añadir: this.container.innerHTML = html (arriba) ya sustituyó todo
    // el DOM anterior por elementos nuevos, así que los listeners viejos
    // desaparecieron con ellos solos -- intentar quitarlos aquí era una
    // llamada que nunca podía coincidir con nada (bind() crea una función
    // nueva cada vez) sobre elementos que ya no existían.
