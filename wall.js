// ==================== wall.js ====================
// Versión: 4.19 - CACHÉ EN VIVO, parte "Muro": (1) likes sin parpadeo --
//                iniciarListener() ahora distingue "solo cambiaron los
//                likes" (mismos ids/orden, ver _mismosIdsEnMismoOrden) de
//                un cambio de verdad: en ese caso solo se actualizan en el
//                sitio el contador `.like-count` y la clase `liked` del
//                botón de cada tarjeta afectada, sin destruir ni recrear
//                ninguna tarjeta ni sus mini-mapas (antes caía siempre al
//                render() completo, que es lo que causaba el parpadeo al
//                dar/quitar un like). (2) Fotos que "aparecen tarde": se
//                añade _precargarAvatares(entries), que precarga con
//                `new Image()` todos los photoURL únicos ANTES de tocar el
//                DOM -- se llama desde render() y cargarMas() justo antes
//                de construir el HTML. El contenido anterior se queda en
//                pantalla mientras se precarga (sin hueco en blanco), y
//                cuando se reemplaza, las fotos ya están listas para
//                pintarse al instante. render() pasa a ser async por este
//                motivo (ningún llamante lo esperaba de forma síncrona).
// Versión: 4.18 - FIX "parpadeo al entrar en el Muro" (de verdad esta vez):
//                app.js llama a Wall.precargarMuro() durante el arranque
//                (junto al banner de tienda, ver app.js::precargarDatos)
//                desde la v4.63 de app.js -- pero esa función NUNCA
//                existió en este archivo. La comprobación `if
//                (Wall.precargarMuro)` de app.js fallaba en silencio, así
//                que el muro nunca se precargaba de verdad y el usuario
//                seguía viendo el hueco vacío + aparición de golpe al
//                entrar en Comunidad → Muro. Se añade la función que
//                faltaba: engancha ya el listener en tiempo real desde el
//                arranque y espera a su primer snapshot (con un timeout de
//                seguridad de 4s). Para no crear los mini-mapas de Leaflet
//                sobre un contenedor todavía oculto (display:none no tiene
//                tamaño real -> mapa mal dimensionado, el mismo problema
//                que ya resolvió la v4.16 por otro motivo), el propio
//                listener comprueba si el contenedor es visible de verdad
//                (offsetParent) antes de pintar: si la precarga se
//                adelantó con la pestaña aún oculta, currentEntries se deja
//                listo en memoria y es init() quien pinta de verdad en
//                cuanto el usuario entra, con el contenedor ya visible.
// Versión: 4.17 - A petición del usuario: los números de portal/vivienda
//                de las calles (que en el callejero de OpenStreetMap solo
//                aparecen a partir de cierto nivel de zoom) se veían en
//                el mini-mapa de las tarjetas, sobre todo en carreras
//                cortas o muy encuadradas. Se limita el zoom automático
//                del encuadre (maxZoom:16 en los dos fitBounds) para
//                quedarse siempre por debajo de ese nivel -- los nombres
//                de las calles se siguen viendo con normalidad. Mismo
//                límite que gps-track-viewer.js v2.10.
// Versión: 4.16 - Dos ajustes a petición del usuario: (1) las miniaturas
//                de las tarjetas usaban todavía el tile layer viejo de
//                CartoDB ('basemaps.cartocdn.com'), que exige una clave de
//                API que no se tiene -- sin ella los mosaicos se veían
//                tapados por una marca de agua enorme "API KEY REQUIRED"
//                (eso era el "medio mapa sin pintar"). Se cambia al mismo
//                tile layer de OpenStreetMap que ya usan gps-tracker.js y
//                gps-track-viewer.js, para que las tres vistas (resumen al
//                terminar, visor de una sesión, y mini-mapa en las
//                tarjetas) se vean exactamente igual. Como el mapSnapshot
//                que se cachea para Perfil es una captura de ESTE mismo
//                mapa, esto también arregla las miniaturas de Perfil para
//                las carreras que se guarden desde ahora (las que ya
//                tenían un mapSnapshot cacheado de antes conservan la
//                imagen vieja hasta que se vuelva a generar). (2) FIX
//                "parpadeo al entrar en el Muro": init() hacía
//                detenerListener()+iniciarListener() sin condición cada
//                vez que se entraba en la pestaña, tirando el listener que
//                ya estaba activo y forzando un render() completo aunque
//                no hubiera nada nuevo. Ahora solo se abre el listener si
//                no había uno activo, y dentro del propio listener se
//                añade _sonIgualesEntradas() (mismo criterio que la v4.16
//                original de hace unos días: solo id/orden/likes) para no
//                repintar si Firestore reenvía el mismo contenido tras una
//                reconexión.
...
const Wall = {
  ...
  // Funciones en espera de que llegue el PRIMER snapshot del listener del
  // muro (ver precargarMuro() e iniciarListener() más abajo). No es una
  // caché de datos -- es una lista de "avísame en cuanto sepas algo",
  // vacía la mayor parte del tiempo.
  _resolversPrimerSnapshot: [],
  ...
  init() {
    ...
    // 🔥 FIX "parpadeo al entrar en el Muro": antes, cada vez que se
    // llamaba a init() (es decir, cada vez que se entraba en la pestaña),
    // se hacía detenerListener() + iniciarListener() sin condición -- eso
    // tira el listener en tiempo real que ya estaba activo y abre uno
    // nuevo, que dispara su propio snapshot inicial y un render() completo
    // aunque el contenido no haya cambiado ni un poco desde la última
    // visita. Ahora solo se abre el listener si no había uno ya activo
    // (iniciarListener() ya se protege solo con `if (this._unsubscribe)
    // return`); si ya estaba activo, se deja tal cual -- las tarjetas que
    // ya estaban en pantalla se quedan como están, sin parpadeo.
    this.iniciarListener();
    this._iniciarPurgaPeriodica();

    // Si el listener ya estaba activo de una visita anterior (por eso no se
    // ha vuelto a abrir arriba) pero este contenedor concreto está vacío
    // -- por ejemplo, la primerísima vez que se pinta esta pestaña en la
    // sesión -- se repinta con los datos que ya se tenían, sin esperar a
    // que llegue un cambio nuevo de Firestore (que podría no llegar en
    // horas si nadie publica ni da like). Este es el caso normal en el
    // arranque: precargarMuro() ya dejó currentEntries listo desde antes
    // (con la pestaña todavía oculta, sin pintar mini-mapas todavía -- ver
    // iniciarListener()), y es AHORA, con el contenedor ya visible de
    // verdad, cuando se pinta por primera vez.
    if (this.container.children.length === 0 && this.currentEntries.length > 0) {
  ...
  iniciarListener() {
    ...
    this._unsubscribe = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        if (this.currentEntries.length > 20) { this._resolverPrimerSnapshot(); return; }
        try {
          const nuevasEntries = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

          // 🔥 FIX "parpadeo al entrar en el Muro" (2ª parte): si las
          // entradas visibles no han cambiado de verdad (mismos ids, mismo
          // orden, mismos contadores de likes) respecto a lo que ya había
          // en pantalla, no se toca el DOM ni se reinicializa ningún mapa.
          // Sin esto, cualquier motivo por el que Firestore reenvíe el
          // mismo snapshot (una reconexión tras volver de segundo plano,
          // por ejemplo) forzaba un render() completo -- destruir y
          // recrear todas las tarjetas y sus mini-mapas -- aunque no
          // hubiera nada nuevo que mostrar.
          const hayAlgoPintado = !!this.container && this.container.children.length > 0;
          if (hayAlgoPintado && this._sonIgualesEntradas(this.currentEntries, nuevasEntries)) {
            this.currentEntries = nuevasEntries;
            this.hayMasEntradas = snapshot.docs.length === 20;
            return;
          }

          // 🔥 CAMBIO v4.19 (likes sin parpadeo): si las entradas siguen
          // siendo las mismas y en el mismo orden que las de pantalla (solo
          // ha cambiado algún likeCount/likes -- el caso típico de "alguien
          // ha dado like"), se actualiza en el sitio SOLO el contador y la
          // clase 'liked' de cada botón afectado, sin destruir/recrear
          // ninguna tarjeta ni sus mini-mapas. Antes esto caía al render()
          // completo de más abajo, que es lo que causaba el parpadeo al
          // dar/quitar un like.
          if (hayAlgoPintado && this._mismosIdsEnMismoOrden(this.currentEntries, nuevasEntries)) {
            nuevasEntries.forEach(entry => {
          ...
          // 🔥 FIX "parpadeo al entrar en el Muro" (3ª parte): si este
          // snapshot llega mientras la pestaña Muro todavía está oculta
          // (p.ej. porque precargarMuro() adelantó el listener durante el
          // arranque, antes de que el usuario haya entrado en Comunidad),
          // NO se pinta todavía. Un contenedor con display:none no tiene
          // tamaño real, y crear los mini-mapas de Leaflet sobre él sale
          // mal (medio mapa negro, ver _crearMiniMapa) -- currentEntries
          // ya se queda listo igualmente, y es init() quien lo pinta en
          // cuanto la pestaña sea de verdad visible, con su tamaño ya
          // asentado.
  ...
  _resolverPrimerSnapshot() {
    if (!this._resolversPrimerSnapshot.length) return;
    const resolvers = this._resolversPrimerSnapshot;
    this._resolversPrimerSnapshot = [];
    resolvers.forEach(fn => fn());
  },

  // Se llama durante el arranque de la app (ver AppState.precargarDatos en
  // app.js -- junto al banner de tienda, esperado con Promise.all antes de
  // ocultar el splash). Engancha ya el listener en tiempo real del muro
  // (iniciarListener() se protege sola contra duplicados, así que cuando
  // el usuario entre de verdad en la pestaña no se abre un segundo
  // listener) y espera a que llegue su primer snapshot. Así, cuando el
  // usuario toque Comunidad → Muro, init() encuentra currentEntries ya
  // listo -- sin ningún hueco vacío ni parpadeo de "cargando tarjetas".
  //
  // ⚠️ Esta función faltaba en el archivo (app.js ya la llamaba,
  // protegida con `if (Wall.precargarMuro)`, desde la v4.63 -- pero como
  // el nombre no existía de verdad aquí, esa comprobación fallaba en
  // silencio y el muro nunca se precargaba: ESTE era el parpadeo que se
  // seguía viendo al entrar).
  precargarMuro() {
    return new Promise((resolve) => {
      if (!this.container) this.container = document.getElementById('wallContainer');
      let resuelto = false;
      const terminar = () => { if (!resuelto) { resuelto = true; resolve(); } };
      this._resolversPrimerSnapshot.push(terminar);
      this.iniciarListener();
      // Salvaguarda: una red muy lenta no debe retrasar el arranque de la
      // app para siempre (además, el splash ya tiene su propio timeout
  ...
  // Compara dos listas de entradas del Muro para saber si de verdad hace
  // falta repintar (ver el FIX "parpadeo al entrar en el Muro" en
  // iniciarListener()). Solo id, orden y contadores de likes -- es
  // deliberadamente ligera, y basta para decidir "no ha cambiado nada
  // visible".
  _sonIgualesEntradas(a, b) {
    if (!a || !b) return false;
    if (a.length !== b.length) return false;
    for (let i = 0; i < a.length; i++) {
      if (a[i].id !== b[i].id) return false;
      if ((a[i].likeCount || 0) !== (b[i].likeCount || 0)) return false;
      if ((a[i].likes?.length || 0) !== (b[i].likes?.length || 0)) return false;
    }    return true;
  },

  // 🔥 CAMBIO v4.19 (likes sin parpadeo): comparación más ligera que
  // _sonIgualesEntradas -- solo mira id y orden, IGNORANDO likeCount/likes
  // a propósito. Si esto da true pero _sonIgualesEntradas dio false (ver
  // iniciarListener()), significa que lo único que ha cambiado es algún
  // like, y basta con parchear los botones afectados en vez de repintar
  // toda la tarjeta.
  _mismosIdsEnMismoOrden(a, b) {
    if (!a || !b) return false;
    if (a.length !== b.length) return false;
    for (let i = 0; i < a.length; i++) {
      if (a[i].id !== b[i].id) return false;
  ...
  async _crearMiniMapa(containerId, trackPoints, entryId = null, esPropia = false) {
    ...
    // Tile layer de OpenStreetMap (el mismo que GPSTrackViewer y el propio
    // GPSTracker en la pantalla de resumen de la carrera -- antes este
    // mini-mapa usaba CartoDB, que empezó a exigir una clave de API: sin
    // ella, los mosaicos se veían tapados por una marca de agua enorme
    // "API KEY REQUIRED", lo que se veía como el mapa medio pintado).
    // crossOrigin:true sigue haciendo falta aquí (a diferencia de
    // GPSTrackViewer) para poder "leer" luego el mapa como imagen (canvas)
    // y cachearlo -- sin esto el navegador bloquea la lectura del canvas
    // por seguridad (CORS), aunque el mapa se vea perfectamente en pantalla.
    ...
    map.invalidateSize();
    map.fitBounds(polyline.getBounds(), { padding: [10, 10], maxZoom: 16 });
  ...
  async cargarMas() {
    ...
    try {
      ...
      // 🔥 CAMBIO v4.19: mismo motivo que en render() -- precargar antes
      // de insertar, para que las fotos de las tarjetas nuevas no salten.
      await this._precargarAvatares(nuevas);
  ...
  // 🔥 CAMBIO v4.19 (fotos del muro "aparecen tarde"): precarga con
  // `new Image()` todos los avatares (photoURL) únicos de las entradas
  // ANTES de insertar el HTML de las tarjetas, para que el navegador ya
  // tenga la imagen en su caché cuando el <img> real se inserte en el DOM
  // -- así se evita que la foto aparezca con un salto/retraso visible
  // justo después de la tarjeta. Si una imagen tarda demasiado o falla, no
  // bloquea a las demás: cada una se resuelve como máximo a los 3s.
  async _precargarAvatares(entries) {
    const urls = [...new Set((entries || []).map(e => e.photoURL).filter(Boolean))];
  ...
  async render(entries) {
    ...
    // 🔥 CAMBIO v4.19: se precargan los avatares ANTES de tocar el DOM --
    // el contenido anterior se queda en pantalla mientras tanto (nada de
    // hueco en blanco), y cuando por fin se reemplaza, las fotos ya están
    // listas para pintarse al instante.
    await this._precargarAvatares(entries);
...
window.Wall = Wall;
console.log('✅ Wall v4.19 - likes sin parpadeo (patch en el sitio) + precarga de avatares antes de pintar');