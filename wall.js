...
const Wall = {
          ...
          <div class="wall-item" data-entry-id="${entry.id}">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
              ...
              <button class="wall-like-btn ${likeClass}" data-entry-id="${entry.id}"
                style="position:relative; background:transparent; border:none; padding:6px 12px; border-radius:20px; cursor:pointer; display:inline-flex; align-items:center; gap:6px; font-size:14px; color:var(--text-secondary); transition:all 0.2s ease;">
    ...
    for (const entry of entries) {
      if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
        ...
        if (tapOverlay) {
          ...
          // Leaflet, y se engancha ya (no hace falta esperar a que el mapa
          // termine de crearse). Se usa bindTap (basado en touchend) en vez
          // de un 'click' normal: en móvil, un simple click a veces necesita
          // un primer toque "de calentamiento" antes de disparar de verdad.
  ...
  _createLikesModal(users) {
    ...
    for (const user of users) {
      ...
      userItem.style.cssText = 'display:flex; align-items:center; gap:10px; padding:8px; border-radius:16px; background:var(--bg-secondary); cursor:pointer; transition:background 0.2s;';
      userItem.innerHTML = `
        ${avatarHTML}
        ...
        <button class="view-profile-btn" data-uid="${user.uid}"
          style="background:var(--zone-2); border:none; padding:5px 9px; border-radius:16px; color:var(--bg-primary); cursor:pointer; flex-shrink:0; white-space:nowrap; font-size:11px; line-height:1.2;">
          Ver perfil
        </button>
      `;
