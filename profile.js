// ==================== profile.js ====================
// Versión: 10.12 - La EDAD ya no se escribe a mano: en Editar perfil se
//                pide la FECHA DE NACIMIENTO y la edad se calcula sola a
//                partir de ella (se refresca en cada visita, así que sube
//                sola en el cumpleaños). El campo EDAD de la calculadora
//                de zonas se rellena y bloquea solo cuando ya se conoce la
//                edad, así no hay que volver a escribirla ahí tampoco.
//                Perfiles antiguos sin fecha de nacimiento siguen usando
//                su número de edad guardado hasta que la rellenen.
...
const Profile = {
  ...
  // 🔥 FIX (ronda de optimización Firestore): caché en memoria de
  // "mis últimos entrenamientos" (TTL 60s) + deduplicación de promesa en
  // vuelo -- ver _actualizarMisEntrenamientos() más abajo.
  _misEntrenamientosCache: null, // { uid, snapshot, timestamp }
  _misEntrenamientosPromiseEnVuelo: null,
  _MIS_ENTRENAMIENTOS_TTL_MS: 60000,

  // Invalida la caché de "mis últimos entrenamientos": se llama al marcar
  // o desmarcar una sesión (calendar.js), para que la próxima vez que se
  // pinte el Perfil se traiga la lista real actualizada en vez de la
  // versión cacheada de hace hasta 60s.
  invalidarCacheMisEntrenamientos() {
    this._misEntrenamientosCache = null;
  },
  ...
  // 🔥 v294: EDAD A PARTIR DE FECHA DE NACIMIENTO
  // Calcula la edad actual a partir de una fecha de nacimiento (string
  // 'YYYY-MM-DD', el formato nativo de <input type="date">). Se recalcula
  // en caliente cada vez que hace falta (no se "congela" en ningún sitio),
  // así que en cuanto pasa el cumpleaños del usuario, la próxima vez que
  // abra la app la edad ya sale actualizada sin que nadie tenga que tocar
  // nada.
  _calcularEdad(birthDateStr) {
    if (!birthDateStr) return null;
    const nacimiento = new Date(birthDateStr + 'T00:00:00');
  ...
  // Edad "real" de un perfil: si tiene fecha de nacimiento, se calcula al
  // vuelo a partir de ella; si no (perfiles de antes de este cambio, que
  // solo tenían el número de edad suelto guardado a mano), se usa ese
  // número tal cual hasta que el usuario rellene su fecha de nacimiento en
  // Editar perfil -- a partir de ahí, la fecha manda siempre.
  _edadDePerfil(profile) {
    if (!profile) return null;
    if (profile.birthDate) {
      const edad = this._calcularEdad(profile.birthDate);
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      // 🔥 FIX: FieldPath.documentId() en vez del string suelto '__name__'
      // (ver app.js > Sesiones hoy / friends.js > _limpiarAmigosHuérfanos).
      // Además, si esta comprobación falla por lo que sea, se deja
      // friendIds tal cual en vez de dar por hecho que esos amigos ya no
      // existen -- así un fallo puntual de red no acaba borrando amigos
      // reales de Firestore.
      try {
      ...
      const profile = userData.profile || {
        bio: '', city: '', age: null, birthDate: null, gender: '', weight: null, height: null,
        privacySettings: { showTrainings: 'friends', showProfile: 'public' },
        photoURL: null
      };

      // 🔥 v294: edad calculada a partir de la fecha de nacimiento (o la
      // edad suelta antigua si aún no hay fecha guardada). Si hay fecha de
      // nacimiento y la edad calculada ya no coincide con la que había
      // guardada (típicamente porque ha pasado un cumpleaños desde la
      // última vez que se guardó), se actualiza sola en Firestore -- sin
      // esperar (no bloquea el pintado) y sin que el usuario tenga que
      // volver a abrir "Editar perfil" para que se refleje.
      const edadCalculada = this._edadDePerfil(profile);
      if (profile.birthDate && edadCalculada !== profile.age) {
        userRef.update({ 'profile.age': edadCalculada }).catch(e => console.warn('No se pudo refrescar la edad calculada:', e));
        firebaseServices.db.collection('perfilSocial').doc(AppState.currentUserId).set({ age: edadCalculada }, { merge: true }).catch(e => console.warn('No se pudo refrescar la edad calculada (perfilSocial):', e));
        profile.age = edadCalculada;
        if (AppState.currentUserData?.profile) AppState.currentUserData.profile.age = edadCalculada;
      }

      // 🔥 v294: si ya se conoce la edad (por fecha de nacimiento o, para
      // perfiles antiguos, por el número suelto), se precarga y se
      // bloquea el campo EDAD de la calculadora de zonas -- ya no hace
      // falta volver a escribirla cada vez que se calculan zonas nuevas.
      // Si todavía no se conoce (perfil nuevo sin rellenar), se deja
      // editable como siempre.
      const ageCalcInput = document.getElementById('age');
      const ageCalcNote = document.getElementById('ageAutoNote');
      if (ageCalcInput) {
        if (edadCalculada) {
          ageCalcInput.value = edadCalculada;
          ageCalcInput.readOnly = true;
          ageCalcInput.style.opacity = '0.75';
          ageCalcInput.style.cursor = 'not-allowed';
          if (ageCalcNote) ageCalcNote.style.display = 'block';
        } else {
          ageCalcInput.readOnly = false;
          ageCalcInput.style.opacity = '';
          ageCalcInput.style.cursor = '';
          if (ageCalcNote) ageCalcNote.style.display = 'none';
        }
        if (window.AppState && AppState.actualizarBotonCalcular) AppState.actualizarBotonCalcular();
      }
      ...
      // 🔥 v10.14: reparación automática y silenciosa (UNA sola vez por
      // usuario, ver Gamification.repararTodoDesdeHistorialSiHaceFalta):
      // la primera vez que este usuario entra a Perfil tras la
      // actualización, se recalculan desde la verdad la distancia total,
      // el nivel, la racha, los récords y los km de la zapatilla actual,
      // por si arrastraban alguna desviación histórica de antes del fix
      // de v5.17/5.18. Sin botón ni aviso: si no hacía falta (ya reparado,
      // o el usuario es nuevo), no hace nada.
      if (gamificationData && !gamificationData.historialRecalculadoV519) {
        try {
          const reparado = await Gamification.repararTodoDesdeHistorialSiHaceFalta(AppState.currentUserId, gamificationData);
          if (reparado) gamificationData = await Gamification.getData(AppState.currentUserId);
        } catch (e) { console.warn('Error en la reparación automática de gamificación:', e); }
      }
      ...
      const age = edadCalculada ? Utils.escapeHTML(edadCalculada + ' años') : '—';
      ...
      if (gamificationData) {
        ...
        // 🔥 FIX (ronda de optimización Firestore): gamificationData ya trae
        // el doc completo de gamification/{uid} (cargado arriba) -- pedir la
        // zapatilla con Gamification.getCurrentShoe() volvía a leer ese
        // mismo documento otra vez (o repetía el hit de caché sin necesidad).
        const shoe = gamificationData.currentShoe;
  ...
  async _actualizarMisEntrenamientos() {
    ...
    let snapshot;
    const uid = AppState.currentUserId;
    // 🔥 FIX: caché en memoria (TTL 60s) + deduplicación de promesa en
    // vuelo. cargarPerfil() puede repintarse varias veces seguidas en poco
    // tiempo (cambios de bio/zapatilla/foto/nivel) y cada repintado llamaba
    // a esta consulta de nuevo aunque los últimos 5 entrenamientos casi
    // nunca hubieran cambiado en ese margen.
    const cache = this._misEntrenamientosCache;
    if (cache && cache.uid === uid && (Date.now() - cache.timestamp) < this._MIS_ENTRENAMIENTOS_TTL_MS) {
      snapshot = cache.snapshot;
    } else if (this._misEntrenamientosPromiseEnVuelo) {
      try {
        snapshot = await this._misEntrenamientosPromiseEnVuelo;
      } catch (error) {
        console.warn('Error cargando mis últimos entrenamientos:', error);
        return;
      }
    } else {
      this._misEntrenamientosPromiseEnVuelo = firebaseServices.db
      ...
      try {
        snapshot = await this._misEntrenamientosPromiseEnVuelo;
        this._misEntrenamientosCache = { uid, snapshot, timestamp: Date.now() };
      } catch (error) {
        console.warn('Error cargando mis últimos entrenamientos:', error);
        return;
      } finally {
        this._misEntrenamientosPromiseEnVuelo = null;
      }
    }
  ...
  _mostrarModalCambiarZapatilla() {
    ...
    modal.innerHTML = `
      ...
      <div style="margin-bottom: 16px;">
        ...
        <input type="text" id="newShoeBrand" placeholder="Ej. Nike" style="width: 100%; padding: 10px; border-radius: 10px; background: var(--bg-primary); border: 1px solid var(--border-color); color: var(--text-primary); text-align:center;">
      </div>
      <div style="margin-bottom: 24px;">
        <label style="display: block; text-align: left; font-size: 12px; color: var(--text-secondary); margin-bottom: 4px;">Modelo</label>
        <input type="text" id="newShoeModel" placeholder="Ej. Pegasus 40" style="width: 100%; padding: 10px; border-radius: 10px; background: var(--bg-primary); border: 1px solid var(--border-color); color: var(--text-primary); text-align:center;">
      </div>
  ...
  cargarDatosEnModal() {
    ...
    try {
      ...
      // Perfiles antiguos que aún no tienen fecha de nacimiento guardada
      // (solo el número de edad suelto) verán este campo vacío hasta que
      // la rellenen una vez -- a partir de ahí la fecha manda siempre.
  ...
  async seleccionarFoto() {
    ...
    input.onchange = async (e) => {
      ...
      Utils.showLoading();
      try {
        // 🔥 FIX (punto menor [15] de la ronda de optimización): el objetivo
        // de compresión era "hasta 5 MB", pero storage.rules exige
        // request.resource.size < 2 MB (ver storage.rules ->
        // profile_pictures). En la práctica casi nunca fallaba porque el
        // resultado solía quedar bastante por debajo de 5 MB, pero una foto
        // muy ruidosa/grande podía comprimir a algo entre 2 y 5 MB y la
        // subida se rechazaba silenciosamente por la regla. Se alinea el
        // objetivo del código con el límite real de la regla (con margen:
        // 1.8 MB en vez de 2 MB justos, para no quedar al filo del corte).
        const compressedFile = await this.compressImageToTarget(file, 1920, 1.8 * 1024 * 1024);
        const url = await Storage.uploadProfilePicture(AppState.currentUserId, compressedFile);
        if (url) {
          // 🔥 FIX toast contradictorio: la foto ya está subida y guardada
          // en este punto -- lo que sigue (refrescar perfil/amigos/badge)
          // es solo housekeeping. Antes iba en el mismo try: si fallaba,
          // se veía "Error al procesar la imagen" aunque la foto SÍ se
          // hubiera subido y guardado bien.
          ...
          try {
            ...
            console.error('Error refrescando UI tras actualizar la foto:', refreshErr);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  async guardarPerfil() {
    ...
    try {
      ...
      // 🔥 v294: la edad ya no se escribe a mano -- se calcula a partir de
      // la fecha de nacimiento en el momento de guardar (y se recalcula
      // sola en cada visita, ver cargarPerfil). Se valida el rango 14-85
      // sobre la edad resultante, igual que antes se validaba el número
      // escrito a mano. Si se deja el campo vacío, se borra también la
      // edad (perfil "sin edad" hasta que se rellene la fecha).
      const age = birthDate ? this._calcularEdad(birthDate) : null;
      if (birthDate && age === null) {
        Utils.showToast('⚠️ Fecha de nacimiento no válida', 'error');
        Utils.hideLoading();
        return;
      }
      if (birthDate && new Date(birthDate + 'T00:00:00') > new Date()) {
        Utils.showToast('⚠️ La fecha de nacimiento no puede ser futura', 'error');
        Utils.hideLoading();
        return;
      }
      ...
      // 🔥 FIX toast contradictorio: el perfil ya está guardado en
      // Firestore en este punto -- refrescar la vista y cerrar el modal
      // es solo housekeeping. Antes iba en el mismo try: si el refresco
      // fallaba, se veía "Error al guardar perfil" aunque SÍ se hubiera
      // guardado bien.
      try {
        await this.cargarPerfil(true);
        this.cerrarModal();
      } catch (refreshErr) {
        console.error('Error refrescando UI tras guardar el perfil:', refreshErr);
      }
    } catch (error) {
...
console.log('✅ profile.js v10.12 - Edad calculada desde fecha de nacimiento (ya no se escribe a mano)');