...
const Profile = {
        ...
        if (recordReciente) {
          ...
          recordRecienteHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid var(--border-color);">
              ...
              <strong style="font-size:16px; color:var(--level-color);">${Utils.formatTime(recordReciente.durationMs, true)}</strong>
            </div>`;
        } else {
        ...
        html += `
          <div class="records-card" onclick="Profile.abrirModalRecords()" role="button" tabindex="0" style="margin-top:16px; border:1px solid var(--border-color); border-radius:20px; padding:16px 20px; background:var(--bg-secondary); cursor:pointer;">
            ...
            <div style="display:flex; justify-content:space-between; gap:8px; text-align:center; margin-bottom:16px;">
              <div style="flex:1;">
                <div style="font-size:18px; font-weight:600; color:var(--level-color);">${bestStreak}</div>
              ...
              <div style="flex:1; border-left:1px solid var(--border-color);">
                <div style="font-size:18px; font-weight:600; color:var(--level-color);">${maxDistSingle.toFixed(1)}</div>
                <div style="font-size:9px; color:var(--text-secondary); text-transform:uppercase;">km en una sesión</div>
              </div>
              <div style="flex:1; border-left:1px solid var(--border-color);">
                <div style="font-size:18px; font-weight:600; color:var(--level-color);">${bestPaceGlobal ? Utils.formatTime(bestPaceGlobal * 60, false) : '--:--'}</div>
  ...
  abrirModal() {
    ...
    // Reset también de la PÁGINA (no solo del modal): este formulario
    // tiene varios inputs (bio, ciudad, edad, peso, altura) y en móvil,
    // al tocar un campo más abajo del todo, el teclado virtual empuja y
    // el navegador desplaza la página entera para mantener el campo
    // visible -- ese desplazamiento de la página se queda ahí si luego
    // cierras y reabres el modal SIN cambiar de pestaña (el reset de
    // scroll al cambiar de pestaña, forzarScrollTop(), no se dispara
    // aquí porque sigues en la misma pestaña Perfil).
  ...
  async abrirModalRecords() {
    ...
    modal.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 style="margin:0; color:var(--level-color);">🏆 RÉCORDS PERSONALES</h3>
      </div>
      ...
      const filasRecord = Gamification.RECORD_DISTANCES.map(d => {
        ...
        return `
          <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid var(--border-color);">
            ...
            <strong style="font-size:16px; color:var(--level-color);">${Utils.formatTime(r.durationMs, true)}</strong>
          </div>`;
      }).join('');
