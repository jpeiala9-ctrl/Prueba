// ==================== profile.js ====================
// Versión: 11.1 - SOLO ARREGLO DE EVENTOS
//                 - Mantiene el HTML original exacto
//                 - Solo corrige los listeners
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      // --- CONSTRUIR HTML (EXACTAMENTE IGUAL QUE EL ORIGINAL) ---
      ...
      // Gamificación - MANTENIENDO EL MISMO HTML ORIGINAL
      if (gamificationData) {
      ...
      // --- MIS ÚLTIMOS ENTRENAMIENTOS (EXACTAMENTE EL HTML ORIGINAL) ---
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // HTML EXACTAMENTE IGUAL AL ORIGINAL
            entrenamientosHTML += `
      ...
      // --- RENDERIZAR ---
      // Si ya se pintó desde caché, no se vuelve a escribir el DOM
  ...
  // ================================================================
  //  UN SOLO DELEGADO PARA TODOS LOS EVENTOS
  // ================================================================
  _handleContainerClick(e) {
    const target = e.target;
    
    // --- 1. BOTÓN CAMBIAR ZAPATILLA (por ID) ---
    ...
    // --- 2. BOTÓN HISTORIAL ZAPATILLA (por ID) ---
  ...
  // ================================================================
  //  MOSTRAR LIKES - CON TOAST SI NO HAY LIKES
  // ================================================================
  ...
  _crearModalLikes(users) {
    ...
    let html = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding-bottom:12px; border-bottom:1px solid var(--border-color);">
        ...
        <button id="closeLikesModalProfileBtn" style="background:transparent; border:none; font-size:28px; cursor:pointer; color:var(--text-secondary);">&times;</button>
      </div>
    ...
    for (const user of users) {
      ...
      html += `
        <div class="like-user-item" data-uid="${user.uid}" style="display:flex; align-items:center; gap:12px; padding:8px 12px; border-radius:12px; background:var(--bg-secondary); cursor:pointer;">
          ${avatarHTML}
          <div style="flex:1;">
          ...
          <button class="ver-perfil-desde-like" data-uid="${user.uid}" style="background:var(--zone-2); border:none; padding:4px 12px; border-radius:20px; color:var(--bg-primary); cursor:pointer; font-size:12px;">Ver perfil</button>
        </div>
      `;
    }
  ...
  // ================================================================
  //  EDITAR PERFIL (MODALES) - SIN CAMBIOS
  // ================================================================
  abrirModal() {
...
console.log('✅ profile.js v11.1 - SOLO ARREGLO DE EVENTOS, diseño original intacto');