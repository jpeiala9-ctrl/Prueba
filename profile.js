...
const Profile = {
        ...
        html += `
          <div class="records-card" onclick="Profile.abrirModalRecords()" role="button" tabindex="0" style="margin-top:16px; border:1px solid var(--border-color); border-radius:20px; padding:16px 20px; background:var(--bg-secondary); cursor:pointer;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
              ...
              <div style="font-size:10px; color:var(--gold);">Ver detalle ›</div>
            </div>
  ...
  _createLikesModal(users) {
    ...
    modal.style.cssText = `background:var(--bg-primary); border-radius:24px; max-width:500px; width:90%; max-height:80vh; overflow-y:auto; overflow-x:hidden; box-sizing:border-box; padding:20px; box-shadow:0 10px 30px rgba(0,0,0,0.3); border:1px solid var(--border-color);`;
  ...
  async abrirModalRecords() {
    ...
    document.getElementById('recordsModalOverlay')?.remove();
    document.getElementById('recordsModal')?.remove();
    ...
    `;
    modal.innerHTML = `
      <h3 style="margin-top:0; text-align:center; color:var(--gold);">🏆 RÉCORDS PERSONALES</h3>
      <div id="recordsModalContenido"><p style="text-align:center; color:var(--text-secondary); padding:20px 0;">Cargando…</p></div>
      <button class="action-button" onclick="Profile.cerrarModalRecords()" style="margin-top:16px;">CERRAR</button>
    `;
    ...
    try {
      ...
      contenido.innerHTML = `
        ...
        ${filasRecord}
        <p style="text-align:center; font-size:10px; color:var(--text-secondary); margin-top:14px;">Una sesión cuenta como intento a una distancia estándar si se queda dentro de un margen del 15% de esa distancia.</p>
      `;
    } catch (e) {
...
console.log('✅ profile.js v10.7 - Tarjeta de récords compacta + modal de detalle');