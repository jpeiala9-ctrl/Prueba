...
const Profile = {
    ...
    try {
      ...
      setTimeout(() => {
        document.querySelectorAll('.wall-item').forEach(item => {
          item.removeEventListener('click', this._handleItemClickProfile);
          item.addEventListener('click', this._handleItemClickProfile.bind(this));
        });
        document.querySelectorAll('.wall-like-btn').forEach(btn => {
          btn.removeEventListener('click', this._handleLikeClickProfile);
          btn.addEventListener('click', this._handleLikeClickProfile.bind(this));
        });
        document.querySelectorAll('.wall-delete-btn-profile').forEach(btn => {
          btn.removeEventListener('click', this._handleDeleteClickProfile);
          btn.addEventListener('click', this._handleDeleteClickProfile.bind(this));
        });
        ...
        document.querySelectorAll('.badge-icon').forEach(icon => {
          icon.removeEventListener('click', this._mostrarModalInsignias);
          icon.addEventListener('click', this._mostrarModalInsignias);
        });
  ...
  async _handleDeleteClickProfile(e) {
    e.stopPropagation();
    const btn = e.currentTarget;
    const entryId = btn.dataset.entryId;
    if (!entryId || !AppState.currentUserId) return;
    if (!confirm('¿Eliminar este entrenamiento de tu perfil y del muro? No se puede deshacer.')) return;
    try {
      await firebaseServices.db.collection('globalFeed').doc(entryId).delete();
      // Quitarlo del DOM al instante
      const item = document.querySelector(`.wall-item[data-entry-id="${entryId}"]`);
      if (item) item.remove();
      delete this._gpsEntries[entryId];
      // Invalidar la caché local del perfil para que la próxima carga no
      // vuelva a mostrar (ni a comparar como "sin cambios") el entrenamiento
      // ya borrado.
