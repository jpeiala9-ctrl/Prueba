// ==================== profile.js ====================
// Versión: 10.7 - CORREGIDO: Listeners persistentes con event delegation
//                 - Botones cambiar zapatilla e historial con onclick en HTML
//                 - Sesiones del perfil con eventos correctos
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            entrenamientosHTML += `
              <div class="wall-item perfil-sesion-item" data-entry-id="${entryId}" style="margin-bottom:16px;">
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
                  ...
                  <button class="wall-like-btn perfil-like-btn ${likeClass}" data-entry-id="${entryId}" style="background:transparent; border:none; padding:6px 12px; border-radius:20px; cursor:pointer; display:inline-flex; align-items:center; gap:6px; font-size:14px; color:var(--text-secondary); transition:all 0.2s ease;">
      ...
      // ================================================================
      //  EVENT LISTENERS CON DELEGACIÓN (NUNCA SE PIERDEN)
      // ================================================================
      // En lugar de añadir listeners individuales que pueden perderse si
      // el DOM se reemplaza, usamos event delegation en el contenedor.
      // Esto asegura que funcionen siempre, incluso si se recarga el perfil.
      
      // Quitar listeners antiguos para evitar duplicados
      container.removeEventListener('click', this._handlePerfilClick);
      container.removeEventListener('click', this._handleLikeClickPerfil);
      
      // Añadir listener de delegación para clicks en el contenedor
      container.addEventListener('click', this._handlePerfilClick.bind(this));
      container.addEventListener('click', this._handleLikeClickPerfil.bind(this));

      // ================================================================
      //  BOTONES DE ZAPATILLA: asignación directa segura
      // ================================================================
      // En lugar de usar onclick en el HTML (que puede sobrescribirse),
      // buscamos los botones por ID y les asignamos el evento directamente.
      // Se ejecuta siempre después de pintar, así que siempre están.
      setTimeout(() => {
        const changeBtn = document.getElementById('changeShoeBtn');
        if (changeBtn) {
          // Quitar listener antiguo y añadir nuevo
  ...
  // ================================================================
  //  MANEJADOR DE CLICKS POR DELEGACIÓN (SESIONES DEL PERFIL)
  // ================================================================
  _handlePerfilClick(e) {
    const target = e.target.closest('.perfil-sesion-item');
    if (target) {
      const entryId = target.dataset.entryId;
      if (entryId) {
        // Si el clic fue en el mapa, no hacemos nada (el mapa tiene su propio evento)
        if (e.target.closest('.gps-minimap-tap-profile')) return;
        this._mostrarLikesDeEntrenamiento(entryId);
        return;
      }
    }

    // Clic en el mini mapa (abrir visor GPS)
    const mapTarget = e.target.closest('.gps-minimap-tap-profile');
    if (mapTarget) {
      e.stopPropagation();
      const entryId = mapTarget.dataset.entryId;
  ...
  _handleLikeClickPerfil(e) {
    const btn = e.target.closest('.perfil-like-btn');
    if (btn) {
      e.stopPropagation();
  ...
  // ================================================================
  //  MOSTRAR LIKES DE UN ENTRENAMIENTO
  // ================================================================
  ...
  // ================================================================
  //  MODALES DE ZAPATILLAS E INSIGNIAS
  // ================================================================
...
console.log('✅ profile.js v10.7 - Listeners con event delegation, botones de zapatilla corregidos');