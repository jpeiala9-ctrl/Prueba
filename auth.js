...
// Versión: 5.19 - Renderizado desde caché de datos en lugar de HTML
// ====================
...
const Auth = {
  ...
  // === CARGA COMPLETA CON CACHÉ DE DATOS ===
  async _cargarDatosYMostrarApp(uid, email, userData) {
    ...
    // 2. Mostrar la app inmediatamente
    document.getElementById("loginPage").style.display = "none";
    document.getElementById("verificationPage").style.display = "none";
    ...
    // 4. Si la pestaña activa es "inicio", renderizar desde caché de datos
    const activeTab = document.querySelector('.tab-content.active')?.id;
    if (activeTab === 'tab-inicio') {
      if (typeof renderDashboardFromCache === 'function') {
        renderDashboardFromCache();
        console.log('📦 Dashboard renderizado desde caché de datos (instantáneo)');
      }
    }

    // 5. Actualizar bienvenida
    ...
    // 6. Cargar datos en segundo plano (sin bloquear)
    this._cargarDatosEnBackground(uid);

    // 7. Llamar a cargarDashboard para refrescar (no esperamos)
    ...
    try {
      ...
      if (cachedUserData) {
        try {
          ...
          // Si la pestaña activa es inicio, renderizar desde caché de datos
          const activeTab = document.querySelector('.tab-content.active')?.id;
          if (activeTab === 'tab-inicio' && typeof renderDashboardFromCache === 'function') {
            renderDashboardFromCache();
          }
