...
const Auth = {
  ...
  async createUserDocument(userId, email, username, extraData = {}) {
    ...
    // BUG CORREGIDO: este mensaje de bienvenida se escribía en la colección
    // antigua 'mensajes' (doc por usuario, con un array 'mensajes' dentro).
    // La pantalla de "Mis mensajes" del usuario ya no lee de ahí -- lee de
    // Storage.getMensajesSoporteUsuario(), que consulta la subcolección
    // users/{uid}/mensajes con documentos individuales (fromUid/toUid). El
    // mensaje de bienvenida se creaba de verdad, pero un usuario nuevo
    // nunca llegaba a verlo en ningún sitio de la app. Ahora se envía con
    // Storage.enviarMensajeSoporte(), la misma función que usa el admin
    // para responder mensajes de soporte, así queda exactamente donde la
    // pantalla del usuario lo busca.
    try {
      ...
      if (adminUid) {
        ...
        const textoPeriodoGratis = (userData.premium && fechaExpiraStr)
          ...
          : '';
        await Storage.enviarMensajeSoporte(
          adminUid,
          userId,
          `👋 ¡Bienvenido a RI5, ${username}! Este es tu espacio de soporte directo con el administrador. Aquí recibirás notificaciones importantes y puedes enviar tus consultas.${textoPeriodoGratis} ¡Disfruta de la app! 🏃`
        );
        console.log('✅ Mensaje de bienvenida creado');
      } else {
        console.warn('⚠️ No se pudo crear el mensaje de bienvenida: no hay ningún admin registrado todavía');
      }
    } catch (e) {
      console.warn('Error creando mensaje de bienvenida:', e);
    }
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        if (!userDoc.exists) {
          ...
          // 🔥 FIX (punto menor [14] de la ronda de optimización): antes esta
          // rama hacía update({ emailVerified: true }) -- redundante, porque
          // createUserDocument() ya deja ese campo en true al crear el
          // documento (línea ~131) y nada en el código lo vuelve a poner en
          // false después -- seguido de un SEGUNDO get() del mismo
          // users/{uid} que ya se había leído dos líneas más arriba (userDoc,
          // sin ningún cambio de por medio al quitar el update). Se elimina
          // el update y se reutiliza el userDoc ya leído, sin volver a
          // consultar Firestore.
        }
  ...
  // Bloque compartido por _cargarDatosYMostrarApp() y la ruta de caché de
  // checkSavedSession(): revela mainContent y rellena el mensaje de
  // bienvenida + el campo #name. Antes este bloque estaba repetido letra
  // por letra en los dos sitios; ahora recibe userData como parámetro
  // explícito para que ambos sitios sigan pasando exactamente los mismos
  // datos que ya pasaban antes de extraerlo.
  _mostrarAppYBienvenida(userData) {
    document.getElementById("loginPage").style.display = "none";
    document.getElementById("verificationPage").style.display = "none";
  ...
  async _cargarDatosYMostrarApp(uid, email, userData) {
    ...
    try {
      // Último cálculo
      // 🔥 FIX (ronda de optimización Firestore): userData ya es el doc de
      // users/{uid} leído en el login -- se le pasa su ultimoCalculoId para
      // que Storage.getUltimoCalculo no tenga que releerlo.
      const calc = await Storage.getUltimoCalculo(uid, userData && userData.ultimoCalculoId);
      if (calc) {
    ...
    this._mostrarAppYBienvenida(AppState.currentUserData);
  ...
  async _cargarDatosEnBackground(uid, ultimoCalculoIdConocido) {
    ...
    promises.push(Storage.getUltimoCalculo(uid, ultimoCalculoIdConocido).then(calc => {
      if (calc) {
        AppState.setLastCalc(calc);
        localStorage.setItem('ri5_last_calculation', JSON.stringify(calc));
        if (window.Training) Training.mostrarResultadosGuardados(calc);
  ...
  async registerUser() {
    ...
    btn.disabled = true;
    // Mismo tratamiento que el botón de acceder (ver loginUser): animación
    // de letras dentro del propio botón en vez de un simple cambio de
    // texto, para que ambos botones de acceso se sientan consistentes.
    // Mismo motivo que en loginUser: este botón también tiene fondo
    // dorado, así que al asentarse las letras viran a var(--bg-primary)
    // en vez de al dorado del efecto original, para que sigan siendo
    // legibles en los dos temas sin perder los colores de nivel de la
    // entrada.
    Utils._animarTextoDorado(btn, 'REGISTRANDO...', 'var(--bg-primary)');
    // Sin esto, el fundido a opacity:0.4 que aplica .button-main:disabled
    // mientras el botón está deshabilitado (aquí, durante todo el
    // registro) apagaba también los colores de la animación de arriba,
    // dejándolos apagados/grisáceos -- se restaura la opacidad real al
    // terminar (ver el 'finally' de más abajo), para no romper el
    // atenuado normal de este botón cuando se deshabilita por la casilla
    // de protección de datos sin marcar.
    btn.style.opacity = '1';
  ...
  async loginUser() {
    ...
    btn.disabled = true;
    // Misma animación de letras entrando y virando a dorado que ya usa el
    // overlay de carga (Utils._animarTextoDorado) -- pero aquí, dentro del
    // propio botón, en vez de textContent plano. Antes se le añadía
    // también la clase 'pulse', que no hacía nada (no existe ninguna
    // regla CSS '.pulse' en toda la app) -- quitada.
    // Este botón (.button-main) tiene fondo dorado (var(--gold)); las
    // letras entran igualmente con los colores de nivel de la app, pero
    // al asentarse viran a var(--bg-primary) (el mismo color que usa
    // normalmente el texto de este botón) en vez de al dorado del efecto
    // original, que quedaba invisible sobre el propio fondo dorado del
    // botón en los dos temas.
    Utils._animarTextoDorado(btn, 'ACCEDIENDO...', 'var(--bg-primary)');
    // Mismo motivo que en registerUser: se restaura la opacidad real del
    // botón al terminar (en los 4 puntos de salida de abajo), para que el
    // atenuado a 0.4 de .button-main:disabled no apague también los
    // colores de esta animación mientras dura el login.
    btn.style.opacity = '1';
    ...
    try {
      ...
      if (cachedUserData) {
        try {
          ...
          this._mostrarAppYBienvenida(userData);
          ...
          await this._cargarDatosEnBackground(uid, userData && userData.ultimoCalculoId);
