...
// Versión: 5.20 - Restauración inmediata del dashboard desde caché HTML
// ====================
...
const Auth = {
  ...
  // === CARGA COMPLETA CON CACHÉ DE HTML ===
  async _cargarDatosYMostrarApp(uid, email, userData) {
    ...
    // 1. Mostrar la app inmediatamente
    document.getElementById("loginPage").style.display = "none";
    document.getElementById("verificationPage").style.display = "none";
    ...
    // 2. Restaurar pestaña activa
    await UI.restaurarEstado();

    // 3. Si la pestaña activa es "inicio", restaurar el dashboard desde caché HTML
    const activeTab = document.querySelector('.tab-content.active')?.id;
    if (activeTab === 'tab-inicio' && typeof restaurarDashboardDesdeCache === 'function') {
      restaurarDashboardDesdeCache();
      console.log('📦 Dashboard restaurado desde caché HTML (instantáneo)');
    }

    // 4. Actualizar bienvenida
    ...
    // 5. Cargar datos en segundo plano (sin bloquear)
    this._cargarDatosEnBackground(uid);

    // 6. Llamar a cargarDashboard para refrescar (no esperamos)
    if (typeof cargarDashboard === 'function') {
      cargarDashboard().catch(e => console.warn('Error en cargarDashboard en background:', e));
    }
  ...
  async _cargarDatosEnBackground(uid) {
    ...
    if (typeof cargarDashboard === 'function') {
      promises.push(cargarDashboard().catch(e => console.warn('Error cargando dashboard en background:', e)));
    }
    ...
    try {
      ...
      if (cachedUserData) {
        try {
          ...
          // Si la pestaña activa es inicio, restaurar desde caché HTML
          const activeTab = document.querySelector('.tab-content.active')?.id;
          if (activeTab === 'tab-inicio' && typeof restaurarDashboardDesdeCache === 'function') {
            restaurarDashboardDesdeCache();
          }
