...
const ShareCard = {
  ...
  _cargarQRLib() {
    return new Promise(resolve => {
      if (window.qrcode && this._qrLibCargada) { resolve(); return; }
      if (window.qrcode) { this._qrLibCargada = true; resolve(); return; }
      const s = document.createElement('script');
      s.src = 'https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.0.3/qrcode.min.js';
      s.onload = () => { this._qrLibCargada = true; resolve(); };
      s.onerror = () => resolve(); // si falla, simplemente no se dibuja el QR
  ...
  _dibujarTexturaPanel(ctx, x, y, w, h) {
    ctx.save();
    ctx.fillStyle = 'rgba(192,160,96,0.10)';
    const paso = 42;
    for (let px = x + paso / 2; px < x + w; px += paso) {
      for (let py = y + paso / 2; py < y + h; py += paso) {
        ctx.beginPath();
  ...
  _dibujarPanelCentral(ctx, datos, x, y, w, h) {
    ...
    bg.addColorStop(0, 'rgba(192,160,96,0.14)');
    bg.addColorStop(1, 'rgba(192,160,96,0.03)');
    ctx.fillStyle = bg;
  ...
  async _generarCanvas(datos) {
    ...
    // Cabecera, centrada — usuario y fecha con ancho protegido (nombres
    // de usuario largos no pueden salirse de la tarjeta)
    const logo = await this._cargarLogo();
    let y = 118;
    if (logo) ctx.drawImage(logo, cx - 36, y - 64, 72, 72);
    ctx.fillStyle = '#c0a060';
    ctx.font = '600 30px "Courier New", monospace';
    ctx.fillText('RI5 · RUNNING INTELLIGENCE', cx, y + 32);
    ctx.fillStyle = '#e8e8e8';
    this._fillTextAjustado(ctx, `${datos.username} · ${datos.fecha}`, cx, y + 68, W - 140, '400', '"Courier New", monospace', 26, 18);
    ...
    // pulsables. El texto de la URL sirve de respaldo para quien prefiera
    // escribirla a mano.
    ctx.fillStyle = 'rgba(255,255,255,0.55)';
    ctx.font = '600 24px "Courier New", monospace';
    ...
    y += 28;
    const qrSize = 190;
    await this._dibujarQR(ctx, this._urlUnirse(), cx - qrSize / 2, y, qrSize);
    y += qrSize + 46;
    ctx.fillStyle = '#c0a060';
  ...
  async mostrarModal(datos) {
    ...
    // min-width:0 es la corrección estándar para un bug clásico de
    // flexbox: por defecto un hijo de un contenedor flex (aquí, el
    // propio modal dentro de overlay, y la imagen dentro del modal) NO
    // se encoge por debajo de su tamaño intrínseco a menos que se le
    // indique min-width:0 explícitamente. Sin esto, en algunos
    // navegadores (Safari sobre todo) la imagen se mostraba a su ancho
    // real (1080px) en vez de encogerse a la pantalla, cortando todo lo
    // que quedaba fuera — es lo que se veía en la captura.
    modal.style.cssText = `
      background:#0f0f0f; border-radius:20px; padding:18px; max-width:420px; width:100%;
      min-width:0; box-sizing:border-box;
      border:1px solid #2a2a2a; display:flex; flex-direction:column; gap:16px;
      max-height:90vh; overflow:auto; overflow-x:hidden; font-family:'Courier New', monospace;
    `;
    ...
    modal.innerHTML = `
      ...
      <img src="${imgDataUrl}" style="width:100%; max-width:100%; min-width:0; height:auto; border-radius:12px; display:block; border:1px solid #2a2a2a; box-sizing:border-box;">
