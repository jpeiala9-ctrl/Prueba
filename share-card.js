...
const ShareCard = {
  ...
  _dibujarCiudadEnPanel(ctx, ciudad, x, y, w, h) {
    ...
    ctx.fillStyle = '#ffffff';
    this._fillTextAjustado(ctx, ciudad.ciudad, cx, cy + 55, w - 90, '700', '"Courier New", monospace', 46);

    if (ciudad.region && ciudad.region !== ciudad.ciudad) {
      ctx.fillStyle = 'rgba(255,255,255,0.55)';
      this._fillTextAjustado(ctx, ciudad.region, cx, cy + 96, w - 90, '400', '"Courier New", monospace', 26);
    }
    ctx.restore();
  },
  ...
  _dibujarFallbackEnPanel(ctx, tipo, x, y, w, h) {
    ...
    ctx.fillText(this._tipoEmoji(tipo), cx, cy + 20);
    ctx.globalAlpha = 1;
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.font = '600 24px "Courier New", monospace';
    ctx.fillText('SESIÓN SIN GPS', cx, cy + 80);
    ctx.restore();
  },
  ...
  async _generarCanvas(datos) {
    ...
    this._dibujarGraficaSemana(ctx, datos.kmPorDia, cx, y + 18, 780, 136);
    y += 18 + 136 + 66;
