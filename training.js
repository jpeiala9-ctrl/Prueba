...
const Training = {
  ...
  // Carga una imagen (p.ej. la foto de perfil de Firebase Storage) para
  // poder dibujarla en el canvas. Con crossOrigin='anonymous': si el
  // recurso no manda cabeceras CORS válidas, el navegador la bloquea para
  // canvas.toBlob() (imagen "tainted") -- en ese caso resolvemos con null
  // en vez de reventar toda la generación de la tarjeta, y se dibuja el
  // círculo de nivel sin foto (como en el resto de la app).
  _cargarImagen(url) {
  ...
  // usuario. Además se añade su foto de perfil (o un avatar por defecto)
  // con el anillo del color de SU nivel, igual que en el resto de la app.
  async _generarTarjetaImagen() {
    ...
    // Foto de perfil + color de nivel + username del usuario actual. Si
    // algo falla (sin conexión, sin datos aún) se sigue con los valores
    // por defecto en vez de romper la generación de la tarjeta.
    let photoURL = null;
    let colorNivel = GOLD;
    ...
    try {
      ...
      console.warn('No se pudieron cargar foto/nivel/username para la tarjeta:', e);
    }
    const avatarImg = await this._cargarImagen(photoURL);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const avatarR = 80;
        const avatarCX = W / 2, avatarCY = 150;

        // Dibuja todo el contenido que crece hacia abajo (avatar, título,
        // datos, predicciones, zonas y cartel final) y devuelve el punto Y
        // justo debajo de lo último dibujado. Con ctx = null no toca el
        // canvas -- solo recorre los mismos incrementos para MEDIR cuánto
        // ocupa el contenido antes de crear el canvas de verdad. Así el
        // pie de página nunca se coloca en una coordenada fija (H - 60)
        // que antes podía quedar solapada con el cartel "CALCULA LAS
        // TUYAS EN RI5" cuando había muchas predicciones/zonas, o con
        // demasiado hueco de sobra cuando había pocas.
        const dibujarContenido = (ctx) => {
          // Avatar circular con el anillo del color de nivel del usuario
          if (ctx) {
            ctx.save();
            ctx.beginPath();
            ...
            ctx.clip();
            if (avatarImg) {
              // "cover": recorta la imagen a un cuadrado centrado antes de
              // encajarla en el círculo, para no deformarla si no es 1:1.
              const side = Math.min(avatarImg.width, avatarImg.height);
              const sx = (avatarImg.width - side) / 2;
              const sy = (avatarImg.height - side) / 2;
              ctx.drawImage(avatarImg, sx, sy, side, side, avatarCX - avatarR, avatarCY - avatarR, avatarR * 2, avatarR * 2);
            } else {
              ...
              ctx.fillText('👤', avatarCX, avatarCY + 6);
              ctx.textBaseline = 'alphabetic';
            }
            ctx.restore();
            ctx.strokeStyle = colorNivel;
            ctx.lineWidth = 7;
            ctx.beginPath();
            ctx.arc(avatarCX, avatarCY, avatarR, 0, Math.PI * 2);
            ctx.stroke();
          ...
          // Espacio real entre el borde del avatar y la cabecera "RI5"
          // (antes eran solo ~10px -- quedaba todo pegado). Ahora hay un
          // hueco de aire claro tanto tras el avatar como tras los
          // puntitos decorativos.
          const avatarBottom = avatarCY + avatarR;
          const dotsY = avatarBottom + 90;
          let y = dotsY + 90;
        ...
        ctx.fillStyle = TEXT_SEC;
        ctx.font = '400 22px Arial';
        ctx.fillText('RI5 · Running Intelligence', W / 2, finContenido + 60);
