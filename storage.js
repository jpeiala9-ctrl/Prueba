...
//                'friendRequests': id fijo `${from}_${to}`, set()
//                sobrescribe si se reenvía tras un rechazo) y los campos
...
//                amistad. Ver session-invites.js::_precargarUsuarios, que
//                ahora parte de alumnosAceptados en vez de friendIds para
//                un entrenador no-admin.
...
const Storage = {
  ...
  // set() sobrescribe si se reenvía tras un rechazo/revocación), pero en
  // su propia colección para no mezclar el estado de amistad con el de
  // entrenamiento.
  async sendTrainerRequest(fromUid, toUid) {
    ...
    try {
      ...
      if (!fromUser || !toUser) return false;
      const requestId = `${fromUid}_${toUid}`;
      await firebaseServices.db.collection('trainerRequests').doc(requestId).set({
        from: fromUid,
        to: toUid,
        fromUsername: fromUser.username,
        toUsername: toUser.username,
  ...
  // Usado tanto si el alumno decide dejar de aceptar sesiones de ese
  // entrenador como si el entrenador decide quitar a un alumno.
