...
const Storage = {
  ...
  // 🔥 FIX (ronda de optimización Firestore): acepta un segundo parámetro
  // opcional ultimoCalculoIdConocido -- si quien llama YA tiene ese id
  // (p.ej. AppState.currentUserData.ultimoCalculoId, leído momentos antes
  // en el mismo flujo de login), se evita la lectura de users/{uid} que
  // solo servía para sacar ese mismo campo. Si no se pasa, se comporta
  // exactamente igual que antes (lee users/{uid} para obtenerlo).
  async getUltimoCalculo(uid, ultimoCalculoIdConocido) { 
    if (!uid) return null; 
    try { 
      let ultimoCalculoId = ultimoCalculoIdConocido;
      if (!ultimoCalculoId) {
        const userDoc = await firebaseServices.db.collection('users').doc(uid).get(); 
        if (!userDoc.exists) return null; 
        ultimoCalculoId = userDoc.data().ultimoCalculoId; 
      }
