...
const Storage = {
  ...
  // ==================== FUNCIONES ANTIGUAS DE MENSAJES (se mantienen por compatibilidad) ====================
  ...
  async acceptFriendRequest(requestId, fromUid, toUid) { 
    ...
    try { 
      // Leemos el estado real de friendIds de ambos ANTES de escribir, en
      // vez de usar FieldValue.increment(1) a ciegas. Así, si por lo que
      // sea esta función se llamase dos veces para la misma solicitud (p.ej.
      // doble tap con la UI aún no actualizada), friendsCount se recalcula
      // siempre como el tamaño real del array y no queda descuadrado.
      ...
      const nuevoFromFriendIds = fromFriendIds.includes(toUid) ? fromFriendIds : [...fromFriendIds, toUid];
      const nuevoToFriendIds = toFriendIds.includes(fromUid) ? toFriendIds : [...toFriendIds, fromUid];
      ...
      batch.update(fromRef, { 
        friendIds: nuevoFromFriendIds, 
        friendsCount: nuevoFromFriendIds.length 
      }); 
      batch.update(toRef, { 
        friendIds: nuevoToFriendIds, 
        friendsCount: nuevoToFriendIds.length 
      }); 
      await batch.commit(); 
      return true; 
    } catch (error) { 
  ...
  async removeFriend(uid, friendUid) { 
    ...
    try { 
      // Igual que en acceptFriendRequest: leemos el array real antes de
      // escribir y recalculamos friendsCount como su longitud, en vez de
      // increment(-1). Con increment(-1), si esta función se disparaba dos
      // veces para la misma amistad (doble click con la UI todavía sin
      // refrescar), el segundo arrayRemove no hacía nada (la persona ya no
      // estaba en la lista) pero el contador SÍ volvía a bajar, dejando un
      // friendsCount negativo o incorrecto de forma silenciosa. Así queda
      // siempre coherente con el array real, sea cual sea el número de
      // veces que se llame.
      const [userDoc, friendDoc] = await Promise.all([
        firebaseServices.db.collection('users').doc(uid).get(),
        firebaseServices.db.collection('users').doc(friendUid).get()
      ]);
      const userFriendIds = (userDoc.data()?.friendIds || []).filter(id => id !== friendUid);
      const friendFriendIds = (friendDoc.data()?.friendIds || []).filter(id => id !== uid);
      ...
      batch.update(friendRef, { 
        ...
        friendsCount: friendFriendIds.length 
      }); 
      await batch.commit(); 
      return true; 
    } catch (error) { 
  ...
  /**
   * Elimina un mensaje de la subcolección de un usuario.
   */
  async eliminarMensajeSoporteUsuario(usuarioUid, mensajeId) {
    if (!usuarioUid || !mensajeId) return false;
    try {
      await firebaseServices.db
        .collection('users')
        .doc(usuarioUid)
        ...
        .doc(mensajeId)
        .delete();
      console.log(`🗑️ Mensaje ${mensajeId} eliminado de la subcolección de ${usuarioUid}`);
      return true;
    } catch (error) {
      console.error('❌ Error eliminando mensaje de la subcolección:', error);
      return false;
    }
  },
  ...
  /**
   * Borra un mensaje "a Todos" SOLO del panel del admin (la colección
   * global 'soporteMensajes'). Las copias en la subcolección de cada
   * usuario se mantienen intactas: el mensaje sigue existiendo para ellos.
   */
  /**
  ...
  // ==================== PROPUESTAS DE SESIÓN (admin -> usuarios) ====================
  // Una "propuesta de sesión" es una plantilla que el admin escribe a mano
  // (nombre, km, zona, descripción) para un día concreto, y que envía a
  // uno o varios usuarios. Si el usuario la acepta, sustituye la sesión
  // que tenía generada ese día en su plan activo. Colección:
  // propuestasSesion/{id} = { deUid, deNombre, paraUid, estado, fecha
  // ('YYYY-MM-DD'), sesion:{nombre,km,zona,descripcion}, fechaCreacion }

  async enviarPropuestasSesion(uidsDestino, sesionData, fechaISO) {
    const admin = firebaseServices.auth.currentUser;
    if (!admin) throw new Error('No hay sesión de administrador activa');
    const adminDoc = await firebaseServices.db.collection('users').doc(admin.uid).get();
    const deNombre = (adminDoc.data() && adminDoc.data().username) || 'Entrenador';

    const batch = firebaseServices.db.batch();
    const idsCreados = [];
    uidsDestino.forEach(uid => {
      const ref = firebaseServices.db.collection('propuestasSesion').doc();
      idsCreados.push(ref.id);
      batch.set(ref, {
        deUid: admin.uid,
        deNombre,
        paraUid: uid,
        ...
        sesion: {
          ...
          km: sesionData.km,
          zona: sesionData.zona,
          descripcion: sesionData.descripcion
        },
        fechaCreacion: firebaseServices.Timestamp.now()
      });
    });
    await batch.commit();
    return idsCreados;
  ...
  async getPropuestasSesionPendientes(uid) {
    const snapshot = await firebaseServices.db.collection('propuestasSesion')
      .where('paraUid', '==', uid)
  ...
  // Devuelve { ok, aceptada, error? }. error puede ser 'sin_plan' (el
  // usuario no tiene ningún plan activo) o 'fuera_de_rango' (la fecha de
  // la propuesta no cae dentro de las fechas de su plan activo).
  async responderPropuestaSesion(propuestaId, aceptar) {
    const uid = firebaseServices.auth.currentUser.uid;
    const propRef = firebaseServices.db.collection('propuestasSesion').doc(propuestaId);
    const propDoc = await propRef.get();
    if (!propDoc.exists) return { ok: false };
    const prop = propDoc.data();
    if (prop.paraUid !== uid || prop.estado !== 'pendiente') return { ok: false };

    if (!aceptar) {
      await propRef.update({ estado: 'rechazada' });
      return { ok: true, aceptada: false };
    }

    const userDoc = await firebaseServices.db.collection('users').doc(uid).get();
    const planId = userDoc.data() && userDoc.data().ultimoPlanId;
    if (!planId) return { ok: false, error: 'sin_plan' };

    const planRef = firebaseServices.db.collection('users').doc(uid).collection('planes').doc(planId);
    const planDoc = await planRef.get();
    if (!planDoc.exists) return { ok: false, error: 'sin_plan' };
    const planData = planDoc.data();

    const fechaInicio = new Date(planData.params.fechaInicio);
    fechaInicio.setHours(0, 0, 0, 0);
    const fechaObjetivo = new Date(prop.fecha + 'T00:00:00');
    const diaGlobal = Math.round((fechaObjetivo - fechaInicio) / 86400000) + 1;

    const sesiones = planData.sesiones || [];
    const idx = sesiones.findIndex(s => s.diaGlobal === diaGlobal);
    if (idx === -1) return { ok: false, error: 'fuera_de_rango' };

    const anterior = sesiones[idx];
    sesiones[idx] = {
      diaGlobal: anterior.diaGlobal,
      semana: anterior.semana,
      diaSemana: anterior.diaSemana,
      fase: anterior.fase,
      nivel: anterior.nivel,
      tipo: 'personalizada',
      color: 'sesion-personalizada',
      letra: 'E',
      tieneFuerza: false,
      duracion: anterior.duracion || 0,
      asignadaPorAdmin: true,
      entrenador: prop.deNombre || 'Entrenador',
      detalle: {
        nombre: prop.sesion.nombre,
        descripcion: prop.sesion.descripcion,
        estructura: prop.sesion.descripcion,
        objetivo: prop.sesion.descripcion,
        porque: '',
        sensacion: '',
        zona: prop.sesion.zona,
        distanciaEstimada: prop.sesion.km,
        pasosDetallados: []
      }
    };

    await planRef.update({ sesiones });
    await propRef.update({ estado: 'aceptada' });
